...
test('16.0.210 historical maturity assertions coexist with current 17.0.14 version metadata', () => {
  ...
  assert.equal(lock.version, '17.0.14');
  assert.equal(lock.packages[''].version, '17.0.14');
  assert.equal(pkg.scripts['test:source'], 'node scripts/validate-17-0-14.js');
  assert.equal(version.version, '17.0.14');
  assert.equal(version.build, '17.0.14');
