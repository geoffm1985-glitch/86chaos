...
test('16.0.210 historical maturity assertions coexist with current 17.0.19 version metadata', () => {
  ...
  assert.equal(lock.version, '17.0.19');
  assert.equal(lock.packages[''].version, '17.0.19');
  assert.equal(pkg.scripts['test:source'], 'node scripts/validate-17-0-19.js');
  assert.equal(version.version, '17.0.19');
  assert.equal(version.build, '17.0.19');
  assert.equal(version.releaseTitle, 'Release Gate Expected Version Pinning Repair');
  assert.match(appCore, /CURRENT_VERSION = '17\.0\.19'/);
  assert.match(apiVersion, /APP_VERSION = '17\.0\.19'/);
  assert.match(apiVersion, /SECURITY_SCHEMA_VERSION = '17\.0\.19'/);
});
