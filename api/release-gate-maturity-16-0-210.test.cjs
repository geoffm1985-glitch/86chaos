...
test('16.0.210 historical maturity assertions coexist with current 17.0.15 version metadata', () => {
  ...
  assert.equal(lock.version, '17.0.15');
  assert.equal(lock.packages[''].version, '17.0.15');
  assert.equal(pkg.scripts['test:source'], 'node scripts/validate-17-0-15.js');
  assert.equal(version.version, '17.0.15');
  assert.equal(version.build, '17.0.15');
  assert.equal(version.releaseTitle, 'Schedule Publish Authoritative Evidence Repair');
  assert.match(appCore, /CURRENT_VERSION = '17\.0\.14'/);
  assert.match(apiVersion, /APP_VERSION = '17\.0\.14'/);
  assert.match(apiVersion, /SECURITY_SCHEMA_VERSION = '17\.0\.14'/);
});
