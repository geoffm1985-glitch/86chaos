...
test('16.0.210 historical maturity assertions coexist with current 16.0.231 version metadata', () => {
  const pkg = json('package.json');
  const lock = json('package-lock.json');
  const version = json('public/version.json');
  const appCore = read('src/core/appCore.js');
  const apiVersion = read('api/_version.js');
  assert.equal(pkg.version, '16.0.231');
  assert.equal(lock.version, '16.0.231');
  assert.equal(lock.packages[''].version, '16.0.231');
  assert.equal(pkg.scripts['test:source'], 'node scripts/validate-16-0-231.js');
  assert.equal(version.version, '16.0.231');
  assert.equal(version.build, '16.0.231');
  assert.equal(version.releaseTitle, 'Runtime Evidence and Partial Resume Repair');
  assert.match(appCore, /CURRENT_VERSION = '16\.0\.231'/);
  assert.match(apiVersion, /APP_VERSION = '16\.0\.231'/);
  assert.match(apiVersion, /SECURITY_SCHEMA_VERSION = '16\.0\.231'/);
});
