...
test('16.0.210 historical maturity assertions coexist with current 17.0.23 version metadata', () => {
  ...
  assert.equal(lock.version, '17.0.23');
  assert.equal(lock.packages[''].version, '17.0.23');
  assert.equal(pkg.scripts['test:source'], 'node scripts/validate-17-0-22.js');
  assert.equal(version.version, '17.0.23');
  assert.equal(version.build, '17.0.23');
  assert.equal(version.releaseTitle, 'Request Off Cutoff and Blackout Policy');
  assert.match(appCore, /CURRENT_VERSION = '17\.0\.22'/);
  assert.match(apiVersion, /APP_VERSION = '17\.0\.22'/);
  assert.match(apiVersion, /SECURITY_SCHEMA_VERSION = '17\.0\.22'/);
});
