...
test('16.0.210 historical maturity assertions coexist with current 17.0.1 version metadata', () => {
  const pkg = json('package.json');
  const lock = json('package-lock.json');
  const version = json('public/version.json');
  const appCore = read('src/core/appCore.js');
  const apiVersion = read('api/_version.js');
  assert.equal(pkg.version, '17.0.1');
  assert.equal(lock.version, '17.0.1');
  assert.equal(lock.packages[''].version, '17.0.1');
  assert.equal(pkg.scripts['test:source'], 'node scripts/validate-17-0-1.js');
  assert.equal(version.version, '17.0.1');
  assert.equal(version.build, '17.0.1');
  assert.equal(version.releaseTitle, 'Publication Integrity and Hostile Certification Repair');
  assert.match(appCore, /CURRENT_VERSION = '17\.0\.1'/);
  assert.match(apiVersion, /APP_VERSION = '17\.0\.1'/);
  assert.match(apiVersion, /SECURITY_SCHEMA_VERSION = '17\.0\.1'/);
});
