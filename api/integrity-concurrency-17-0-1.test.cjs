'use strict';
const test=require('node:test');const assert=require('node:assert/strict');const admin=require('./_firebase-admin-compat');const {saveDailyClose}=require('./_daily-close-service.cjs');const {mutateWaste}=require('./safe-write.js')._test;const {createTrace,event,createReadBarrier,instrumentFirestoreDb,recordSettlements,writeTrace}=require('../test-tools/firestore-emulator-trace.cjs');
if(!process.env.FIRESTORE_EMULATOR_HOST){test('integrity concurrency requires Firestore emulator',{skip:'FIREBASE EMULATOR fidelity required'},()=>{});}else{
  ...
  test('FIREBASE EMULATOR simultaneous waste deductions both commit and duplicate operation cannot double-decrement',async()=>{await db.collection('inventoryItems').doc('item').set({restaurantId:'rest',currentStock:10,revision:1});const trace=createTrace('waste-deterministic-concurrency',{fixtureSetupMs:0}),barrier=createReadBarrier(2,trace,'waste-first-transaction-read');const make=(operationId,stockDeducted,index,activeDb=db)=>mutateWaste(activeDb,{action:'waste-create',restaurantId:'rest',body:{operationId,data:{itemId:'item',itemName:'Synthetic',stockDeducted,restaurantId:'rest'}},actor:'manager',nowIso:new Date().toISOString()});let finalState={unavailable:true};try{const settled=await Promise.allSettled([make('waste_operation_aaaaaaaa',1,0,instrumentFirestoreDb(db,{trace,callerId:'waste-a',beforeFirstRead:barrier})),make('waste_operation_bbbbbbbb',2,1,instrumentFirestoreDb(db,{trace,callerId:'waste-b',beforeFirstRead:barrier}))]);recordSettlements(trace,settled);assert.equal(settled.every(row=>row.status==='fulfilled'),true,JSON.stringify(trace.settlements));const [a,b]=settled.map(row=>row.value);assert.equal(a.status,'committed');assert.equal(b.status,'committed');assert.equal(trace.events.some(row=>row.type==='read-overlap'&&row.callerIds.length===2),true);assert.equal((await db.collection('inventoryItems').doc('item').get()).data().currentStock,7);const retry=await make('waste_operation_aaaaaaaa',1);assert.equal(retry.status,'idempotent');assert.equal((await db.collection('inventoryItems').doc('item').get()).data().currentStock,7);assert.equal((await db.collection('wasteLogs').get()).size,2);finalState={currentStock:7,wasteLogCount:2,distinctCallers:true,overlap:true};}finally{writeTrace(trace,finalState);}});
}
