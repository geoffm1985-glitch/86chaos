...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'restoreSelected') {
      ...
      await db.collection('system').doc('backupStatus').set({ lastSelectiveRestoreAt: restored.finishedAt, lastSelectiveRestorePath: storagePath, lastSelectiveRestoreCollections: selectedCollections, lastSelectiveRestoreDocuments: restoredDocuments, actor: auth.email || auth.uid, version: '14.0.0' }, { merge: true });
    }
