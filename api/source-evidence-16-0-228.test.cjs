...
const {captureSourceIdentity,compareSourceIdentity,excludedFile}=require('../scripts/86chaos-release-gate/source-identity.cjs');
...
test('full certification rejects null, wrong-branch, and same-version wrong-source identity',()=>{const base={version:'17.0.4',sourceHash:'a'.repeat(64),commit:'commit-1',branch:'testing',dirty:false};const options={requireCertification:true,expectedVersion:'17.0.4',expectedCommit:'commit-1',expectedBranch:'testing',expectedManifest:'a'.repeat(64)};assert.equal(compareSourceIdentity(base,base,options).ok,true);assert.equal(compareSourceIdentity({...base,commit:null},{...base,commit:null},options).ok,false);assert.equal(compareSourceIdentity({...base,branch:'main'},{...base,branch:'main'},options).ok,false);assert.equal(compareSourceIdentity({...base,sourceHash:'c'.repeat(64)},{...base,sourceHash:'c'.repeat(64)},options).ok,false);assert.equal(compareSourceIdentity(base,base,{...options,archiveSha256:'not-a-sha'}).ok,false);});
test('generated release artifacts do not change source manifest',()=>{
  const dir=fs.mkdtempSync(path.join(os.tmpdir(),'chaos-generated-artifact-'));
  try {
    fs.writeFileSync(path.join(dir,'package.json'),'{"version":"17.0.4"}');
    fs.writeFileSync(path.join(dir,'source.js'),'stable');
    const before=captureSourceIdentity(dir);
    fs.writeFileSync(path.join(dir,'86chaos-release-gate-SLIM-UPLOAD-ME.zip'),'generated report bytes');
    fs.mkdirSync(path.join(dir,'test-results'),{recursive:true});
    fs.writeFileSync(path.join(dir,'test-results','run.json'),'generated');
    const after=captureSourceIdentity(dir);
    assert.equal(after.sourceHash,before.sourceHash);
    assert.equal(excludedFile('86chaos-release-gate-SLIM-UPLOAD-ME.zip'),true);
    assert.equal(excludedFile('public/build-identity.json'),true);
  } finally { fs.rmSync(dir,{recursive:true,force:true}); }
});
