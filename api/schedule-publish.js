...
module.exports = async function handler(req,res) {
  ...
  try{
    ...
    const elevated=Boolean(ctx.isSuperAdmin||ctx.user?.isAdmin||ctx.user?.isOwner||ctx.user?.accountOwner||ctx.user?.workspaceOwner||ctx.permissions?.schedule===true);if(!elevated)return res.status(403).json({ok:false,error:'Schedule publishing permission is required.'});
...
module.exports._test={MAX_BODY_BYTES,bodyBytes,parseBody};
