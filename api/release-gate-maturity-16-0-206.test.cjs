...
test('16.0.206 version metadata is consistent', () => {
  const pkg = JSON.parse(read('package.json'));
  const lock = JSON.parse(read('package-lock.json'));
  const version = JSON.parse(read('public/version.json'));
  const apiVersion = read('api/_version.js');
  const appCore = read('src/core/appCore.js');
  assert.equal(pkg.version, '16.0.206');
  assert.equal(lock.version, '16.0.206');
  assert.equal(lock.packages[''].version, '16.0.206');
  assert.equal(pkg.scripts['test:source'], 'node scripts/validate-16-0-206.js');
  assert.equal(version.version, '16.0.206');
  assert.equal(version.build, '16.0.206');
  assert.match(apiVersion, /APP_VERSION = '16\.0\.206'/);
  assert.match(apiVersion, /SECURITY_SCHEMA_VERSION = '16\.0\.206'/);
  assert.match(appCore, /CURRENT_VERSION = '16\.0\.206'/);
});
