...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    console.error('system-admin workspaces failed:', error?.message || error);
    return res.status(500).json({ ok: false, code: 'system-admin-workspaces-failed', error: 'Could not load workspaces.' });
  }
};
