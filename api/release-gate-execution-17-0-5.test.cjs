...
test('17.0.5 actual preflight pins immutable identity and executes mandatory source tests; mismatches execute zero tests',()=>{
 ...
 fs.copyFileSync(path.join(root,'release-source-manifest.json'),path.join(repo,'release-source-manifest.json'));
 ...
 const verifiedIdentity={identityStampStatus:'verified',sourceEvidence:'bundled-manifest',workspaceVerification:'vercel-git-metadata'};
 const client={...local,sourceManifestHash:local.sourceHash,firebaseTestingProject:'chaos-test-d1601',...verifiedIdentity,...config};
 const server={version:local.version,sourceManifestHash:local.sourceHash,gitCommit:local.commit,gitBranch:'testing',vercelDeploymentId:'dpl_fixture1705',vercelDeploymentUrl:pinned,vercelProjectId:'prj_fixture1705',firebaseTestingProject:'chaos-test-d1601',...verifiedIdentity,...config};
 ...
 Object.assign(env,{NODE_OPTIONS:`--require=${JSON.stringify(stub)}`,GATE_HTTP_FIXTURE:fixture,GATE_HTTP_TRACE:path.join(temp,'http.log'),APP_URL:alias,CHAOS_BASE_URL:alias,CHAOS_FIREBASE_AUTH_REFERRER_URL:'https://86chaos-git-testing-cheers-portal-s-projects.vercel.app',CHAOS_CERTIFICATION_MODE:'true',CHAOS_ALLOW_MUTATION:'false',CHAOS_QA_AUTO_PROVISION_TEST_USERS:'false',REACT_APP_FIREBASE_PROJECT_ID:'chaos-test-d1601',REACT_APP_FIREBASE_API_KEY:'fixture-key',REACT_APP_FIREBASE_AUTH_DOMAIN:'chaos-test-d1601.firebaseapp.com',REACT_APP_FIREBASE_APP_ID:'fixture-app'});
