...
test('17.0.5 actual preflight pins immutable identity and executes mandatory source tests; mismatches execute zero tests',()=>{
 ...
 const client={...local,sourceManifestHash:local.sourceHash,firebaseTestingProject:'chaos-test-d1601',...config};
 const server={version:local.version,sourceManifestHash:local.sourceHash,gitCommit:local.commit,gitBranch:'testing',vercelDeploymentId:'dpl_fixture1705',vercelDeploymentUrl:pinned,vercelProjectId:'prj_fixture1705',firebaseTestingProject:'chaos-test-d1601',...config};
 ...
 const started=JSON.parse(fs.readFileSync(path.join(good.runDir,'preflight-test-start.json'),'utf8'));assert.equal(started.started,true);assert.equal(started.exitCode,0);assert.equal(started.appUrl,pinned);assert.equal(started.certified,false);assert.match(good.result.stdout,/17\.0\.5 source validation passed/);
