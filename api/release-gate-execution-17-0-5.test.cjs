...
test('17.0.5 actual preflight pins immutable identity and executes mandatory source tests; mismatches execute zero tests',()=>{
 ...
 const started=JSON.parse(fs.readFileSync(path.join(good.runDir,'preflight-test-start.json'),'utf8'));assert.equal(started.started,true);assert.equal(started.group,'source validator');assert.equal(started.command,'npm run test:source');assert.equal(started.exitCode,0);assert.equal(started.appUrl,alias);assert.equal(started.certified,false);assert.match(good.result.stdout,/\[release-check\] source validator: npm run test:source/);
