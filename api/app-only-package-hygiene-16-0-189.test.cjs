...
test('app-only hygiene rejects forbidden source-controlled artifacts and local secrets', () => {
  const offenders = walk(root).filter(rel => !rel.startsWith('docs/') && !rel.startsWith('.gitignore') && !rel.startsWith('test-results/') && !rel.startsWith('scripts/python/__pycache__'));
...
test('source inventory excludes generated release-gate results from package scans', () => {
  ...
  assert.doesNotMatch(fs.readFileSync(path.join(root, 'scripts/validate-16-0-189.js'), 'utf8'), /test-results\/.*allowed/i);
