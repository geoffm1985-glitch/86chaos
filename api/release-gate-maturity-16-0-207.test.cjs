...
test('16.0.208 historical maturity assertions coexist with current 17.1.1 version metadata', () => {
  ...
  assert.equal(lock.version, '17.1.1');
  assert.equal(lock.packages[''].version, '17.1.1');
  assert.equal(pkg.scripts['test:source'], 'node scripts/validate-17-1-1.js');
  assert.equal(version.version, '17.1.1');
  assert.equal(version.build, '17.1.1');
