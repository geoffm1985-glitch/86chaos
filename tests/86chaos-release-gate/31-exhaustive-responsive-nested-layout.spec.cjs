...
async function auditViewport(browser, vp, testInfo) {
  ...
  try {
    ...
    for(const route of ROUTE_SPECS){
      ...
      const routeText=await gotoTab(page,route.tab,{settleMs:0,timeout:8000,maxText:16000,force:true});
      ...
      for(let i=0;i<states.length;i++){
        ...
        const traversalPath=i>0?await recoverSiblingStatePath(page,states[i-1],path,route.tab):path;
        const state=await applyStatePath(page,traversalPath,{strict:false});
