...
async function settleReactFrames(page) {
...
async function resetRouteRoot(page, route) {
  ...
  await page.evaluate(async ({ neutralTab, targetTab }) => {
    const navigate = async tab => {
      ...
      window.history.replaceState({ ...(window.history.state || {}), tab }, '', `${url.pathname}${url.search}${url.hash}`);
      window.dispatchEvent(new PopStateEvent('popstate', { state: { tab } }));
      window.dispatchEvent(new CustomEvent('chaos:navigate-tab', { detail: { tab } }));
...
function sharedStatePathDepth(previousPath = [], nextPath = []) {
  const limit = Math.min(previousPath.length, nextPath.length);
  let depth = 0;
  while (depth < limit && String(previousPath[depth]) === String(nextPath[depth])) depth += 1;
  return depth;
}

async function visibleAncestorExitControl(page) {
  const exitName = /^(?:Back(?: to .+)?|Return(?: to .+)?|Close|Done|Exit)$/i;
  for (const role of ['button', 'link']) {
    const control = await firstVisibleFromLocator(page.getByRole(role, { name: exitName }), { limit: 12, timeout: 150 });
...
async function recoverSiblingStatePath(page, previousPath = [], nextPath = [], route = 'today') {
  ...
  // Walk back only as far as the common nested ancestor. Most sibling surfaces
  // expose Back/Close while retaining the same top-level route and live data.
  const maxExitAttempts = Math.max(2, previous.length - commonDepth + 1);
  for (let attempt = 0; attempt < maxExitAttempts; attempt += 1) {
    const exit = await visibleAncestorExitControl(page);
    if (!exit) break;
    await exit.click({ timeout: 1800 }).catch(async () => exit.evaluate(node => node.click()).catch(() => {}));
    await settleReactFrames(page);
    await dismissBlockingDialogs(page, { maxPasses: 2 }).catch(() => null);
    if (siblingLabel && await findStateControl(page, siblingLabel)) return siblingPath;
    if (await findStateControl(page, next[0])) return next;
  }

  // A top-level remount is the bounded last resort for a child surface with no
  // recoverable ancestor control; normal sibling traversal never pays this cost.
  await resetRouteRoot(page, route);
  return next;
}
...
function locatorFromFormDescriptor(page, row = {}) {
  ...
  return page.locator(formControlSelectorFor(row)).nth(Number(row.selectorOrdinal || 0));
}

async function uniqueVisibleFormControl(locator, { limit = 20 } = {}) {
  const count = Math.min(await locator.count().catch(() => 0), limit);
  let match = null;
...
async function ensureExactFormProbeTarget(page, row = {}) {
  const exactToken = locatorFromFormDescriptor(page, row);
  if (await exactToken.isVisible({ timeout: 120 }).catch(() => false)) return true;

  const semanticCandidates = [];
  if (row.testId) semanticCandidates.push(page.getByTestId(row.testId));
  if (row.id) semanticCandidates.push(page.locator(`#${String(row.id).replace(/(["\\#.:[\],=])/g, '\\$1')}`));
  if (row.label) {
    const escaped = String(row.label).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    semanticCandidates.push(page.getByLabel(new RegExp(`^${escaped}$`, 'i')));
  }
  for (const semantic of semanticCandidates) {
    const exact = await uniqueVisibleFormControl(semantic);
    if (!exact) continue;
    return exact.evaluate((node, token) => {
      node.setAttribute('data-chaos-probe-token', token);
      return true;
    }, row.probeToken).catch(() => false);
  }

  const structuralRow = { ...row, probeToken: '', testId: '', id: '', label: '' };
  const fallback = page.locator(formControlSelectorFor(structuralRow)).nth(Number(row.selectorOrdinal || 0));
  if (!await fallback.isVisible({ timeout: 150 }).catch(() => false)) return false;
  return fallback.evaluate((node, token) => {
    node.setAttribute('data-chaos-probe-token', token);
    return true;
  }, row.probeToken).catch(() => false);
}
...
async function probeFormControls(page, controls, evidence, { allowValueMutation = true } = {}) {
  ...
  for (const row of descriptors.slice(0, 80)) {
    ...
    const type = row.type || '';
    await ensureExactFormProbeTarget(page, row);
    ...
    try {
      ...
      if (before !== null) {
        await ensureExactFormProbeTarget(page, row);
        restoreResult = await restoreAndObserve(page, locatorFromFormDescriptor(page, row), before, 1200);
      }
