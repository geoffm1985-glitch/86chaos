...
// This PWA check is intentionally request-only. The full release gate already
// exercises real pages in Chromium and mobile Chromium; this metadata matrix must
// not fail the release solely because a late-run optional browser binary exits
// before Playwright can create a page.
test.use({ video: 'off' });
