...
test.describe('86 Chaos Env Diagnostics', () => {
  test('Playwright can see required .env values', async ({}, testInfo) => {
    ...
    await testInfo.attach('env-diagnostics.json', {
      body: JSON.stringify({ BASE_URL, EXPECTED_VERSION, ownerSeen: !!(owner.email && owner.password), staffSeen: !!(staff?.email && staff?.password), managerSeen: hasCreds('MANAGER'), systemAdminSeen: hasCreds('SYSTEM_ADMIN'), debug }, null, 2),
      contentType: 'application/json',
    });

    console.log('86 CHAOS ENV DIAGNOSTICS:', JSON.stringify({ BASE_URL, EXPECTED_VERSION, ownerEmail: owner.email || null, ownerPasswordSeen: !!owner.password, staffEmail: staff?.email || null, staffPasswordSeen: !!staff?.password, managerSeen: hasCreds('MANAGER'), systemAdminSeen: hasCreds('SYSTEM_ADMIN'), debug }, null, 2));

    expect(BASE_URL, 'APP_URL or CHAOS_BASE_URL should be set').toMatch(/^https?:\/\//);
    expect(EXPECTED_VERSION, 'CHAOS_EXPECTED_VERSION should default to 15.1.10 for this rebuild').toBeTruthy();
