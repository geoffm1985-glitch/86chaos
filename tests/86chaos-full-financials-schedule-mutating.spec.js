...
async function readSelectOptions(combo) {
  return await combo.locator('option').evaluateAll((nodes) =>
    nodes.map((node) => ({
      value: node.value,
...
function isTimeLikeScheduleOption(label) {
  return /\b\d{1,2}\s*a\s*-\s*\d{1,2}\s*p\b|\b\d{1,2}:\d{2}\b|custom|preset|shift/i.test(String(label || ''));
}

function chooseStaffOption(options) {
  const wanted = String(process.env.TEST_SCHEDULE_STAFF_NAME || '').trim();
  const realOptions = options.filter((opt) => {
    const label = String(opt.label || '').trim();
    if (!label || opt.disabled) return false;
    if (/select|choose|staff|employee/i.test(label) && label.length < 16) return false;
    if (isTimeLikeScheduleOption(label)) return false;
    return true;
  });

  if (wanted) {
    const exact = realOptions.find((opt) => opt.label.toLowerCase() === wanted.toLowerCase());
    if (exact) return exact;
    const partial = realOptions.find((opt) => opt.label.toLowerCase().includes(wanted.toLowerCase()));
...
async function pickFirstStaff(page, report) {
  ...
  // First pass: native selects or select-backed comboboxes inside the schedule builder.
  const combos = main.getByRole('combobox');
  const comboCount = await combos.count().catch(() => 0);
  for (let i = 0; i < comboCount; i++) {
    const combo = combos.nth(i);
    const options = await readSelectOptions(combo);
    if (!options.length) continue;
    const labels = options.map((opt) => opt.label).join(' | ');
    const looksLikeStaff = /select staff|staff|employee|geoff|owner|manager|admin/i.test(labels) && !options.every((opt) => isTimeLikeScheduleOption(opt.label));
    if (!looksLikeStaff) continue;

    const choice = chooseStaffOption(options);
    if (!choice) continue;

    const selected = await combo.selectOption(choice.value ? { value: choice.value } : { label: choice.label }).then(() => true).catch(async () => {
      return await combo.selectOption({ label: choice.label }).then(() => true).catch(() => false);
    });

    if (selected) {
      report.actions.push({ action: 'selected staff', method: 'combobox/selectOption', staff: choice.label, value: choice.value, availableStaff: options.map((opt) => opt.label) });
      await page.waitForTimeout(900);
      return choice.label;
    }
  }

  // Second pass: any visible native select, in case the role-based locator misses it.
  ...
  for (let i = 0; i < selectCount; i++) {
    ...
    const selected = await select.selectOption(choice.value ? { value: choice.value } : { label: choice.label }).then(() => true).catch(async () => {
      return await select.selectOption({ label: choice.label }).then(() => true).catch(() => false);
    });
    if (selected) {
      report.actions.push({ action: 'selected staff', method: 'native-select', staff: choice.label, value: choice.value, availableStaff: options.map((opt) => opt.label) });
      await page.waitForTimeout(900);
      return choice.label;
    }
  }

  // Third pass: custom combobox fallback. Click the staff-looking combobox, then pick a visible option.
  for (let i = 0; i < comboCount; i++) {
    const combo = combos.nth(i);
    const visible = await combo.isVisible({ timeout: 500 }).catch(() => false);
    if (!visible) continue;
    const text = (await combo.innerText().catch(() => '')).trim();
    if (isTimeLikeScheduleOption(text)) continue;

    await combo.click({ timeout: 2500 }).catch(() => {});
    await page.waitForTimeout(300);
    const option = page.getByRole('option').filter({ hasText: /geoff|owner|manager|admin|^[A-Za-z][A-Za-z0-9 ._-]{2,}$/i }).first();
    ...
    report.actions.push({ action: 'selected staff', method: 'custom-combobox', staff: label });
    await page.waitForTimeout(900);
    return label;
  }

  const scheduleText = await main.innerText().catch(() => '');
  report.actions.push({ action: 'selected staff failed', comboboxCount: comboCount, selectCount, visibleTextStart: scheduleText.slice(0, 800) });
  return null;
}
...
async function setScheduleTimes(page, report) {
  await closeAnyModal(page);
  const main = page.locator('main');
  const allInputs = main.locator('input, textarea, [contenteditable="true"]');
  ...
  for (let i = 0; i < inputCount; i++) {
    ...
    const value = await input.inputValue().catch(async () => await input.innerText().catch(() => ''));
    const name = await input.getAttribute('aria-label').catch(() => '') || await input.getAttribute('placeholder').catch(() => '') || '';
    if (type === 'time' || /^\d{1,2}:\d{2}$/.test(String(value || '').trim()) || /in|out|start|end|time/i.test(name)) {
      timeLike.push(input);
    }
  }

  if (timeLike.length >= 2) {
    await timeLike[0].fill(SCHEDULE_FIXTURE.startTime).catch(async () => {
      await timeLike[0].click();
      await page.keyboard.press(process.platform === 'darwin' ? 'Meta+A' : 'Control+A');
      await page.keyboard.type(SCHEDULE_FIXTURE.startTime);
    });
    await timeLike[1].fill(SCHEDULE_FIXTURE.endTime).catch(async () => {
      await timeLike[1].click();
      await page.keyboard.press(process.platform === 'darwin' ? 'Meta+A' : 'Control+A');
      await page.keyboard.type(SCHEDULE_FIXTURE.endTime);
    });
    report.actions.push({ action: 'set time inputs', method: 'visible-time-like-inputs', start: SCHEDULE_FIXTURE.startTime, end: SCHEDULE_FIXTURE.endTime });
    await page.waitForTimeout(500);
    return true;
  }

  // Last fallback: use accessible textboxes. The snapshot reports the time fields as textboxes, not always input[type=time].
  const boxes = main.getByRole('textbox');
  const boxCount = await boxes.count().catch(() => 0);
  const candidates = [];
  for (let i = 0; i < boxCount; i++) {
    const box = boxes.nth(i);
    const visible = await box.isVisible({ timeout: 300 }).catch(() => false);
    if (!visible) continue;
    const value = await box.inputValue().catch(async () => await box.innerText().catch(() => ''));
    if (/^\d{1,2}:\d{2}$/.test(String(value || '').trim())) candidates.push(box);
  }
  if (candidates.length >= 2) {
    await candidates[0].fill(SCHEDULE_FIXTURE.startTime);
    await candidates[1].fill(SCHEDULE_FIXTURE.endTime);
    report.actions.push({ action: 'set time inputs', method: 'role-textbox-time-values', start: SCHEDULE_FIXTURE.startTime, end: SCHEDULE_FIXTURE.endTime });
    await page.waitForTimeout(500);
    return true;
  }

  report.actions.push({ action: 'set time inputs failed', inputCount, textboxCount: boxCount, visibleTextStart: (await main.innerText().catch(() => '')).slice(0, 800) });
  return false;
}
...
async function clickScheduleCellLikeUser(page, cell) {
  ...
  await page.mouse.move(box.x + Math.min(Math.max(box.width / 2, 8), box.width - 4), box.y + Math.min(Math.max(box.height / 2, 8), box.height - 4));
...
async function selectScheduleCells(page, staffName, report) {
  ...
  const wanted = String(staffName || process.env.TEST_SCHEDULE_STAFF_NAME || '').trim();
  const candidates = [];

  for (let i = 0; i < Math.min(rowCount, 90); i++) {
    const row = rows.nth(i);
    const text = (await row.innerText().catch(() => '')).trim();
    if (!text || /Proj\. Cost|Month Total|Scheduled Hours Tracker|Employee\s+Wk|Staff\s+W/i.test(text)) continue;
    ...
    const score =
      ...
      /geoff|owner|manager|admin/i.test(firstCellText) ? 50 :
  ...
  for (const candidate of candidates.slice(0, 8)) {
    const cellsToTry = [];
    for (let i = 1; i < Math.min(candidate.cellCount, 24); i++) {
      const cell = candidate.cells.nth(i);
      const txt = (await cell.innerText().catch(() => '')).trim();
      cellsToTry.push({ index: i, cell, txt, empty: !txt, existingShift: /\d{1,2}\s*[ap]\s*-\s*\d{1,2}\s*[ap]|\d{1,2}:\d{2}/i.test(txt) });
    }
    cellsToTry.sort((a, b) => Number(b.empty) - Number(a.empty) || Number(a.existingShift) - Number(b.existingShift));

    for (const item of cellsToTry.slice(0, 12)) {
  ...
  report.actions.push({
    action: 'selected schedule cells failed',
    staffName,
    clicked,
    assignState: finalState,
    rowCount,
    candidateRows: candidates.slice(0, 8).map((c) => ({ firstCellText: c.firstCellText, score: c.score, textStart: c.text.slice(0, 160) })),
  });
  return clicked;
}
