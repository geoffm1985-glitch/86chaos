...
async function checkManagerBriefMath(page, problems, mathChecks) {
  ...
  const summaryText = (await summary.textContent().catch(() => '')) || (await page.locator('body').innerText());
...
test.describe('86 Chaos deep automated checks', () => {
  ...
  test('important app pages load without crashes', async ({ page }, testInfo) => {
    ...
    for (const pageInfo of pagesToTry) {
      await page.goto(`${APP_URL}/${pageInfo.tab}`, { waitUntil: 'domcontentloaded' });
      await page.waitForTimeout(1200);
      await expect(page.locator('main')).toBeVisible({ timeout: 30000 });
  ...
  test('legacy public tab links normalize to live app routes', async ({ page }, testInfo) => {
    ...
    for (const route of legacyRoutesToSmoke) {
      await page.goto(`${APP_URL}/${route.from}`, { waitUntil: 'domcontentloaded' });
      await page.waitForTimeout(750);
      await expect(page.locator('main')).toBeVisible({ timeout: 30000 });
      const url = page.url();
      const bodyText = await page.locator('body').innerText().catch(() => '');
  ...
  test('owner account should see admin-level controls', async ({ page }, testInfo) => {
    ...
    for (const text of ownerControlText) {
      const visible = await page.getByText(text).first().isVisible({ timeout: 2500 }).catch(() => false);
      if (visible) found.push(String(text));
    }
    ...
    expect(found.length, `Owner/admin controls found: ${JSON.stringify(found)}`).toBeGreaterThan(0);
    expect(problems, JSON.stringify(problems, null, 2)).toEqual([]);
  });
});
