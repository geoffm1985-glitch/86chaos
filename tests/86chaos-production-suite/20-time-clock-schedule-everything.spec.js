...
async function dismissBlockingModals(page) {
  // The employee quick-start tour can legitimately appear on fresh test accounts.
  // Close it before clicking schedule subtabs so Playwright does not click through an overlay.
  const closeQuickStart = page.getByRole('button', { name: /close employee quick start/i }).first();
  if (await closeQuickStart.isVisible({ timeout: 1200 }).catch(() => false)) {
    await closeQuickStart.click({ timeout: 5000 }).catch(() => {});
    await page.waitForTimeout(300);
  }

  const skipQuickStart = page.getByRole('button', { name: /skip for now/i }).first();
  if (await skipQuickStart.isVisible({ timeout: 800 }).catch(() => false)) {
    await skipQuickStart.click({ timeout: 5000 }).catch(() => {});
    await page.waitForTimeout(300);
  }

  // Do not fail the whole test just because a product modal took a moment to animate out.
  await expect(page.locator('.chaos-modal-backdrop')).toHaveCount(0, { timeout: 5000 }).catch(() => {});
}
...
async function inspectRequestOff(page, label, testInfo) {
  ...
  const duplicateBadges = page.locator('span').filter({ hasText: /^\d+\s+req$/i });
  const duplicateBadgeCount = await duplicateBadges.count().catch(() => 0);
  let duplicateDialogSeen = false;
  let clickableDuplicateBadgeFound = false;

  // Safe/read-only mode must not click arbitrary historical request-count days.
  // A visible "1 req" badge may be on a past, locked, published, or scroll-sensitive calendar cell.
  // The real duplicate warning is best covered by source validation and optional controlled mutation tests,
  // not by poking live restaurant calendar history during every read-only smoke run.
  if (duplicateBadgeCount > 0) {
    const firstBadgeText = await duplicateBadges.first().innerText({ timeout: 1000 }).catch(() => '');
    expect(firstBadgeText, `${label} duplicate request badge should render a readable count`).toMatch(/^\d+\s+req$/i);
  }
