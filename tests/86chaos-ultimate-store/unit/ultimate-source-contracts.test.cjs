...
test('version 16.0.63 is consistent across active release files', () => {
  const pkg = json('package.json');
  const version = json('public/version.json');
  const apiVersion = read('api/_version.js');
  const core = read('src/core/appCore.js');
  assert.equal(pkg.version, '16.0.63');
  assert.equal(version.version, '16.0.63');
  assert.match(apiVersion, /16\.0\.63/);
  assert.match(core, /16\.0\.63/);
  assert.ok(exists('scripts/validate-16-0-63.js'));
});
