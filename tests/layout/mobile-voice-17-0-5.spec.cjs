...
for(const [width,height] of [[360,740],[412,915],[540,720],[412,580],[1024,768]])test(`17.0.5 actual voice CSS layout ${width}x${height}`,async({page})=>{
 ...
 if(width<=720){expect(height-box.y-box.height).toBeCloseTo(44,0);const main=page.locator('main'),bounds=await main.boundingBox();expect(bounds.y+bounds.height).toBeLessThanOrEqual(box.y);await main.evaluate(el=>el.scrollTop=el.scrollHeight);const last=await page.locator('.shift-row').last().boundingBox();expect(last.y+last.height).toBeLessThanOrEqual(box.y);for(const name of ['Copy Month','Publish','Event']){await main.evaluate(el=>el.scrollTop=0);const control=await page.getByRole('button',{name,exact:true}).boundingBox();expect(control.y+control.height).toBeLessThan(box.y);}}
