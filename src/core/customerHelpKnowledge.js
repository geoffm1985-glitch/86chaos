...
const CUSTOMER_HELP_VERSION = '16.0.238';
...
const ARTICLES = [
  article("scanner-how-it-works", "How invoice scanning works", "inventory-purchasing", "invoice-scanning", "Manager / Owner", "OpenAI extracts invoice evidence; deterministic checks separate products, supplies, and document noise.", ["how does AI work", "scanner missed product"], ["how does AI work", "scanner missed product"], [{"title": "How it works", "body": ["Upload a clear invoice in Inventory → Invoices. Review the extracted vendor, received quantities, pack sizes, and prices. Stock Matcher suggests existing items; Needs Review holds uncertain rows. Freight, tax, totals, addresses, and terms are excluded from stock. Purchased gloves and other supplies remain supply items, outside food costing. Approval is the step that saves inventory and latest costs."]}], [], [], ["invoices"]),
  article("scanner-troubleshooting", "Scanner troubleshooting", "inventory-purchasing", "invoice-scanning", "Manager / Owner", "Check the original row before retrying a scan.", ["invoice did not match", "invoice didn't match", "scanner missed product"], ["invoice did not match", "invoice didn't match", "scanner missed product"], [{"title": "How it works", "body": ["If a product is missing, open Needs Review and inspect excluded or uncertain rows against the invoice. Use a clearer crop or split a long document if extraction failed. Check page allowances and file limits shown by the scanner. A low-confidence result needs review, not repeated attempts to obtain a different answer. An unavailable provider may use a configured fallback within the same call budget."]}], [], [], ["invoices"]),
  article("stock-matcher-explained", "How Stock Matcher decides", "inventory-purchasing", "invoice-review", "Manager / Owner", "Each recommendation explains the evidence behind its match.", ["why did scanner pick this", "ranch boom boom sauce", "wrong item selected"], ["why did scanner pick this", "ranch boom boom sauce", "wrong item selected"], [{"title": "How it works", "body": ["Exact approved vendor mappings and vendor SKU matches come first. Otherwise strong normalized product names or restaurant aliases may suggest a match. Similar names, missing units, substitutions, price jumps, or pack conflicts require review. Read the explanation beside the row; select the right inventory item if the suggestion is wrong. No match is automatically approved."]}], [], [], ["invoices"]),
  article("needs-review-explained", "What Needs Review means", "inventory-purchasing", "invoice-review", "Manager / Owner", "Needs Review means the available evidence does not safely establish a product, match, quantity, or cost.", ["why does this need review", "needs review", "unknown catch weight"], ["why does this need review", "needs review", "unknown catch weight"], [{"title": "How it works", "body": ["Compare the row with the original invoice. Confirm whether it is a purchased product or document noise. Choose the inventory item and enter actual received stock quantity and cost per inventory unit when prompted. Add a short reason. Catch-weight rows require actual delivered weight and the invoice price basis; a nominal case weight is insufficient."]}], [], [], ["invoices"]),
  article("review-first-contract", "Why 86 Chaos is review-first", "inventory-purchasing", "invoice-review", "Manager / Owner", "Scans and forecasts can extract information and suggest next steps. A suggestion is not your approval.", ["why review first", "automatic order", "AI approval"], ["why review first", "automatic order", "AI approval"], [{"title": "How it works", "body": ["Scans cannot change inventory, costs, menu links, or par levels until an authorized person approves. Order Suggestions prepares quantities for review; it does not send vendor orders or change par levels. Managers remain in control of every business change."]}], [], [], ["invoices"]),
  article("learned-vendor-mappings", "How learned vendor mappings work", "inventory-purchasing", "items-vendors", "Manager / Owner", "A manager-approved invoice match becomes workspace-specific vendor memory.", ["vendor SKU alias", "CK522 CHIX WNG", "wrong learned mapping"], ["vendor SKU alias", "CK522 CHIX WNG", "wrong learned mapping"], [{"title": "How it works", "body": ["After approval, vendor shorthand such as CK522 CHIX WNG can suggest Chicken Wings on the next invoice from that vendor. Inventory → Vendors contains learned mappings: select the vendor to inspect, correct, or revoke one. A revoked mapping is not silently reactivated by another invoice. Changed packs, units, substitutions, or unusual prices still require review. Mappings are not shared across restaurants."]}], [], [], ["vendors"]),
  article("unit-mismatch-costing", "Why unit mismatch can break food-cost accuracy", "inventory-purchasing", "invoice-review", "Manager / Owner", "Purchase units, inventory units, and recipe portions must describe the same quantity.", ["food cost wrong", "wrong case size", "unit mismatch"], ["food cost wrong", "wrong case size", "unit mismatch"], [{"title": "How it works", "body": ["A case price is not a pound price. Weight ounces are not fluid ounces; the app will not guess a density conversion. A 40 lb case at $80 costs $2 per lb before yield loss. Check inventory unit, pack size, delivered quantity, and price basis before approving. Unclear conversions remain in review."]}], [], [], ["invoices"]),
  article("case-pack-math", "How case-pack math works", "inventory-purchasing", "invoice-review", "Manager / Owner", "Case-pack parsing uses the printed contents of each case.", ["4/10 LB", "2/5 LB", "12/8 OZ", "wrong case size"], ["4/10 LB", "2/5 LB", "12/8 OZ", "wrong case size"], [{"title": "How it works", "body": ["4/10 LB means four 10 lb packs, or 40 lb per case. Two cases of 2/5 LB contain 20 lb total. 12/8 OZ contains 96 weight ounces; 24 CT means 24 pieces. Partial cases use the actual received fraction when the package is known. Ordered quantities and backorders are not received stock. Malformed packs and catch weight need a human decision."]}], [], [], ["invoices"]),
  article("menu-cost-propagation", "Why menu cost updates only after invoice approval", "inventory-purchasing", "menu-intelligence", "Manager / Owner", "Approved invoice cost becomes the inventory item’s latest cost, which feeds approved menu links.", ["why is not menu cost updating", "why isn't menu cost updating", "invoice cost menu cost"], ["why is not menu cost updating", "why isn't menu cost updating", "invoice cost menu cost"], [{"title": "How it works", "body": ["Finish invoice review and approve it first. Then check that Menu Intelligence links the ingredient to the correct inventory item, with a valid portion and unit. Unapproved scan previews do not alter saved costing. Missing prices, incompatible units, and missing links stay visibly incomplete. Review outdated menu scans through the existing scan history edit action."]}], [], [], ["menu-intelligence"]),
  article("batch-yield-costing", "How batch and portion costing work", "inventory-purchasing", "menu-intelligence", "Manager / Owner", "Approve a recipe’s batch ingredients and usable yield, then link menu portions to that batch.", ["house ranch", "sauce batch", "recipe yield", "menu food cost"], ["house ranch", "sauce batch", "recipe yield", "menu food cost"], [{"title": "How it works", "body": ["Open a recipe’s spec sheet and choose Review batch ingredients & yield. Enter one full batch, its finished quantity and unit, and the usable yield percentage. Approve that recipe costing. In Menu Intelligence choose the approved batch and review the portion quantity. Batch cost divided by usable yield gives unit cost; the menu portion multiplies that unit cost. Estimated portions remain labeled until reviewed."]}], [], [], ["menu-intelligence"]),
  article("outside-product-research", "How outside product research is used", "inventory-purchasing", "invoice-review", "Manager / Owner", "Research gathers supporting evidence for a manager; it never changes stock or costs.", ["research product", "public food lookup", "unknown vendor product"], ["research product", "public food lookup", "unknown vendor product"], [{"title": "How it works", "body": ["Use research from a row needing review. The app checks approved workspace/vendor memory and bounded invoice history before a public product lookup. Public results may suggest a brand or package and include source links, but do not prove a restaurant match. Do not put customer addresses or invoice details into a product search. Weak evidence stays in Needs Review."]}], [], [], ["invoices"]),
  article("customer-ai-policy", "Customer AI policy in plain language", "inventory-purchasing", "invoice-scanning", "Manager / Owner", "86 Chaos selects approved AI models on the server and limits every request.", ["how does AI work", "what AI model", "customer model policy"], ["how does AI work", "what AI model", "customer model policy"], [{"title": "How it works", "body": ["OpenAI is the primary invoice and menu extraction provider. Customer OpenAI workflows use explicitly approved models up to standard GPT-5.5; premium or unknown models cannot be selected by a restaurant account. A controlled Gemini fallback may help during an outage. File, page, output, request, and usage limits still apply. AI never gains authority to approve business changes."]}], [], [], ["invoices"]),
  article("food-safety-logs", "How to use food-safety logs", "recipes-prep-tasks", "line-checks", "Everyone", "Log temperatures and checks quickly in Prep → Line Check.", ["food safety log", "cooler temperature", "corrective action", "missed check"], ["food safety log", "cooler temperature", "corrective action", "missed check"], [{"title": "How to do it", "body": ["Choose the equipment, food, or location and enter the temperature. For a daily checklist choose Pass or Attention. Record corrective action for an out-of-range result; a manager can sign off with a note. Managers can configure ranges and check intervals with Edit Line Check. Missing required checks appear in the existing Line Check screen. Cooling needs start time and checkpoint notes. These records support your kitchen process and do not guarantee legal compliance."]}], [], [], ["prep"]),
  article("printer-setup", "Label printer setup and calibration", "recipes-prep-tasks", "labels", "Everyone", "Browser printing is the supported fallback for every label printer.", ["Brother QL-810W", "label size", "printer calibration"], ["Brother QL-810W", "label size", "printer calibration"], [{"title": "How to do it", "body": ["Install the manufacturer’s driver and confirm that the printer works from your operating system first. Match the loaded media to the label preset, choose 100% scale, and disable browser headers and footers. Print a single test label before a batch. For a Brother QL-810W-style workflow, use the driver print dialog; no direct hardware connection is assumed. Select a preset and calibration before printing, and retain the default 3.5 × 1.1 inch layout when it matches your media."]}], [], [], ["prep"]),
...
const SYNONYMS = {
  matcher: ['scanner pick','wrong match','didnt match','did not match'], pack: ['case size','unit mismatch','case pack'], costing: ['food cost wrong','menu cost updating','yield'], review: ['needs review','approval','uncertain'],
...
const CUSTOMER_HELP_ARTICLES_LEGACY = CUSTOMER_HELP_ARTICLES.map(serializeArticleForOldHelp);

export { CUSTOMER_HELP_VERSION, AUDIENCE, HELP_SUBJECTS, HELP_SUBTOPICS, HELP_DEEP_LINKS, CUSTOMER_FEATURES, CUSTOMER_HELP_ARTICLES, CUSTOMER_HELP_ARTICLES_LEGACY, INTERNAL_CONTENT_PATTERNS, FORBIDDEN_CUSTOMER_ARTICLE_IDS, tokenize, searchCustomerHelp, makeDeterministicHelpAnswer, buildCustomerHelpCoverage, validateCustomerHelpCorpus, validateDeepLinks, articleSearchText };

export default { CUSTOMER_HELP_VERSION, AUDIENCE, HELP_SUBJECTS, HELP_SUBTOPICS, HELP_DEEP_LINKS, CUSTOMER_FEATURES, CUSTOMER_HELP_ARTICLES, CUSTOMER_HELP_ARTICLES_LEGACY, INTERNAL_CONTENT_PATTERNS, FORBIDDEN_CUSTOMER_ARTICLE_IDS, tokenize, searchCustomerHelp, makeDeterministicHelpAnswer, buildCustomerHelpCoverage, validateCustomerHelpCorpus, validateDeepLinks, articleSearchText };
