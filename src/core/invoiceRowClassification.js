...
export const inferInvoiceProductFields = (row = {}) => {
  ...
  const inferred = {
    ...original,
    productCode,
    quantity: hasValue(original.receivedQty ?? original.shippedQty ?? original.quantity ?? original.qty) ? (original.receivedQty ?? original.shippedQty ?? original.quantity ?? original.qty) : (leading?.[1] || ''),
