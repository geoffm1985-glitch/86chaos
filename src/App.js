...
import { Trash2, Edit3, Plus, Search, FileText, CheckCircle, XCircle } from 'lucide-react';
// Assuming T (theme) and db are passed in or accessible in your scope

export default function TabInventory({ appUser, db, addToast, inventoryItems, vendors, invoices, T }) {
  const [subTab, setSubTab] = useState('count');
  ...
  const handleAddItem = async (e) => {
    e.preventDefault();
    if (!newItemName || !newCategory || !newVendor) return addToast('Error', 'Name, Category, and Vendor are required.');
    try {
      await addDoc(collection(db, "inventoryItems"), {
        productCode: newItemCode || '',
        name: newItemName,
        category: newCategory,
        vendor: newVendor,
        casePrice: parseFloat(newPrice) || 0,
        yield: parseFloat(newYield) || 1,
        unitPrice: (parseFloat(newPrice) || 0) / (parseFloat(newYield) || 1),
        restaurantId: appUser.restaurantId,
        inStock: 0,
        par: 0
      });
      addToast('Success', 'Item added to master list.');
      setNewItemCode(''); setNewItemName(''); setNewPrice(''); setNewYield('1');
    } catch (err) { addToast('Error', err.message); }
  };
  ...
  const handleDeleteItem = async (id) => {
    ...
    addToast('Deleted', 'Item removed from system.');
  };

  // --- GEMINI INVOICE/CSV AI PARSER ---
  const handleAIImport = async (event) => {
    const file = event.target.files[0];
    if (!file) return;
    addToast('Analyzing...', 'Gemini is reading the invoice...');
    
    // This is where you call your Gemini API function. 
    // Notice the updated prompt explicitly hunts for the Product Code.
    const systemPrompt = `
      You are an expert culinary data extractor. Read this supplier file and return ONLY a valid JSON array of objects.
      Every object MUST have these exact keys:
      - "productCode" (String: Extract the SKU, Item Number, or Product ID. Leave blank if none exists)
      - "name" (String: The name of the product)
      - "category" (String: Guess the best category like Meat, Produce, Dry Goods)
      - "vendor" (String: The name of the supplier)
      - "casePrice" (Number: Total price for the case)
      - "yield" (Number: How many units are in the case)
    `;
    
    // Simulate AI Processing (Replace with your actual Gemini fetch call)
    console.log("Prompt sent to Gemini:", systemPrompt);
    addToast('Notice', 'Hook up your Gemini fetch logic here using the updated prompt above.');
    event.target.value = ''; 
  };

  // --- DISPATCHED ORDER LOGIC ---
  const handleCancelOrder = async (orderId) => {
    if (!window.confirm("Are you sure you want to cancel and delete this order?")) return;
    try {
      await deleteDoc(doc(db, "invoices", orderId));
      addToast('Canceled', 'Order has been removed from dispatch.');
    } catch (err) { addToast('Error', err.message); }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4 pb-24 animate-[fadeIn_0.3s_ease-out]">
      
      {/* HEADER & SUB-TABS (No Side Scrolling) */}
      <div className="flex flex-wrap gap-2 mb-4 bg-[#12161A] p-2 rounded-xl border border-[#2A353D]">
        <button onClick={() => setSubTab('count')} className={`px-4 py-2 text-xs font-bold rounded-lg transition-colors flex-1 min-w-[100px] ${subTab === 'count' ? 'bg-[#D4A381] text-black' : 'text-slate-400 hover:bg-[#1A2126]'}`}>Count</button>
        <button onClick={() => setSubTab('dispatched')} className={`px-4 py-2 text-xs font-bold rounded-lg transition-colors flex-1 min-w-[100px] ${subTab === 'dispatched' ? 'bg-[#D4A381] text-black' : 'text-slate-400 hover:bg-[#1A2126]'}`}>Dispatched</button>
        <button onClick={() => setSubTab('manage')} className={`px-4 py-2 text-xs font-bold rounded-lg transition-colors flex-1 min-w-[100px] ${subTab === 'manage' ? 'bg-[#D4A381] text-black' : 'text-slate-400 hover:bg-[#1A2126]'}`}>Manage</button>
      </div>

      {/* SUB-TAB: COUNT */}
      {subTab === 'count' && (
        <div className={`${T.card} p-5`}>
          <h2 className={`text-lg font-black ${T.copper} mb-4`}>Current Inventory Count</h2>
          <p className="text-xs text-slate-400">Inventory count interface goes here...</p>
        </div>
      )}
      ...
      {subTab === 'dispatched' && (
        <div className="space-y-4">
          {invoices?.length === 0 ? (
            <div className={`${T.card} p-5 text-center text-slate-500 text-xs font-bold`}>No pending orders.</div>
          ) : (
            invoices?.map(inv => (
              <div key={inv.id} className={`${T.card} p-4 flex flex-col sm:flex-row justify-between items-center gap-4`}>
                <div>
                  <h3 className="font-bold text-white flex items-center gap-2"><FileText size={16} className="text-[#D4A381]"/> {inv.vendor || 'Unknown Vendor'} Order</h3>
                  <p className="text-xs text-slate-400">Total: ${parseFloat(inv.total || 0).toFixed(2)} | Items: {inv.items?.length || 0}</p>
                </div>
                <div className="flex gap-2 w-full sm:w-auto">
                  <button className="flex-1 sm:flex-none bg-emerald-900/30 text-emerald-400 border border-emerald-900/50 px-4 py-2 rounded-lg text-xs font-bold hover:bg-emerald-900/50 flex items-center justify-center gap-2"><CheckCircle size={14}/> Receive</button>
                  <button onClick={() => handleCancelOrder(inv.id)} className="flex-1 sm:flex-none bg-red-900/20 text-red-400 border border-red-900/50 px-4 py-2 rounded-lg text-xs font-bold hover:bg-red-900/40 flex items-center justify-center gap-2"><XCircle size={14}/> Cancel</button>
                </div>
              </div>
            ))
          )}
      ...
      {subTab === 'manage' && (
        <div className="space-y-4">
          ...
          {/* AI IMPORT BUTTON */}
          <label className="w-full bg-[#12161A] text-[#D4A381] border border-[#2A353D] font-black text-xs uppercase tracking-widest py-3 rounded-xl hover:bg-[#1A2126] transition-colors flex items-center justify-center gap-2 cursor-pointer shadow-sm">
            <span>📊 IMPORT CSV / INVOICE VIA GEMINI</span>
            <input type="file" accept=".csv, .pdf, image/*" onChange={handleAIImport} className="hidden" />
          </label>

          {/* COMPACT ADD ITEM BOX */}
          <form onSubmit={handleAddItem} className="bg-[#12161A] p-4 border border-[#2A353D] rounded-xl shadow-sm">
            <h3 className="text-[#D4A381] font-black uppercase tracking-widest mb-3 text-[10px]">Add New Item</h3>
            <div className="grid grid-cols-2 sm:grid-cols-6 gap-3">
              ...
              <input type="text" placeholder="Category..." value={newCategory} onChange={e=>setNewCategory(e.target.value)} className={`${T.input} col-span-1 py-1.5 text-xs`} required />
              <input type="text" placeholder="Vendor..." value={newVendor} onChange={e=>setNewVendor(e.target.value)} className={`${T.input} col-span-1 py-1.5 text-xs`} required />
              <input type="number" step="0.01" placeholder="Case $" value={newPrice} onChange={e=>setNewPrice(e.target.value)} className={`${T.input} col-span-1 py-1.5 text-xs`} />
              <input type="number" step="0.01" placeholder="Yield" value={newYield} onChange={e=>setNewYield(e.target.value)} className={`${T.input} col-span-1 py-1.5 text-xs`} />
            </div>
            <div className="mt-3 flex justify-end">
              <button type="submit" className="bg-[#D4A381] text-black font-black text-xs uppercase tracking-widest px-6 py-2 rounded-lg hover:bg-[#b08565] transition-colors flex items-center gap-2"><Plus size={14}/> Add to Master List</button>
            </div>
          </form>
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Search inventory by name or product number..." value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} className="w-full bg-[#1A2126] border border-[#2A353D] rounded-xl pl-9 pr-4 py-2.5 text-xs text-white focus:border-[#D4A381] outline-none transition-colors" />
          </div>

          {/* MASTER LIST RENDER */}
          <div className="space-y-2">
            {filteredItems.map(item => (
              <div key={item.id} className="bg-[#12161A] p-3 border border-[#2A353D] rounded-lg flex justify-between items-center hover:border-slate-600 transition-colors">
                <div>
                  <h4 className="font-bold text-slate-200 text-sm flex items-center gap-2">
                    {item.name} 
                    {item.productCode && <span className="text-[9px] bg-slate-800 text-slate-400 px-1.5 py-0.5 rounded font-mono border border-slate-700">#{item.productCode}</span>}
                  </h4>
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">CASE: ${parseFloat(item.casePrice||0).toFixed(2)} | YIELD: {item.yield}</p>
                </div>
                <div className="flex gap-2">
                  <button className="text-slate-400 hover:text-[#D4A381] p-1 transition-colors"><Edit3 size={16} /></button>
                  <button onClick={() => handleDeleteItem(item.id)} className="text-slate-400 hover:text-red-500 p-1 transition-colors"><Trash2 size={16} /></button>
                </div>
              </div>
            ))}
            {filteredItems.length === 0 && <p className="text-center text-slate-500 text-xs py-4">No items found.</p>}
          </div>
