...
{subTab === 'time-off' && <div className="animate-[slideIn_0.2s_ease-out]"><TabTimeOff timeOffRequests={timeOffRequests} appUser={appUser} users={users} addToast={addToast} events={events} shifts={shifts} /></div>}    </div>
  );
};
                  ...
                  {groupedUsers[role].sort((a,b) => (a.name||'').localeCompare(b.name||'')).map(u => (
                    <tr key={u.id} className={selectedEmp===u.id?'bg-[#12161A]/50':''}>
                      ...
                      {scheduleDays.map(d => {
                        ...
                        // Conflict Check: Alert if a shift overlaps with ANY time-off request
...
// --- TIME OFF REQUESTS (Auto-Approve & Published Lockdown) ---
const TabTimeOff = ({ timeOffRequests, appUser, users, addToast, events = [], shifts = [] }) => {
  ...
  const handleToggleDate = (d) => { 
    ...
    // HARD LOCK: Check if there is ANY published shift on this date
    const isPublished = shifts.some(s => s.date === d && s.isPublished);
    if (isPublished) return addToast('Blocked', 'The schedule for this date is already published and locked.');
  ...
  const handleSubmit = async (e) => { 
    ...
    for (const d of selectedDates) { 
      await addDoc(collection(db, "timeOffRequests"), { 
        ...
        status: 'approved' // Automatically approved because blocked days cannot be selected
      }); 
    } 
      ...
      {appUser?.isAdmin && (
        <div className={`${T.card} overflow-hidden mb-6`}>
          <div className={`bg-[#12161A] p-3 border-b ${T.border} flex justify-between items-center`}>
            ...
            <span className={`text-[10px] font-bold ${T.muted}`}>View or delete logged requests.</span>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className={`md:col-span-2 ${T.card} overflow-hidden`}>
          ...
          <div className={`grid grid-cols-7 border-t ${T.border}`}>
            ...
            {monthDays.map(d => {
              ...
              const isPublished = shifts.some(s => s.date === d && s.isPublished); // Check if locked
              const holiday = getHoliday(d);
              const dayEvents = monthEvents.filter(e => e.date === d);

              return (
                <div key={d} onClick={() => !isPast && !isPublished && handleToggleDate(d)} className={`p-1 border-b border-r ${T.border} min-h-[50px] flex flex-col items-center justify-start pt-1 transition-colors ${isPast || isPublished ? 'bg-[#12161A]/50 opacity-50 cursor-not-allowed' : existingReq ? 'bg-red-900/10 cursor-pointer hover:bg-red-900/20 border border-red-900/30 shadow-inner' : isSelected ? 'bg-[#8F6040]/20 border border-[#C59373] cursor-pointer shadow-inner' : 'hover:bg-[#12161A] cursor-pointer'}`}>
                  ...
                  {dayEvents.map(ev => (
                    <span key={ev.id} className="text-[6px] sm:text-[7px] text-blue-400 font-bold uppercase text-center leading-tight mt-0.5 px-0.5 w-full truncate" title={ev.title}>{ev.title}</span>
                  ))}

                  {isPublished && !existingReq && <Shield size={10} className="mt-auto mb-1 text-slate-500 opacity-60" />}
                  {existingReq && <span className="text-[7px] font-black uppercase mt-auto mb-1 text-red-500">Off</span>}
        ...
        <div className="md:col-span-1">
          <div className={`${T.card} p-4 sm:p-5 sticky top-20`}>
            ...
            <p className={`text-[10px] font-bold ${T.muted} mb-4 leading-tight`}>Tap days on the calendar to mark yourself unavailable. Padlocked days have already been published.</p>
            ...
            <div className="mt-6 pt-6 border-t border-[#2A353D]">
              <h3 className="font-black text-sm text-white mb-3">My Scheduled Days Off</h3>
              <div className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar">
                {myFutureRequests.length === 0 && <div className="text-xs font-bold text-slate-500 text-center py-2 border border-dashed border-[#2A353D] rounded-xl">No upcoming time off.</div>}
                {myFutureRequests.map(r => (
                  <div key={r.id} className="flex items-center justify-between bg-[#12161A] p-2.5 rounded-xl border border-[#2A353D] hover:border-[#D4A381]/50 transition-colors">
                    <div>
                      <div className="text-xs font-bold text-white flex items-center gap-2">{formatDisplayDate(r.date)}</div>
                      {r.isPartial ? (
