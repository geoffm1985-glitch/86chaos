  ...
  return (
    <div className={`min-h-screen font-sans flex flex-col ${isDark ? 'bg-slate-900 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full p-4 sm:p-6 pb-24">
        {activeTabState === 'schedule' && appUser?.isAdmin && <TabSchedule currentDate={currentDate} appUser={liveAppUser} users={users} shifts={shifts} addToast={addToast} />}
  ...
  const handleAuth = async (e) => {
    ...
    try {
      if (isFirstUser) {
        ...
        const newUser = { name: name.trim(), email: MASTER_ADMIN_EMAIL, phone: '', password, role: 'Kitchen', isAdmin: true, isActive: true, forcePasswordChange: false };
...
const TabTeam = ({ appUser, users, addToast }) => {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      await addDoc(collection(db, "users"), { name: name.trim(), email: email.toLowerCase().trim(), phone: phone.trim(), password, role, isAdmin, isActive: true, forcePasswordChange: true });
  ...
  const generateTestAccount = async () => {
    ...
    await addDoc(collection(db, "users"), { name: `Test ${isBartender ? 'Bar' : 'Kit'} ${num}`, email: `test${num}@cheers.local`, phone: '', password: 'password', role: isBartender ? 'Bartender' : 'Kitchen', isAdmin: false, isActive: true, forcePasswordChange: false });
  ...
  const canGenerateTest = appUser?.email === MASTER_ADMIN_EMAIL || appUser?.id === 'dev-backdoor';
...
// --- Tab: Schedule Maker ---
const TabSchedule = ({ currentDate, appUser, users, shifts, addToast }) => {
    ...
    <div className="space-y-6 max-w-[1400px] mx-auto pb-12">
      <div className="bg-white dark:bg-slate-800 p-6 border border-slate-200 dark:border-slate-700 rounded-3xl shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div><h3 className="font-black text-2xl flex items-center gap-2 text-slate-800 dark:text-white"><Calendar className="text-blue-500" size={24}/> Schedule Maker</h3></div>
              ...
              {displayUsers.map(u => {
                ...
                return (
                  <tr key={u.id} className={`transition-colors ${isSelectedEmp ? 'bg-blue-50 dark:bg-blue-900/10' : 'hover:bg-slate-100 dark:hover:bg-slate-800/50'}`}>
                    ...
                    {monthDaysArray.map(d => {
                      const shift = monthShifts.find(s => s.date === d && s.employeeId === u.id); const isDateSelected = assignDates.includes(d); const shiftBgColor = shift ? (shift.isPublished ? (isBartender ? 'bg-blue-600' : 'bg-orange-600') : 'bg-slate-500') : ''; const shiftOutline = shift && selectedShiftId === shift.id ? 'ring-2 ring-red-500 ring-offset-1' : '';
                      ...
                      return (
                        <td key={d} onClick={handleCellClick} className={`p-1 border-l border-slate-200 dark:border-slate-700 cursor-pointer transition-all relative ${isSelectedEmp && isDateSelected ? 'bg-blue-300 dark:bg-blue-600 outline outline-2 outline-blue-600 dark:outline-blue-400 shadow-[inset_0_0_15px_rgba(0,0,0,0.1)] z-10 scale-[1.02]' : isSelectedEmp ? 'hover:bg-blue-200 dark:hover:bg-blue-800' : 'hover:bg-slate-200 dark:hover:bg-slate-800'}`}>
                          {shift ? (<div className={`w-full rounded font-bold text-[10px] py-1 text-center text-white shadow-sm ${shiftBgColor} ${shiftOutline}`} title={`${formatTime12Hour(shift.startTime)} - ${formatTime12Hour(shift.endTime)}`}>{formatShortTime(shift.startTime)}-{formatShortTime(shift.endTime)}</div>) : (<div className="w-full h-6 rounded flex items-center justify-center text-slate-400 dark:text-slate-600 font-bold opacity-0 hover:opacity-100 transition-opacity"><Plus size={14}/></div>)}
                        </td>
                      )
                    })}
                  </tr>
...
const TabSettings = ({ addToast, appUser }) => {
  ...
  const handleEnablePush = async () => {
    ...
    try {
      ...
      console.error(err);
      addToast('Error', err.code || 'Push registration failed.'); 
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 pb-12">
      <div className="bg-white dark:bg-slate-800 border rounded-3xl p-8 shadow-sm space-y-8">
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 p-6 rounded-2xl"><h4 className="font-black text-blue-900 dark:text-blue-400 mb-2">Push Notifications</h4><p className="text-sm text-blue-800 dark:text-blue-300 mb-5 font-medium">Link this specific device to your profile to wake your screen for newly published rosters.</p><button onClick={handleEnablePush} className="bg-blue-600 text-white px-6 py-3.5 rounded-xl font-bold w-full">Enable Notifications</button></div>
      </div>
    </div>
  );
};
