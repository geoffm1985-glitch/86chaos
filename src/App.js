...
// --- AUTO-ASK + TOKEN REPAIR FOR PUSH NOTIFICATIONS ---
  ...
  const renderMainContent = () => {
    ...
    if (activeTabState === 'published') return <TabMasterSchedule key={`pub-${rId}-${liveAppUser?.id}`} currentDate={currentDate} setCurrentDate={setCurrentDate} onSubTabChange={setActiveScheduleSubTab} appUser={liveAppUser} users={scheduleDisplayUsers} shifts={shifts} shiftSwaps={shiftSwaps} timeOffRequests={timeOffRequests} events={events} addToast={addToast} initialSubTab={activeScheduleSubTab || "my-schedule"} voiceScheduleSubTabTarget={voiceScheduleSubTabTarget} clientData={displayClientData} myScheduleLegacyRescueState={myScheduleLegacyRescueState} onRetryMyScheduleLegacyRescue={() => setMyScheduleRescueNonce(v => v + 1)} scheduleBuilderProps={{ currentDate, users: scheduleDisplayUsers, shifts, events, timeOffRequests, timePunches, addToast, appUser: liveAppUser, clientData: displayClientData }} />;
