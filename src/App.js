...
const firebaseConfig = { apiKey: "AIzaSyA0kkmRCqGNoB1LXKfuCNIl1JKDyQci9hA", authDomain: "cheers-34b8d.firebaseapp.com", projectId: "cheers-34b8d", storageBucket: "cheers-34b8d.firebasestorage.app", messagingSenderId: "762225019248", appId: "1:762225019248:web:3e142c9563e58ca762a7b5" };
const app = initializeApp(firebaseConfig); const db = getFirestore(app); const auth = getAuth(app);
let messaging = null; try { if (typeof window !== 'undefined' && 'Notification' in window) messaging = getMessaging(app); } catch (e) { console.warn(e); }
...
const getToday = () => formatDate(new Date()); const addDays = (d, days) => { const dt = new Date(d + 'T12:00:00'); dt.setDate(dt.getDate() + days); return formatDate(dt); };
const getMonthStr = (d) => (d || getToday()).substring(0, 7); const formatDisplayDate = (d) => new Date(d + 'T12:00:00').toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' });
...
const CheersLogo = ({ isDark }) => (<svg viewBox="0 0 400 120" className="h-10 sm:h-12 w-auto drop-shadow-sm" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M190,20 C70,0 0,35 0,65 C0,95 100,110 270,110 L270,105 C100,105 30,95 30,65 C30,40 80,25 190,20 Z" className={isDark ? "text-slate-100" : "text-slate-900"} /><text x="95" y="85" fontFamily="'Brush Script MT', 'Great Vibes', cursive" fontStyle="italic" fontSize="90" fontWeight="900" className={isDark ? "text-slate-100" : "text-slate-900"} letterSpacing="-1">Cheers</text></svg>);
...
export default function App() {
  ...
  return (
    <div className={`min-h-screen font-sans flex flex-col ${isDark ? 'bg-slate-900 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      <style>{`@keyframes toastSlide { from { transform: translateY(-20px); opacity: 0; } to { transform: translateY(0); opacity: 1; } } .animate-toast { animation: toastSlide 0.3s cubic-bezier(0.16, 1, 0.3, 1) forwards; } .dark input[type="date"]::-webkit-calendar-picker-indicator, .dark input[type="time"]::-webkit-calendar-picker-indicator { filter: invert(1); }`}</style>
      <header className={`sticky top-0 z-40 shadow-sm border-b h-16 flex items-center justify-between px-4 ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}><CheersLogo isDark={isDark} /><button onClick={() => setIsMenuOpen(true)} className="p-2 border rounded-xl shadow-sm dark:bg-slate-800 dark:border-slate-700 bg-slate-50"><Menu size={20} /></button></header>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border dark:border-slate-700 p-4">
        ...
        <div className="divide-y dark:divide-slate-700">{sortedSales.map(s => (
          <div key={s.id} className="py-3 flex justify-between items-center">
            <div><span className="font-bold block dark:text-white">{formatDisplayDate(s.date)}</span><span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded mt-1 inline-block ${s.tag==='Standard Day'?'bg-slate-100 text-slate-500 dark:bg-slate-700':'bg-amber-100 text-amber-700 dark:bg-amber-900/30'}`}>{s.tag}</span></div>
...
const TabTeam = ({ appUser, users, addToast }) => {
  ...
  const handleAdd = async (e) => {
    ...
    addToast('Staff Added', `Password generated. SMS client opened.`);
    const msg = `Welcome to Cheers! Log in at cheers-portal.vercel.app. Email: ${email} Temp Password: ${tPass}`;
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm border dark:border-slate-700">
          ...
          <form onSubmit={handleAdd} className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4">
              ...
              <button className="bg-blue-600 text-white px-6 py-3 rounded-xl font-bold w-full sm:w-auto mt-4 sm:mt-0 shadow-sm">Add & Notify</button>
            </div>
          </form>
        </div>
      )}
        ...
        <table className="w-full text-left border-collapse"><tbody className="divide-y dark:divide-slate-700">
          {displayUsers.map(u => (
            <tr key={u.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
              ...
              <td className="p-3"><span className={`text-[10px] font-black uppercase px-2 py-1 rounded-full ${u.role==='Bartender'?'bg-blue-100 text-blue-800 dark:bg-blue-900/30':'bg-orange-100 text-orange-800 dark:bg-orange-900/30'}`}>{u.role}</span>{u.isAdmin && <span className="ml-2 text-[10px] font-black bg-slate-900 dark:bg-slate-600 text-white px-2 py-1 rounded-full">Admin</span>}</td>
              <td className="p-3 text-right">{appUser?.isAdmin && u.email !== MASTER_ADMIN_EMAIL && <button onClick={()=>{if(window.confirm("Remove staff?")) deleteDoc(doc(db, "users", u.id));}} className="p-2 text-slate-400 hover:text-red-500"><Trash2 size={18}/></button>}</td>
            </tr>
          ))}
        </tbody></table>
      </div>
...
const TabPublished = ({ currentDate, appUser, users, shifts, shiftSwaps, addToast }) => {
  ...
  const handleOfferSwap = async (shift) => {
    ...
    // Broadcast to Message Board automatically
      ...
      {appUser?.isAdmin && shiftSwaps.filter(sw=>sw.status==='pending_approval').length > 0 && (
         <div className="bg-amber-50 dark:bg-slate-800 p-4 border border-amber-200 dark:border-amber-800 rounded-3xl space-y-3 shadow-sm">
           ...
           {shiftSwaps.filter(sw=>sw.status==='pending_approval').map(sw => (
             <div key={sw.id} className="flex justify-between items-center bg-white dark:bg-slate-700 p-3 rounded-xl border shadow-sm gap-2">
      ...
      {availableSwaps.length > 0 && (
         <div className="bg-blue-50 dark:bg-slate-800 p-4 border border-blue-200 dark:border-blue-800 rounded-3xl space-y-3 shadow-sm">
           ...
           <div className="grid gap-2 sm:grid-cols-2">{availableSwaps.map(sw => {
              ...
              return (
                <div key={sw.id} className="bg-white dark:bg-slate-700 p-3 rounded-xl border flex justify-between items-center shadow-sm">
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border dark:border-slate-700 rounded-2xl overflow-hidden shadow-sm print-container">
      ...
      <div className="grid grid-cols-7 border-t border-l dark:border-slate-700">
        {['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map(d=><div key={d} className="p-1 bg-slate-50 dark:bg-slate-900 text-center font-black text-[10px] text-slate-500 border-b border-r uppercase cell">{d}</div>)}
        {Array.from({length:firstDay}).map((_,i)=><div key={`e-${i}`} className="bg-slate-50/50 dark:bg-slate-800/50 border-b border-r min-h-[50px] cell"/>)}
...
const TabPrep = ({ currentDate, prepItems, appUser }) => {
  ...
  const handleAdd = async (e) => { e.preventDefault(); if(text.trim()) { await addDoc(collection(db, "prepItems"), { date: prepDate, text: text.trim(), isCompleted: false, completedBy: '', qty: 1 }); setText(''); } };
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      <div className="bg-white dark:bg-slate-800 p-4 rounded-3xl border dark:border-slate-700 flex justify-between items-center shadow-sm"><h3 className="font-bold flex items-center gap-2 dark:text-white"><ClipboardList className="text-blue-500"/> Prep List Sync</h3><input type="date" value={prepDate} onChange={e=>setPrepDate(e.target.value)} className="p-2 border rounded-xl outline-none font-bold dark:bg-slate-700 dark:border-slate-600 dark:text-white"/></div>
      <form onSubmit={handleAdd} className="flex gap-2"><input type="text" value={text} onChange={e=>setText(e.target.value)} className="flex-1 p-3 rounded-2xl border dark:border-slate-700 outline-none dark:bg-slate-800 dark:text-white" placeholder="Add custom prep task..." required/><button className="bg-blue-600 text-white px-5 rounded-2xl font-bold"><Plus size={20}/></button></form>
      <div className="bg-white dark:bg-slate-800 rounded-3xl border dark:border-slate-700 divide-y dark:divide-slate-700 overflow-hidden shadow-sm">{items.map(i=>{
        const isDone = i.isMaster ? !!i.completedDates?.[prepDate] : i.isCompleted; const doneBy = i.isMaster ? i.completedDates?.[prepDate] : i.completedBy;
        return (
        <div key={i.id} className="p-3 flex items-center gap-3">
          <div className="flex-1 truncate"><span className={`font-bold ${isDone?'line-through text-slate-400':'dark:text-white'}`}>{i.text}</span> {doneBy && <span className="text-[9px] font-black text-emerald-600 bg-emerald-50 dark:bg-emerald-900/30 px-1.5 py-0.5 rounded ml-2">✓ {doneBy}</span>} {i.isMaster&&<span className="block text-[9px] font-black text-blue-500 uppercase">Master List</span>}</div>
          <button onClick={()=>toggle(i)} className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold transition-colors ${isDone?'bg-slate-200 text-slate-500 dark:bg-slate-700':'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800'}`}>{isDone ? <Repeat size={18}/> : <Check size={18}/>}</button>
...
const TabInventory = ({ inventoryItems, sales, addToast }) => {
  ...
  const getHistoricalContext = () => {
    ...
    const pastSale = sales.find(s => s.date === pastStr);
    return pastSale ? `Last year (${pastStr}): $${pastSale.amount.toFixed(2)} [${pastSale.tag}]` : `No local trend data for this week last year.`;
  };

  const handleOrderChange = (id, val) => setOrderOverrides(p => ({...p, [id]: Math.max(0, val)}));
          ...
          <tbody className="divide-y dark:divide-slate-700">{itemsToOrder.map(item => {
            ...
            return (
              <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                ...
                <td className="p-3">
                  <div className="flex items-center justify-end gap-1"><button onClick={()=>handleOrderChange(item.id, currentOrder - 1)} className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-700 font-bold dark:text-white hover:bg-slate-200 transition-colors">-</button><input type="number" min="0" value={currentOrder} onChange={e=>handleOrderChange(item.id, parseInt(e.target.value)||0)} className="w-12 h-8 text-center font-black bg-blue-50 dark:bg-slate-800 text-blue-700 dark:text-blue-400 rounded-lg outline-none"/><button onClick={()=>handleOrderChange(item.id, currentOrder + 1)} className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-700 font-bold dark:text-white hover:bg-slate-200 transition-colors">+</button></div>
                </td>
              </tr>
            )
          })}</tbody>
        </table>
        {itemsToOrder.length > 0 && <div className="p-4 bg-slate-50 dark:bg-slate-900 border-t dark:border-slate-700"><button onClick={executeOrder} className="w-full bg-blue-600 text-white p-3.5 rounded-xl font-black text-lg shadow-sm">Finalize Order List</button></div>}
      </div>
    </div>
  );
};

// --- SCHEDULE MAKER (With Strict Validation Engine) ---
const TabSchedule = ({ currentDate, users, shifts, addToast }) => {
  ...
  const validateSchedule = () => {
    ...
    monthDays.forEach(d => {
       ...
       if (!bar.some(s => s.startTime <= '11:00') || !bar.some(s => s.endTime >= bClose)) warnings.push(`[${dateFmt}] Bar: Missing open-to-close coverage (11a-${bClose==='02:30'?'2:30a':'2a'})`);

       const countO = (arr, st, en) => arr.filter(s => s.startTime <= st && s.endTime >= en).length;
       
       const lBar = countO(bar, '11:00', '14:00'); const lKit = countO(kit, '11:00', '14:00');
       if (isFri) { if (lBar<2 || lKit<3) warnings.push(`[${dateFmt}] Lunch Rush: Missing coverage (Needs 2 Bar/3 Kit, found ${lBar} Bar/${lKit} Kit)`); }
       else if (isSat || isSun) { if (lBar<1 || lKit<2) warnings.push(`[${dateFmt}] Day Rush: Missing coverage (Needs 1 Bar/2 Kit, found ${lBar} Bar/${lKit} Kit)`); }
       else { if (lBar<2 || lKit<2) warnings.push(`[${dateFmt}] Lunch Rush: Missing coverage (Needs 2 Bar/2 Kit, found ${lBar} Bar/${lKit} Kit)`); }

       const dBar = countO(bar, '17:00', '20:00'); const dKit = countO(kit, '17:00', '20:00');
       if (isFri) { if (dBar<2 || dKit<3) warnings.push(`[${dateFmt}] Dinner Rush: Missing coverage (Needs 2 Bar/3 Kit, found ${dBar} Bar/${dKit} Kit)`); }
       else if (!isSat && !isSun) { if (dBar<2 || dKit<2) warnings.push(`[${dateFmt}] Dinner Rush: Missing coverage (Needs 2 Bar/2 Kit, found ${dBar} Bar/${dKit} Kit)`); }

       const cls = bar.filter(s => s.endTime >= '02:00').length;
       if ((isFri || isSat) && cls < 2) warnings.push(`[${dateFmt}] Bar: Missing closers (Needs 2, found ${cls})`);
       else if (!isFri && !isSat && cls < 1) warnings.push(`[${dateFmt}] Bar: Missing closer (Needs 1, found ${cls})`);
    });
    return warnings;
  };
  ...
  return (
    <div className="space-y-4 pb-12">
      ...
      <div className="flex gap-2 p-3 bg-slate-100 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 items-end"><div className="flex-1"><label className="block text-[10px] font-bold mb-1 dark:text-slate-400">Start</label><input type="time" value={startTime} onChange={e=>setStartTime(e.target.value)} className="w-full p-2 text-xs rounded-lg border outline-none dark:bg-slate-700 dark:border-slate-600 dark:text-white"/></div><div className="flex-1"><label className="block text-[10px] font-bold mb-1 dark:text-slate-400">End</label><input type="time" value={endTime} onChange={e=>setEndTime(e.target.value)} className="w-full p-2 text-xs rounded-lg border outline-none dark:bg-slate-700 dark:border-slate-600 dark:text-white"/></div><button onClick={handleAssign} disabled={!selectedEmp||assignDates.length===0} className="bg-blue-600 text-white px-4 h-[34px] rounded-lg font-bold text-xs disabled:opacity-50 flex-1">Assign ({assignDates.length})</button></div>
    </div>
  );
};
