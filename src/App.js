...
import { Bell, BookOpen, Briefcase, Bug, CalendarDays, ChevronLeft, ChevronRight, ClipboardList, Clock, Database, DollarSign, HelpCircle, Home, Loader2, Lock, Menu, MessageSquare, Mic, Moon, MoreHorizontal, Package, Rocket, Search, Settings, Shield, Send, Users, X } from 'lucide-react';
...
import { ExactToday, ExactSchedule, ExactPrep, ExactInventory, ExactRecipes, ExactFinancials, ExactSettings, ExactTeam, ExactMessages, ExactSystemAdmin } from './features/referenceExact';
...
if (liveAppUser && clientData) {
  ...
  // Login tours are first-login training, not release notes. Keep these keys versionless
  // so app updates never make a completed, skipped, or already-shown tour pop again.
  const getTourSessionKey = (mode = tourMode) => `tourSeenThisSession_${liveAppUser?.id || 'guest'}_${mode || 'tour'}`;
  ...
  const getTourShownKey = (mode = tourMode) => mode === 'manager' ? `managerTourShown_${rId || 'workspace'}` : `employeeTourShown_${liveAppUser?.id || 'employee'}`;
  const persistTourShown = async (mode = tourMode, { completed = false } = {}) => {
    if (!liveAppUser || isDemoMode || !mode) return;
    const now = new Date().toISOString();
    try {
      localStorage.setItem(getTourShownKey(mode), 'true');
      if (completed) localStorage.setItem(getTourCompleteKey(mode), 'true');
      if (mode === 'manager' && liveAppUser?.id) {
        await updateDoc(doc(db, 'users', liveAppUser.id), completed
          ? {
              'settings.managerTourComplete': true,
              'settings.managerTourCompletedAt': now,
              'settings.managerTourSkipped': false,
              'settings.managerTourShown': true,
              'settings.managerTourShownAt': now
            }
          : {
              'settings.managerTourSkipped': true,
              'settings.managerTourSkippedAt': now,
              'settings.managerTourShown': true,
              'settings.managerTourShownAt': now
            });
      }
      if (mode === 'manager' && rId) {
        await updateDoc(doc(db, 'restaurants', rId), completed
          ? {
              'systemSettings.workspaceOnboardingComplete': true,
              'systemSettings.workspaceOnboardingCompletedAt': now,
              'systemSettings.workspaceOnboardingSkipped': false,
              'systemSettings.workspaceOnboardingTourShown': true,
              'systemSettings.workspaceOnboardingTourShownAt': now
            }
          : {
              'systemSettings.workspaceOnboardingSkipped': true,
              'systemSettings.workspaceOnboardingSkippedAt': now,
              'systemSettings.workspaceOnboardingTourShown': true,
              'systemSettings.workspaceOnboardingTourShownAt': now
            });
      }
      if (mode === 'employee' && liveAppUser?.id) {
        await updateDoc(doc(db, 'users', liveAppUser.id), completed
          ? {
              ...
              'settings.onboardingTourShown': true,
              'settings.onboardingTourShownAt': now
            }
          : {
      ...
      console.warn('Tour first-login marker save failed', e);
    }
  };
  const dismissTourForNow = () => {
    ...
    void persistTourShown(mode, { completed: false });
    setTourMode(null);
    setTourStep(0);
  };
  const finishTour = async () => {
    ...
    await persistTourShown(mode, { completed: true });
  };
  ...
  useEffect(() => {
    ...
    const managerShownLocal = rId ? localStorage.getItem(`managerTourShown_${rId}`) === 'true' : false;
    const employeeCompleteLocal = liveAppUser?.id ? localStorage.getItem(`employeeTourComplete_${liveAppUser.id}`) === 'true' : false;
    const employeeShownLocal = liveAppUser?.id ? localStorage.getItem(`employeeTourShown_${liveAppUser.id}`) === 'true' : false;
    const managerTourAlreadySeen = Boolean(
      displayClientData?.workspaceOnboardingComplete === true ||
      displayClientData?.workspaceOnboardingSkipped === true ||
      displayClientData?.workspaceOnboardingTourShown === true ||
      displayClientData?.systemSettings?.workspaceOnboardingComplete === true ||
      displayClientData?.systemSettings?.workspaceOnboardingSkipped === true ||
      displayClientData?.systemSettings?.workspaceOnboardingTourShown === true ||
      liveAppUser?.settings?.managerTourComplete === true ||
      liveAppUser?.settings?.managerTourSkipped === true ||
      liveAppUser?.settings?.managerTourShown === true ||
      managerCompleteLocal ||
      managerShownLocal
    );
    const employeeTourAlreadySeen = Boolean(
      liveAppUser?.onboardingComplete === true ||
      liveAppUser?.onboardingSkipped === true ||
      liveAppUser?.onboardingTourShown === true ||
      liveAppUser?.settings?.onboardingComplete === true ||
      liveAppUser?.settings?.onboardingSkipped === true ||
      liveAppUser?.settings?.onboardingTourShown === true ||
      employeeCompleteLocal ||
      employeeShownLocal
    );
    if ((liveAppUser.isAdmin || liveAppUser.permissions?.team) && displayClientData && !managerTourAlreadySeen) {
      const key = getTourSessionKey('manager');
      if (sessionStorage.getItem(key) === 'true') return;
      sessionStorage.setItem(key, 'true');
      localStorage.setItem(getTourShownKey('manager'), 'true');
      void persistTourShown('manager', { completed: false });
      setTourMode('manager');
      setTourStep(0);
    } else if (!employeeTourAlreadySeen) {
      const key = getTourSessionKey('employee');
      if (sessionStorage.getItem(key) === 'true') return;
      sessionStorage.setItem(key, 'true');
      localStorage.setItem(getTourShownKey('employee'), 'true');
      void persistTourShown('employee', { completed: false });
      setTourMode('employee');
      setTourStep(0);
    }
  }, [liveAppUser?.id, liveAppUser?.onboardingComplete, liveAppUser?.onboardingSkipped, liveAppUser?.onboardingTourShown, liveAppUser?.settings?.onboardingComplete, liveAppUser?.settings?.onboardingSkipped, liveAppUser?.settings?.onboardingTourShown, liveAppUser?.settings?.managerTourComplete, liveAppUser?.settings?.managerTourSkipped, liveAppUser?.settings?.managerTourShown, liveAppUser?.isAdmin, liveAppUser?.permissions?.team, displayClientData?.workspaceOnboardingComplete, displayClientData?.workspaceOnboardingSkipped, displayClientData?.workspaceOnboardingTourShown, displayClientData?.systemSettings?.workspaceOnboardingComplete, displayClientData?.systemSettings?.workspaceOnboardingSkipped, displayClientData?.systemSettings?.workspaceOnboardingTourShown, rId, isDemoMode, tourMode]);
...
// --- AUTO-ASK + TOKEN REPAIR FOR PUSH NOTIFICATIONS ---
  ...
  const ReferenceFunctionShell = ({ preview, children, label = 'Full 16.0.6 tools and controls' }) => (
    <div className="srx-function-safe-shell">
      {preview}
      <details className="srx-real-tools" open>
        <summary>{label} preserved below</summary>
        <div className="srx-real-tools-body">{children}</div>
      </details>
    </div>
  );
  ...
  const renderMainContent = () => {
    ...
    if (activeTabState === 'today') return <ReferenceFunctionShell label="Full Today / Manager Brief tools" preview={<ExactToday currentDate={currentDate} appUser={liveAppUser} users={displayUsers} shifts={shifts} sales={sales} timePunches={timePunches} inventoryItems={inventoryItems} prepItems={prepItems} tasks={tasks} restaurantAdminAlerts={restaurantAdminAlerts} clientData={displayClientData} setActiveTab={setActiveTab} />}><TabToday key={`tdy-${rId}`} currentDate={currentDate} appUser={liveAppUser} users={displayUsers} shifts={shifts} shiftSwaps={shiftSwaps} timeOffRequests={timeOffRequests} events={events} sales={sales} timePunches={timePunches} inventoryItems={inventoryItems} maintenanceLogs={maintenanceLogs} prepItems={prepItems} tasks={tasks} recipes={recipes} menuDependencies={menuDependencies} restaurantAdminAlerts={restaurantAdminAlerts} clientData={displayClientData} setActiveTab={setActiveTab} addToast={addToast} registerUndo={registerUndo} /></ReferenceFunctionShell>;
    if (activeTabState === 'schedule' && (liveAppUser?.isAdmin || liveAppUser?.permissions?.schedule)) return <ReferenceFunctionShell label="Full Schedule Builder, coverage, availability, and open shifts tools" preview={<ExactSchedule currentDate={currentDate} users={displayUsers} shifts={shifts} timePunches={timePunches} timeOffRequests={timeOffRequests} sales={sales} clientData={displayClientData} appUser={liveAppUser} setActiveTab={setActiveTab} />}><TabMasterSchedule key={`schpub-${rId}-${liveAppUser?.id}`} currentDate={currentDate} setCurrentDate={setCurrentDate} onSubTabChange={setActiveScheduleSubTab} appUser={liveAppUser} users={displayUsers} shifts={shifts} shiftSwaps={shiftSwaps} timeOffRequests={timeOffRequests} events={events} addToast={addToast} initialSubTab="schedule-builder" voiceScheduleSubTabTarget={voiceScheduleSubTabTarget} clientData={displayClientData} scheduleBuilderProps={{ currentDate, users: displayUsers, shifts, events, timeOffRequests, timePunches, addToast, appUser: liveAppUser, clientData: displayClientData }} /></ReferenceFunctionShell>;
    ...
    if (activeTabState === 'published') return <ReferenceFunctionShell label="Full Schedule, Time Clock, shift trades, and availability tools" preview={<ExactSchedule currentDate={currentDate} users={displayUsers} shifts={shifts} timePunches={timePunches} timeOffRequests={timeOffRequests} sales={sales} clientData={displayClientData} appUser={liveAppUser} setActiveTab={setActiveTab} />}><TabMasterSchedule key={`pub-${rId}-${liveAppUser?.id}`} currentDate={currentDate} setCurrentDate={setCurrentDate} onSubTabChange={setActiveScheduleSubTab} appUser={liveAppUser} users={displayUsers} shifts={shifts} shiftSwaps={shiftSwaps} timeOffRequests={timeOffRequests} events={events} addToast={addToast} voiceScheduleSubTabTarget={voiceScheduleSubTabTarget} clientData={displayClientData} scheduleBuilderProps={{ currentDate, users: displayUsers, shifts, events, timeOffRequests, timePunches, addToast, appUser: liveAppUser, clientData: displayClientData }} /></ReferenceFunctionShell>;
    ...
    if ((activeTabState === 'financials' || activeTabState === 'sales' || activeTabState === 'labor') && (liveAppUser?.isSuperAdmin || liveAppUser?.isAdmin || liveAppUser?.permissions?.labor || liveAppUser?.permissions?.sales)) return <ReferenceFunctionShell label="Full Financials, Daily Ledger, labor, and timesheet tools" preview={<ExactFinancials currentDate={currentDate} appUser={liveAppUser} users={displayUsers} sales={sales} timePunches={timePunches} clientData={displayClientData} setActiveTab={setActiveTab} />}><TabFinancials key={`fin-${rId}`} currentDate={currentDate} users={displayUsers} shifts={shifts} sales={sales} timePunches={timePunches} addToast={addToast} appUser={liveAppUser} clientData={displayClientData} setActiveTab={setActiveTab} initialSubTab={activeTabState === 'sales' ? 'ledger' : activeTabState === 'labor' ? 'labor' : 'overview'} /></ReferenceFunctionShell>;
    if (activeTabState === 'messages' && displayClientFeatures?.messages !== false) return <ReferenceFunctionShell label="Full Message Board tools" preview={<ExactMessages events={events} setActiveTab={setActiveTab} />}><TabMessages key={`msg-${rId}`} events={events} appUser={liveAppUser} users={displayUsers} addToast={addToast} /></ReferenceFunctionShell>;
    if (activeTabState === 'prep' && displayClientFeatures?.prep !== false) return <ReferenceFunctionShell label="Full Prep, Tasks, labels, and 86 alert tools" preview={<ExactPrep currentDate={currentDate} appUser={liveAppUser} prepItems={prepItems} tasks={tasks} inventoryItems={inventoryItems} setActiveTab={setActiveTab} />}><TabPrep key={`prp-${rId}`} currentDate={currentDate} appUser={liveAppUser} addToast={addToast} setLabelsToPrint={setLabelsToPrint} /></ReferenceFunctionShell>;
    if (activeTabState === 'recipes' && displayClientFeatures?.recipes !== false) return <ReferenceFunctionShell label="Full Recipes and menu-cost tools" preview={<ExactRecipes appUser={liveAppUser} setActiveTab={setActiveTab} />}><TabRecipes key={`rec-${rId}`} appUser={liveAppUser} addToast={addToast} voiceRecipeTarget={voiceRecipeTarget} /></ReferenceFunctionShell>;
    if (activeTabState === 'inventory' && displayClientFeatures?.inventory !== false) return <ReferenceFunctionShell label="Full Inventory, invoice scan, menu scan, burn log, and ordering tools" preview={<ExactInventory appUser={liveAppUser} clientData={displayClientData} inventoryItems={inventoryItems} setActiveTab={setActiveTab} />}><TabInventory key={`inv-${rId}-${inventorySubTabTarget || 'default'}`} addToast={addToast} appUser={liveAppUser} clientData={displayClientData} initialSubTab={inventorySubTabTarget} onInitialSubTabConsumed={() => setInventorySubTabTarget(null)} /></ReferenceFunctionShell>;
    ...
    if (activeTabState === 'team' && displayClientFeatures?.team !== false) return <ReferenceFunctionShell label="Full Staff Roster tools" preview={<ExactTeam users={displayUsers} appUser={liveAppUser} clientData={displayClientData} setActiveTab={setActiveTab} />}><TabTeam key={`tea-${rId}`} appUser={liveAppUser} users={displayUsers} clientData={displayClientData} addToast={addToast} /></ReferenceFunctionShell>;
    ...
    if (activeTabState === 'settings' && !isDemoMode) return <ReferenceFunctionShell label="Full Settings, preferences, account security, branding, and workspace tools" preview={<ExactSettings appUser={liveAppUser} clientData={displayClientData} users={displayUsers} setActiveTab={setActiveTab} />}><TabSettings key={`set-${rId}`} addToast={addToast} appUser={liveAppUser} clientData={displayClientData} users={displayUsers} presenceSelf={selfPresenceRecord} /></ReferenceFunctionShell>;
    ...
    if (activeTabState === 'godmode' && (liveAppUser?.isSuperAdmin === true || serverSaysSuperAdmin || (MASTER_ADMIN_EMAIL && (liveAppUser?.email || '').toLowerCase() === MASTER_ADMIN_EMAIL.toLowerCase()))) return <ReferenceFunctionShell label="Full System Administrator tools" preview={<ExactSystemAdmin appUser={{ ...liveAppUser, isSuperAdmin: true, serverAdminCheck }} setActiveTab={setActiveTab} />}><TabGodMode key={`god-${rId}`} appUser={{ ...liveAppUser, isSuperAdmin: true, serverAdminCheck }} addToast={addToast} setGhostTenant={setGhostTenant} setActiveTab={setActiveTab} /></ReferenceFunctionShell>;
    ...
    <div style={appThemeStyle} onClickCapture={blockDemoMutation} onSubmitCapture={blockDemoMutation} className={`native-app-shell premium-app-shell serious-app-system-v104 exact-reference-ui-v108 reference-app-v151 desktop-pro-shell ui-v13-polished ui-v12-compact cockpit-shell ${activeTabState === 'godmode' ? '' : 'non-admin-controls-compact'} kitchen-simple-shell ui-density-${liveAppUser?.preferences?.uiDensity || displayClientData?.systemSettings?.uiDensity || 'compact'} recipe-density-${liveAppUser?.preferences?.recipeDensity || displayClientData?.systemSettings?.recipeCardDensity || 'tight'} motion-${liveAppUser?.preferences?.motionMode || displayClientData?.systemSettings?.cockpitLights || 'normal'} min-h-screen font-sans flex flex-col w-full max-w-[100vw] overflow-x-hidden ${T.bg}`}>
      ...
      <header className="app-header native-command-bar sticky top-0 z-40 border-b flex items-center justify-between px-3 sm:px-4 bg-[#0F1318]/96 backdrop-blur-md border-[#202A31]">
        ...
        <div className="native-command-actions flex items-center gap-2 flex-shrink-0">
          <button type="button" onClick={() => window.dispatchEvent(new CustomEvent('chaosOpenVoiceCommandDock'))} className="reference-top-action"><Mic size={17}/> <span className="hidden sm:inline">86Voice Command</span></button>
          ...
          <button type="button" onClick={() => setIsMenuOpen(true)} className="native-icon-button hidden md:inline-flex" title="All tools"><Menu size={18}/>{hasAnyMenuAlert && <span className="native-alert-dot"></span>}</button>
