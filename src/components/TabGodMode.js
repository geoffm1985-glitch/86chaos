          ...
          {ADMIN_SECTIONS.map((section) => {
            ...
            return (
              <div key={section.id} className="relative group">
                ...
                <button type="button" onClick={(event) => openAdminHelp(section.id, event)} className="absolute -right-1 -top-1 w-6 h-6 rounded-full border border-[#D4A381]/60 bg-[#0B0E11] text-[#D4A381] hidden group-hover:flex items-center justify-center shadow-md" aria-label={`Explain ${section.label}`} title={`Explain ${section.label}`}>
                  <HelpCircle size={12}/>
                </button>
              </div>
            );
