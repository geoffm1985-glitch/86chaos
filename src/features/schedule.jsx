...
import * as scheduleMonthPrint from '../core/scheduleMonthPrint.cjs';
...
{subTab === 'full-schedule' && (() => {
      ...
      {subTab === 'month-view' && <div className="animate-[slideIn_0.2s_ease-out]"><TabMonth currentDate={currentDate} users={users} shifts={shifts} appUser={appUser} addToast={addToast} /></div>}
...
const TabMonth = ({ currentDate, users, shifts, appUser, addToast }) => {
  const [roleFilter, setRoleFilter] = useState('All');
  const uniqueRoles = ['All', ...new Set(users.map(u => u.role).filter(Boolean))].sort();

  const monthStr = getMonthStr(currentDate);
  const monthCalendarModel = useMemo(() => {
    ...
    const dayRows = Array.from({ length: days }).map((_, i) => {
      ...
      return { date, dayNumber: i + 1, shifts: shiftsForDay };
    });
    const roleFilterLabel = roleFilter === 'All' ? 'Whole Schedule' : roleFilter === 'ME' ? 'My Shifts Only' : roleFilter;
    return {
      monthStr,
      firstDay,
      days,
  ...
  }, [monthStr, roleFilter, users, shifts, appUser]);

  const handlePrintCalendar = useCallback(() => {
    try {
      const opened = scheduleMonthPrint.openScheduleMonthPrintWindow(monthCalendarModel);
      if (!opened) {
        addToast?.('Print Blocked', 'Allow pop-ups for 86 Chaos and try Print Calendar again.');
      }
    } catch (error) {
      addToast?.('Print Error', error?.message || 'Month View calendar could not be prepared for printing.');
    }
  }, [monthCalendarModel, addToast]);

  return (
    <div className={`${T.card} overflow-hidden`}>
<div className="flex justify-between items-center p-2 border-b border-[#2A353D] bg-[#12161A]">
        <div className="flex items-center gap-2">
    ...
    <select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)} className="bg-[#1A2126] border border-[#2A353D] text-[#D4A381] text-xs font-bold rounded-lg px-2 py-1.5 outline-none cursor-pointer shadow-inner">
      ...
      <div className={`grid grid-cols-7 border-t border-l ${T.border}`}>
        ...
        {Array.from({length:monthCalendarModel.firstDay}).map((_,i)=><div key={`e-${i}`} className={`bg-[#12161A]/50 border-b border-r ${T.border} min-h-[50px] cell`}/>)}
        
{monthCalendarModel.dayRows.map(dayRow => (
            <div key={dayRow.date} className={`p-0.5 border-b border-r ${T.border} min-h-[50px] flex flex-col cell ${dayRow.shifts.length >= 6 ? 'print-day-dense' : ''}`}>
              <span className={`text-right text-[9px] font-black ${T.muted} mb-0.5 cell-date`}>{dayRow.dayNumber}</span>
