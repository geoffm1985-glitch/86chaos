...
const TabMonth = ({ currentDate, users, shifts, appUser }) => {
  ...
  const buildPrintableCalendarHtml = () => {
    ...
    return `<!doctype html><html><head><meta charset="utf-8" /><title>86 Chaos Schedule ${escapeSchedulePrintHtml(monthTitle)}</title><style>@page{size:letter landscape;margin:0.15in}*{box-sizing:border-box}html,body{width:10.7in;height:8.2in;margin:0;padding:0;overflow:hidden}body{color:#000;background:#fff;font-family:Arial,Helvetica,sans-serif}.calendar{width:100%;height:100%;display:grid;grid-template-rows:auto auto minmax(0,1fr);page-break-inside:avoid;break-inside:avoid-page}h1{margin:0 0 4px;text-align:center;font-size:17px;line-height:1.05;text-transform:uppercase;letter-spacing:.04em}.meta{margin:0 0 4px;display:flex;justify-content:space-between;gap:8px;font-size:8.5px;font-weight:700;color:#111}.grid{min-height:0;height:100%;display:grid;grid-template-columns:repeat(7,1fr);grid-template-rows:18px repeat(${weeks},minmax(0,1fr));border-top:1.5px solid #000;border-left:1.5px solid #000}.weekday,.day{border-right:1.5px solid #000;border-bottom:1.5px solid #000}.weekday{display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:900;text-transform:uppercase;background:#f1f5f9}.day{min-height:0;height:100%;padding:2px;overflow:hidden}.blank{background:#f8fafc}.date{text-align:right;font-size:10px;font-weight:900;margin-bottom:1px}.shiftStack{display:flex;flex-direction:column;gap:.5px}.shift{border:1px solid #94a3b8;border-radius:2px;background:#f8fafc;padding:0 1px;font-size:6px;line-height:1;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:clip;color:#000}</style></head><body><div class="calendar"><h1>86 Chaos Schedule ${escapeSchedulePrintHtml(monthTitle)}</h1><div class="meta"><span>${escapeSchedulePrintHtml(visibleMonthShifts.length)} published shifts</span><span>Printed ${escapeSchedulePrintHtml(new Date().toLocaleString())}</span></div><div class="grid">${weekdayHeader}${blanks}${dayCells}</div></div><script>window.addEventListener('load',function(){setTimeout(function(){window.focus();window.print();},150);});</script></body></html>`;
  };
