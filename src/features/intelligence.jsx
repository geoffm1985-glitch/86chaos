...
import { hasNativeLocalReminderBridge, readDeviceLocalReminders, upsertDeviceLocalReminder, scheduleDeviceLocalReminder, shareReminderWithoutServer } from '../core/localReminderBridge';
...
const TabPersonalReminders = ({ appUser, addToast }) => {const initial=getInitialReminderDate(),uid=auth?.currentUser?.uid||appUser?.id||'';const[title,setTitle]=useState(''),[notes,setNotes]=useState(''),[dateInput,setDateInput]=useState(initial.date),[timeInput,setTimeInput]=useState(initial.time),[deliveryMode,setDeliveryMode]=useState(()=>hasNativeLocalReminderBridge()?'device':'cloud'),[localRows,setLocalRows]=useState(()=>readDeviceLocalReminders());const cloudRows=usePersonalReminderRows(appUser,{enabled:!!appUser?.restaurantId&&!!appUser?.id,limitCount:120,fallbackLimitCount:60,debugLabel:'personal-reminders'}),rows=[...(cloudRows||[]),...localRows.map(r=>({...r,deviceLocal:true}))];const save=async e=>{e.preventDefault();const when=makeReminderDate(dateInput,timeInput);if(!title.trim()||!when)return addToast('Reminder','Enter reminder text, date, and time.');if(deliveryMode==='device'){const r={id:`local-${Date.now()}-${Math.random().toString(36).slice(2,8)}`,title:title.trim(),notes:notes.trim(),scheduledAt:when.toISOString(),status:'scheduled',deviceLocal:true,createdBy:uid,updatedAt:new Date().toISOString()};try{const d=await scheduleDeviceLocalReminder(r);setLocalRows(upsertDeviceLocalReminder({...r,deliveryMode:d.mode,closedAppCapable:d.closedApp}));addToast(d.closedApp?'Device Reminder Scheduled':'Local Reminder Saved',d.closedApp?'Closed-app alert scheduled on this phone.':'Web fallback saved locally.')}catch(err){addToast('Reminder Permission',err.message||'Could not schedule reminder.')}return}const q=await secureFetch('/api/personal-reminder-save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({restaurantId:appUser.restaurantId,assignedToUserId:uid,title:title.trim(),notes:notes.trim(),scheduledAt:when.toISOString(),recurrence:'none',timezone:Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC'})});if(!q.ok)throw new Error('Reminder could not be saved.');requestPersonalReminderRefresh({restaurantId:appUser.restaurantId,uid})};const share=async r=>{try{await shareReminderWithoutServer(r);addToast('Reminder Shared','No Firebase persistence was used.')}catch(e){addToast('Share Unavailable',e.message)}};return <div className="space-y-4" data-testid="personal-reminders-v17-1-27"><div className={`${T.card} p-4`}><h2 className="text-xl font-black text-white">Personal Reminders</h2><p className="text-xs text-slate-400">Device-local reminders plus cloud self-reminders. Sharing uses the OS/Web Share API only.</p></div><div className={`${T.card} p-4`} data-testid="device-local-reminder-status">{hasNativeLocalReminderBridge()?'Native bridge connected: closed-app reminders are available.':'Web fallback active. Closed-app delivery requires the native Android/iPhone wrapper.'}</div><form onSubmit={save} className={`${T.card} p-4 grid lg:grid-cols-5 gap-3`}><input value={title} onChange={e=>setTitle(e.target.value)} className={T.input}/><input type="date" value={dateInput} onChange={e=>setDateInput(e.target.value)} className={T.input}/><input type="time" value={timeInput} onChange={e=>setTimeInput(e.target.value)} className={T.input}/><select aria-label="Reminder delivery" value={deliveryMode} onChange={e=>setDeliveryMode(e.target.value)} className={T.input}><option value="device">This device only</option><option value="cloud">86 Chaos cloud</option></select><button className={T.btn}>Add</button><input value={notes} onChange={e=>setNotes(e.target.value)} className={`${T.input} lg:col-span-5`}/></form><div className={T.card}>{rows.map(r=><div key={(r.deviceLocal?'l:':'c:')+r.id} className={`${T.row} flex justify-between`}><span>{r.title}</span><button type="button" aria-label="Share reminder" onClick={()=>share(r)} className={T.btnAlt}><Share2 size={15}/></button></div>)}</div></div>};
