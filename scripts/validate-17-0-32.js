...
assert.equal(lock.version, '17.0.32');
assert.equal(lock.packages[''].version, '17.0.32');
assert.equal(version.version, '17.0.32');
assert.equal(version.build, '17.0.32');
assert.equal(version.releaseTitle, 'Hostile Certification Assertion Parity Repair');
...
assert(pkg.scripts['test:new-implementations']?.startsWith('npm run validate:17.0.32'));

for (const [file, needle] of [
  ['src/core/appCore.js', "CURRENT_VERSION = '17.0.32'"],
  ['api/_version.js', "APP_VERSION = '17.0.32'"],
  ['api/_version.js', "SECURITY_SCHEMA_VERSION = '17.0.32'"],
  ['api/_pos-bridge-config.js', "APP_RELEASE = '17.0.32'"],
  ['src/core/customerHelpKnowledge.js', "CUSTOMER_HELP_VERSION = '17.0.32'"],
  ['src/core/customerHelpKnowledge.cjs', "CUSTOMER_HELP_VERSION = '17.0.32'"],
  ['src/core/schedulePdf.js', '86 Chaos 17.0.32'],
...
for (const file of ['test-tools/certification/groups.json','test-tools/regressions/registry.json','test-tools/certification/cost-performance-baselines.json']) {
  assert.equal(json(file).release, '17.0.32', `${file} release identity matches`);
}

console.log('17.0.32 Hostile Certification Assertion Parity Repair validation passed; this does not certify the release.');
