...
assert.equal(pkg.version, '17.0.32');
assert.equal(lock.version, pkg.version);
assert.equal(lock.packages[''].version, pkg.version);
assert.equal(version.version, pkg.version);
assert.equal(version.build, pkg.version);
assert.equal(version.releaseTitle, 'Live System Administrator Concept 1 Repair');
assert.equal(pkg.scripts['test:source'], 'node scripts/validate-17-0-32.js');
assert.equal(pkg.scripts['validate:17.0.32'], 'node scripts/validate-17-0-32.js');
assert(pkg.scripts['test:repair:17.0.32']?.includes('test:current-release-targeted'));
...
assert(pkg.scripts['test:release:fast']?.includes('validate:17.0.32'));
...
assert(scope.includes("const CURRENT_RELEASE_VERSION = '17.0.32'"));
...
assert(management.includes('data-testid="system-admin-live-concept1"'));
assert(management.includes('data-testid="system-admin-desktop-directory"'));
assert(management.includes('data-testid="system-admin-mobile-directory"'));
assert(management.includes('data-testid="system-admin-live-nav-card"'));
...
assert(read('tests/86chaos-new-implementations/10-app-bootstrap-i18n-runtime.spec.cjs').includes('Version 17\\.0\\.32'));
...
]) assert(read(file).includes('17.0.32'), `${file} carries current version 17.0.32`);
for (const file of [
...
console.log('17.0.32 Live System Administrator Concept 1 Repair validation passed; this does not certify the release.');
