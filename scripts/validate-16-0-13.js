...
assert(pkg.version === '16.0.13', 'package.json version is 16.0.13');
assert(lock.version === '16.0.13' && lock.packages?.['']?.version === '16.0.13', 'package-lock.json version is 16.0.13');
...
assert(!lockText.includes('@types/yargs/-/yargs-16.0.13.tgz'), 'package-lock does not point to nonexistent @types/yargs 16.0.13 tarball');
...
assert(version.version === '16.0.13' && version.build === '16.0.13', 'public/version.json is 16.0.13');
assert(appCore.includes("CURRENT_VERSION = '16.0.13'"), 'CURRENT_VERSION is 16.0.13');
assert(pkg.scripts?.test === 'node scripts/validate-16-0-13.js', 'npm test runs 16.0.13 validator');
assert(pkg.scripts?.['test:ci'] === 'node scripts/validate-16-0-13.js', 'npm test:ci runs 16.0.13 validator');
...
assert(styles.includes('Schedule Builder event visibility row'), '16.0.13 event visibility CSS note is present');
...
if (failures) {
  console.error(`16.0.13 vercel lockfile deploy validator failed with ${failures} issue(s).`);
  process.exit(1);
}
console.log('16.0.13 vercel lockfile deploy validator passed.');
