...
assert.equal(version.releaseTitle, 'Exact Concept 1 System Administrator Home');
assert.equal(pkg.scripts['test:source'], 'node scripts/validate-17-0-33.js');
assert.equal(pkg.scripts['validate:17.0.33'], 'node scripts/validate-17-0-33.js');
assert(pkg.scripts['test:repair:17.0.33']?.includes('test:current-release-targeted'));
...
assert(pkg.scripts['test:release:fast']?.includes('validate:17.0.33'));
...
assert(scope.includes("const CURRENT_RELEASE_VERSION = '17.0.33'"));
...
assert(read('tests/86chaos-new-implementations/10-app-bootstrap-i18n-runtime.spec.cjs').includes('Version 17\\.0\\.33'));
...
]) assert(read(file).includes('17.0.33'), `${file} carries current version 17.0.33`);
for (const file of [
...
console.log('17.0.33 Exact Concept 1 System Administrator Home validation passed; this does not certify the release.');
