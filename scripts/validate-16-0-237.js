...
const inherited = childProcess.spawnSync(process.execPath, ['scripts/validate-16-0-233.js'], {
  ...
  env: {
    ...process.env,
    CHAOS_VALIDATION_VERSION: '16.0.237',
    CHAOS_VALIDATION_RELEASE_TITLE: 'Native Backup Watchdog Robustness and Deployment Identity Repair',
...
assert(pkg.version === '16.0.237' && lock.version === '16.0.237' && lock.packages?.['']?.version === '16.0.237', 'package and lock versions are 16.0.237');
assert(version.version === '16.0.237' && version.build === '16.0.237' && version.releaseTitle === 'Native Backup Watchdog Robustness and Deployment Identity Repair', 'public version metadata identifies the watchdog and deployment identity repair');
assert(read('api/_version.js').includes("APP_VERSION = '16.0.237'") && read('api/_version.js').includes("SECURITY_SCHEMA_VERSION = '16.0.237'"), 'API reports 16.0.237');
assert(read('src/core/appCore.js').includes("CURRENT_VERSION = '16.0.237'"), 'active client reports 16.0.237');
assert(read('src/core/customerHelpKnowledge.js').includes("CUSTOMER_HELP_VERSION = '16.0.237'") && read('src/core/customerHelpKnowledge.cjs').includes("CUSTOMER_HELP_VERSION = '16.0.237'"), 'customer Help version mirrors remain synchronized');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-237.js' && pkg.scripts['validate:16.0.237'] === 'node scripts/validate-16-0-237.js', '16.0.237 validator is wired');
...
assert(pkg.scripts['test:new-implementations']?.startsWith('npm run validate:16.0.237'), 'new-implementation command uses the 16.0.237 validator');
...
if (failures) { console.error(`16.0.237 source validation failed with ${failures} failure(s).`); process.exit(1); }
console.log('16.0.237 source validation passed.');
