...
assert(pkg.version === '16.0.16', 'package.json version is 16.0.16');
assert(lock.version === '16.0.16' && lock.packages?.['']?.version === '16.0.16', 'package-lock.json version is 16.0.16');
...
assert(!lockText.includes('@types/yargs/-/yargs-16.0.16.tgz'), 'package-lock does not point to nonexistent @types/yargs 16.0.16 tarball');
...
assert(version.version === '16.0.16' && version.build === '16.0.16', 'public/version.json is 16.0.16');
assert(appCore.includes("CURRENT_VERSION = '16.0.16'"), 'CURRENT_VERSION is 16.0.16');
assert(pkg.scripts?.test === 'node scripts/validate-16-0-16.js', 'npm test runs 16.0.16 validator');
assert(pkg.scripts?.['test:ci'] === 'node scripts/validate-16-0-16.js', 'npm test:ci runs 16.0.16 validator');



// 16.0.16: Schedule hours math must understand AM/PM display strings and ignore exact duplicate shift docs.
...
assert(schedule.includes('const dayRawShifts = userShifts.filter(s => getScheduleShiftDateKey(s) === d);'), 'Scheduled Hours Tracker groups raw shifts by day before summing unique intervals');
...
// 16.0.16: Scheduled Hours Tracker must use merged per-day time intervals, not raw document sums.
...
assert(schedule.includes('} else {\n        return null;\n      }'), 'impossible end-before-start shifts are rejected instead of padded by 24 hours');
...
assert(schedule.includes('const dayRawShifts = userShifts.filter(s => getScheduleShiftDateKey(s) === d);'), 'Scheduled Hours Tracker groups raw shifts by day before math');
assert(schedule.includes('return sum + getUniqueScheduledHoursForShifts(dayRawShifts);'), 'Scheduled Hours Tracker sums merged daily intervals');
...
if (failures) {
  console.error(`16.0.16 schedule hours math validator failed with ${failures} issue(s).`);
  process.exit(1);
}
console.log('16.0.16 schedule hours math validator passed.');
