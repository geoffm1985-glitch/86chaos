...
assert(pkg.version === '16.0.57', 'package.json version is 16.0.57');
assert(lock.version === '16.0.57' && lock.packages?.['']?.version === '16.0.57', 'package-lock.json version is 16.0.57');
assert(version.version === '16.0.57' && version.build === '16.0.57', 'public/version.json is 16.0.57');
assert(appCore.includes("CURRENT_VERSION = '16.0.57'"), 'CURRENT_VERSION is 16.0.57');
assert(apiVersion.includes("APP_VERSION = '16.0.57'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.57'"), 'API version constants are centralized at 16.0.57');
assert(pkg.scripts?.['test:source'] === 'node scripts/validate-16-0-57.js', 'package scripts point at the 16.0.57 source validator');
...
assert((styles.includes('16.0.51 mobile Schedule Builder readability') || styles.includes('16.0.57 failed-only release-gate repairs')) && styles.includes('scheduled-hours-tracker-table'), 'mobile schedule times and hours tracker labels stay readable and horizontal');
...
assert(!appJs.includes('fcmTokens: arrayUnion(currentToken)') && appJs.includes('primaryTokenMissing') && appJs.includes("pushTokenDedupeVersion: '16.0.57'") && appJs.includes('writePushProfilePatch'), 'push token sync repairs the primary token and pushDevices without writing the disallowed browser fcmTokens array');
...
assert(exists('scripts/test-16-0-54-targeted.cjs'), 'targeted 16.0.57 fix test exists');

// 16.0.57 temporary release-gate account provisioning guardrails
try {
  ...
  failures += 1;
  console.error('16.0.57 temporary-account guard failed:', error.message);
}


if (failures) {
  console.error(`16.0.57 source validator failed with ${failures} issue(s).`);
  process.exit(1);
}


// 16.0.57 Schedule Builder delete visibility guardrails
try {
  const schedule = read('src/features/schedule.jsx');
  ...
  failures += 1;
  console.error('16.0.57 schedule delete guard failed:', error.message);
}

console.log('16.0.57 source validator passed. This is a source guard only. It does not replace unit, emulator, build, browser, or staging notification tests.');

// 16.0.57 specific guardrails
try {
  const app = require('fs').readFileSync('src/App.js', 'utf8');
  if (/\[activeTabState,\s*isMenuOpen,\s*isWorkspaceSwitcherOpen\]/.test(app)) {
    console.error('16.0.57 guard failed: accessibility observer must not reference menu state before hook initialization.');
    process.exitCode = 1;
  }
  const sw = require('fs').readFileSync('public/firebase-messaging-sw.js', 'utf8');
  if (!sw.includes('/notification-badge.png')) {
    console.error('16.0.57 guard failed: notification badge fallback missing.');
    process.exitCode = 1;
  }
} catch (error) {
  console.error('16.0.57 guard failed:', error.message);
  process.exitCode = 1;
}


// 16.0.57 Schedule Hours / global-setup seed-auth guardrails
try {
  const schedule = read('src/features/schedule.jsx');
