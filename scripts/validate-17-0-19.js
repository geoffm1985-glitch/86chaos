...
const expected = '17.0.19';
const releaseTitle = 'Release Gate Certification Integrity, Full-Universe, and Resume Repair';
...
for (const file of ['src/core/appCore.js', 'api/_version.js', 'api/_pos-bridge-config.js']) assert(read(file).includes("'17.0.19'"), `${file} current release metadata`);
for (const directory of ['src', 'api', 'scripts', 'test-tools', 'tests']) assert(fs.statSync(directory).isDirectory(), `${directory} is required`);
for (const file of ['firebase.json', 'vercel.json', 'firestore.rules', 'storage.rules', 'RUN_86CHAOS_PLAY_STORE_RELEASE_GATE.ps1', 'RUN_86CHAOS_UPDATE_TEST_DEPLOY.ps1', 'RELEASE_TEST_POLICY.md', 'RELEASE_17_0_19.md', '.gitignore', 'release-source-manifest.json']) assert(fs.statSync(file).isFile(), `${file} is required`);
...
assert(updater.includes("[string]$ExpectedVersion = '17.0.19'"));
...
assert(pkg.scripts['test:repair:17.0.19'].includes('api/release-gate-maturity-16-0-207.test.cjs'));
assert(pkg.scripts['test:repair:17.0.19'].includes('api/release-gate-certification-integrity-17-0-19.test.cjs'));
assert(pkg.scripts['test:release:fast'].includes('validate:17.0.19'));
assert(pkg.scripts['test:new-implementations'].includes('validate:17.0.19'));
assert(json('test-tools/regressions/registry.json').defects.some(row => row.defectId === 'RG-CERTIFICATION-INTEGRITY-FULL-UNIVERSE-1719' && row.fixedVersion === expected));
...
console.log('17.0.19 source validation passed; this does not certify the release.');
