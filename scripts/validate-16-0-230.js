...
assert(pkg.version === '16.0.230', 'package.json version is 16.0.230');
assert(lock.version === '16.0.230' && lock.packages?.['']?.version === '16.0.230', 'package-lock root versions are 16.0.230');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-230.js', 'test:source points to the 16.0.230 validator');
assert(version.version === '16.0.230' && version.build === '16.0.230', 'public version/build are 16.0.230');
assert(version.releaseTitle === 'Release Gate Expected Skip Repair', 'release title identifies the expected-skip report repair');
assert(apiVersion.includes("APP_VERSION = '16.0.230'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.230'"), 'API reports 16.0.230');
assert(appCore.includes("CURRENT_VERSION = '16.0.230'"), 'app reports 16.0.230');
...
if (failures) {
  console.error(`16.0.230 source validation failed with ${failures} failure(s).`);
  process.exit(1);
}
console.log('16.0.230 source validation passed.');
