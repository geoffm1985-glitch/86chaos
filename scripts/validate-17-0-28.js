...
assert.equal(pkg.scripts['validate:17.0.28'], 'node scripts/validate-17-0-28.js');
assert(pkg.scripts['test:repair:17.0.28']?.includes('test:current-release-targeted'));
...
assert(scope.includes("const CURRENT_RELEASE_VERSION = '17.0.28'"));
...
assert(browserSmoke.includes('Version 17\\.0\\.28'));
...
]) assert(read(file).includes('17.0.28'), `${file} carries current version 17.0.28`);
for (const file of [
...
console.log('17.0.28 Browser-Safe Spanish Runtime Emergency Repair validation passed; this does not certify the release.');
