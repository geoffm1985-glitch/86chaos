...
const assert = (condition, message) => {
  if (!condition) {
    console.error(`16.0.70 source validation failed: ${message}`);
    process.exitCode = 1;
  }
};
...
assert(pkg.version === '16.0.70', 'package.json version is 16.0.70');
assert(lock.version === '16.0.70' && lock.packages?.['']?.version === '16.0.70', 'package-lock root version is 16.0.70');
assert(version.version === '16.0.70' && version.build === '16.0.70', 'public version/build is 16.0.70');
assert(appCore.includes("CURRENT_VERSION = '16.0.70'"), 'appCore CURRENT_VERSION is 16.0.70');
assert(apiVersion.includes("APP_VERSION = '16.0.70'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.70'"), 'API version constants are 16.0.70');
assert(pkg.scripts?.['test:source'] === 'node scripts/validate-16-0-70.js', 'test:source points at 16.0.70 validator');
...
assert(schedule.includes('isScheduleShiftPublished(s) && isShiftStillCurrentOrUpcoming'), 'My Schedule uses flexible published detection for employee-facing shifts');
...
if (!process.exitCode) console.log('16.0.70 source validator passed.');
