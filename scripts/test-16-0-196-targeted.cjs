...
const commands = [
  ['node', 'scripts/validate-16-0-196.js'],
  ['node', '--test', 'api/schedule-query-planner-client-test-16-0-196.test.cjs'],
  ['node', '--test', 'api/page-load-readiness-16-0-196.test.cjs'],
  ...
  ['node', '--test', 'api/app-only-package-hygiene-16-0-196.test.cjs']
];
for (const cmd of commands) {
  console.log(`[16.0.196 targeted] ${cmd.join(' ')}`);
  const childEnv = { ...process.env };
  if (cmd.includes('api/app-only-package-hygiene-16-0-196.test.cjs')) childEnv.CHAOS_ALLOW_GENERATED_TEST_ARTIFACTS = '1';
  const result = spawnSync(cmd[0], cmd.slice(1), { stdio: 'inherit', shell: process.platform === 'win32', env: childEnv });
  if (result.status !== 0) process.exit(result.status || 1);
}
console.log('16.0.196 targeted repair checks passed.');
