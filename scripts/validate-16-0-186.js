...
assert(pkg.version === '16.0.186', 'package.json version is 16.0.186');
assert(lock.version === '16.0.186' && lock.packages?.['']?.version === '16.0.186', 'package-lock root versions are 16.0.186');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-186.js', 'test:source points to 16.0.186 validator');
assert(version.version === '16.0.186' && version.build === '16.0.186', 'public/version.json version/build are 16.0.186');
assert(version.releaseTitle === 'Firebase Read/Write Efficiency and Presence Maturity Completion', 'release title is correct');
assert(appCore.includes("CURRENT_VERSION = '16.0.186'"), 'app core CURRENT_VERSION is 16.0.186');
assert(apiVersion.includes("APP_VERSION = '16.0.186'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.186'"), 'api version reports 16.0.186');
...
assert(pkg.scripts?.['vercel:install'] === 'npm ci --omit=dev --no-audit --no-fund --progress=false', 'package Vercel install keeps npm ci and required optional native bindings');
assert(vercel.includes('"installCommand": "npm ci --omit=dev --no-audit --no-fund --progress=false"'), 'vercel.json install keeps npm ci and required optional native bindings');
assert(!vercel.includes('--omit=optional'), 'Vercel install does not omit required optional native bindings');
...
assert(fs.existsSync(path.join(root, 'scripts/repair-regression-pack-16.0.186.json')), 'repair regression pack manifest exists');
assert(fs.existsSync(path.join(root, 'scripts/86chaos-release-gate/repair-regression-16.0.186.json')), 'repair browser regression manifest exists');
...
const repairManifest = json('scripts/86chaos-release-gate/repair-regression-16.0.186.json');
...
assert(fs.existsSync(path.join(root, 'scripts/repair-regression-pack-16.0.186.json')), '16.0.186 repair local manifest exists');
assert(fs.existsSync(path.join(root, 'scripts/86chaos-release-gate/repair-regression-16.0.186.json')), '16.0.186 repair browser manifest exists');
assert(read('scripts/run-repair-regression-pack.cjs').includes('repair-regression-pack-16.0.186.json'), 'repair pack runner points to 16.0.186 manifest');
assert(read('scripts/run-repair-browser-regression.cjs').includes('repair-regression-16.0.186.json'), 'repair browser runner points to 16.0.186 manifest');
const repairLocal172 = json('scripts/repair-regression-pack-16.0.186.json');
assert(repairLocal172.version === '16.0.186', 'repair local manifest version is 16.0.186');
assert((repairLocal172.localCommands || []).some((entry) => entry.group === 'Current Source Validator' && Array.isArray(entry.cmd) && entry.cmd.includes('scripts/validate-16-0-186.js')), 'repair local current source validator points to 16.0.186');
...
const repairBrowser172 = json('scripts/86chaos-release-gate/repair-regression-16.0.186.json');
assert(repairBrowser172.version === '16.0.186', 'repair browser manifest version is 16.0.186');
...
// 16.0.186 staff login email edit and full-suite reporting repair.
...
const repair173 = json('scripts/repair-regression-pack-16.0.186.json');
assert((repair173.localCommands || []).some(entry => entry.group === 'Staff Email Editing'), '16.0.186 repair local manifest includes Staff Email Editing');
const repairBrowser173 = json('scripts/86chaos-release-gate/repair-regression-16.0.186.json');
assert((repairBrowser173.selected || []).some(row => row.specPath === 'tests/e2e/staff-email-edit.spec.cjs' && row.leafTitle === 'Manager changes an employee login email and the new email authenticates' && row.project === 'chromium'), '16.0.186 repair browser manifest includes staff email chromium identity');
assert((repairBrowser173.selected || []).some(row => row.specPath === 'tests/e2e/staff-email-edit.spec.cjs' && row.project === 'mobile-chromium'), '16.0.186 repair browser manifest includes staff email mobile identity');
...
assert(fullSuite.includes('86chaos-FULL-SUITE-UPLOAD-ME-16.0.186'), 'full-suite upload ZIP is versioned and finalized');
assert(pkg.scripts['test:full-suite'] && pkg.scripts['test:full-suite:local'], 'full-suite package scripts exist');
assert(pkg.scripts['test:repair-16-0-183'] === 'npm run test:repair-current', 'test:repair-16-0-183 package script exists');
assert(fs.existsSync(path.join(root, 'scripts/test-16-0-186-targeted.cjs')), '16.0.186 targeted test script exists');



// 16.0.186 zero-fail orchestration/resilience/source contracts.
...
assert(fs.existsSync(path.join(root, 'api/release-integrity-account-lifecycle-16-0-177.test.cjs')), '16.0.186 release-integrity/account-lifecycle regression exists');
...
assert(fs.existsSync(path.join(root, 'api/vite-lock-integrity-16-0-180.test.cjs')), 'historical Vite lock integrity regression remains present');
assert(fs.existsSync(path.join(root, 'api/vercel-vite-native-binding-16-0-181.test.cjs')), 'historical Vercel native-binding install regression remains present');
...
// 16.0.186 ESLint ESM API parser-scope repair.
...
assert(fs.existsSync(path.join(root, 'api/lint-esm-api-16-0-179.test.cjs')), '16.0.186 ESM API lint regression exists');


// 16.0.186 Vite JSX-in-.js compatibility repair.
const viteConfig182 = read('vite.config.js');
const viteJsxHelper182 = read('scripts/vite-react-jsx-module-type.cjs');
assert(fs.existsSync(path.join(root, 'scripts/vite-react-jsx-module-type.cjs')), 'Vite JSX module-type helper exists');
assert(fs.existsSync(path.join(root, 'api/vite-jsx-module-type-16-0-183.test.cjs')), 'Vite JSX compatibility regression exists');
assert(viteConfig182.includes("require('./scripts/vite-react-jsx-module-type.cjs')"), 'Vite config loads JSX module-type helper');
assert(viteConfig182.includes('plugins: [createReactJsxModuleTypePlugin(), react()]'), 'JSX module-type compatibility plugin runs before React plugin');
assert(viteJsxHelper182.includes("moduleType: 'jsx'") && viteJsxHelper182.includes("enforce: 'pre'"), 'React source .js files are classified as JSX before Vite core transforms');
assert(viteJsxHelper182.includes("src\\/.*\\.js"), 'JSX compatibility remains scoped to src .js files');

if (failures) { console.error(`\n${failures} validation check(s) failed.`); process.exit(1); }

// 16.0.186 npm registry lock metadata repair.
const registryLockTest = 'api/npm-registry-lock-metadata-16-0-183.test.cjs';
assert(fs.existsSync(path.join(root, registryLockTest)), '16.0.186 npm registry lock metadata regression exists');
const undiciLock = lock.packages?.['node_modules/undici'];
assert(undiciLock?.version === '6.28.0', 'undici is locked to published security-fixed 6.28.0');
assert(undiciLock?.resolved === 'https://registry.npmjs.org/undici/-/undici-6.28.0.tgz', 'undici tarball URL is published 6.28.0');
assert(undiciLock?.integrity === 'sha512-LIY910g9TI13YS95lrMFrs8Rm/u/irgHeTWoKCoteeJ04CUJ92eEfj0rVn+7VKMPBpUPiUoBKfhNyLI23EE/KA==', 'undici integrity metadata matches published 6.28.0');
assert(!JSON.stringify(lock).includes('undici-6.27.1.tgz'), 'phantom undici 6.27.1 tarball is absent');
assert(pkg.scripts['test:repair-16-0-183'] === 'npm run test:repair-current', 'test:repair-16-0-183 package script exists');


// 16.0.186 Vercel Linux Vite/Oxc build precompile repair.
const viteBuildRunner184 = read('scripts/vite-build-with-asset-manifest.cjs');
const vitePrecompile184 = read('scripts/vite-build-source-precompile.cjs');
assert(fs.existsSync(path.join(root, 'scripts/vite-build-source-precompile.cjs')), 'Vite Oxc source precompile helper exists');
assert(fs.existsSync(path.join(root, 'api/vite-oxc-precompile-build-16-0-184.test.cjs')), 'Vite Oxc precompile regression exists');
assert(viteBuildRunner184.includes("await import('vite')") && viteBuildRunner184.includes('transformWithOxc'), 'production build imports Vite Oxc transformer directly');
assert(viteBuildRunner184.includes('createBuildWorkspace(root)') && viteBuildRunner184.includes('cleanupBuildWorkspace(workspaceRoot)'), 'production build uses disposable source workspace with cleanup');
assert(viteBuildRunner184.includes("'--config', path.join(root, 'vite.config.js')"), 'temporary build uses the real Vite project config');
assert(vitePrecompile184.includes("runtime: 'automatic'") && vitePrecompile184.includes("importSource: 'react'"), 'temporary src .js compilation uses React automatic JSX runtime');
assert(read('.gitignore').includes('.vite-build-root-*/'), 'temporary Vite build workspaces are ignored by Git');
assert(pkg.scripts['test:repair-16-0-186'] === 'npm run test:repair-current', 'test:repair-16-0-186 package script exists');



// 16.0.186 release-gate dependency-preflight false-negative repair.
const dependencyPreflight = read('scripts/86chaos-release-gate/dependency-preflight.cjs');
const collectorReport = read('scripts/86chaos-release-gate/collect-release-gate-report.cjs');
assert(dependencyPreflight.includes('require.resolve(name, { paths: [root] })'), 'dependency preflight resolves package exported entry point as install proof');
assert(!dependencyPreflight.includes('require.resolve(`${name}/package.json`'), 'dependency preflight no longer uses package.json subpath as presence test');
assert(dependencyPreflight.includes('packageRootFromNodeModulesPath') && dependencyPreflight.includes('findPhysicalPackageJson'), 'dependency preflight reads physical metadata after entry resolution');
assert(dependencyPreflight.includes('ERR_PACKAGE_PATH_NOT_EXPORTED') === false, 'dependency preflight is not hard-coded to one package export error');
assert(dependencyPreflight.includes('entryPath') && dependencyPreflight.includes('resolutionMethod') && dependencyPreflight.includes('errorCode'), 'dependency preflight report records entry path, resolution method, and error code');
assert(fs.existsSync(path.join(root, 'api/release-gate-dependency-preflight-16-0-185.test.cjs')), '16.0.185 release-gate dependency preflight regression remains');
assert(fs.existsSync(path.join(root, 'api/firebase-efficiency-presence-16-0-186.test.cjs')), '16.0.186 Firebase efficiency/presence regression exists');
const appCore186 = read('src/core/appCore.js');
const reminderQueries186 = read('src/core/personalReminderQueries.js');
const inventory186 = read('src/features/inventory.jsx');
const hr186 = read('src/features/hr.jsx');
const management186 = read('src/features/management.jsx');
assert(appCore186.includes("LOW_COST_PRESENCE_TREE = 'presence'") && !appCore186.includes('statusSummary/${workspaceKey}/${userKey}'), 'presence writes use one device presence tree instead of duplicate statusSummary writes');
assert(appCore186.includes('deriveLowCostPresenceRowsFromDeviceTree'), 'presence summary aggregates multiple device records');
assert(appCore186.includes('liveCollectionReleaseGraceMs') && appCore186.includes('HOT_LIVE_COLLECTION_RELEASE_GRACE_MS'), 'shared Firestore listener release policy is adaptive');
assert(reminderQueries186.includes('7 * 60 * 1000') && !reminderQueries186.includes('setInterval('), 'personal reminders use stale refresh instead of 90-second polling');
assert(inventory186.includes('parDrafts') && inventory186.includes('onBlur={(e) => commitParDraft(item, e.target.value)}'), 'inventory PAR typing is local until commit');
assert(hr186.includes('hrOverviewActive') && hr186.includes("activeTab === 'performance'"), 'HR large datasets are gated by active subtab');
assert(management186.includes('Snapshot-based view') && management186.includes('Load More') && !management186.includes("const logs = useLiveCollection('auditLogs'"), 'customer audit log uses bounded snapshot/load-more instead of permanent listener');
assert(collectorReport.includes('function releaseGateRerunCommandForMode') && collectorReport.includes("return 'npm run test:play-store';"), 'blocked full-mode report suggests npm run test:play-store');
assert(!collectorReport.includes("failed-only' ? 'npm run test:play-store:failed' : 'npm run test:play-store:delta"), 'blocked full-mode report no longer falls through to delta command');

console.log('\n16.0.186 source validation passed.');
