...
assert(pkg.version === '16.0.205', 'package.json version is 16.0.205');
assert(lock.version === '16.0.205' && lock.packages?.['']?.version === '16.0.205', 'package-lock root versions are 16.0.205');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-205.js', 'test:source points to 16.0.205 validator');
assert(version.version === '16.0.205' && version.build === '16.0.205', 'public/version.json version/build are 16.0.205');
assert(version.releaseTitle === 'Failed-Only Manifest Selection Guard Repair', '16.0.205 release title identifies failed-only manifest selection guard repair');
assert(appCore.includes("CURRENT_VERSION = '16.0.205'"), 'app core CURRENT_VERSION is 16.0.205');
assert(apiVersion.includes("APP_VERSION = '16.0.205'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.205'"), 'api version reports 16.0.205');
...
if (failures) {
  console.error(`16.0.205 source validation failed with ${failures} failure(s).`);
  process.exit(1);
}
console.log('16.0.205 source validation passed.');
