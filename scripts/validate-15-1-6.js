...
const checks = [
  ['package.json', /"version"\s*:\s*"15\.1\.6"/, 'package.json version 15.1.6'],
  ['package.json', /"test"\s*:\s*"node scripts\/validate-15-1-6\.js"/, 'npm test points to current validator'],
  ['public/version.json', /15\.1\.6/, 'public version 15.1.6'],
  ['src/core/appCore.js', /CURRENT_VERSION\s*=\s*'15\.1\.6'/, 'runtime version 15.1.6'],
...
if (!exists('README_15_1_6_RELEASE_NOTES.md')) { console.error('FAIL release notes missing'); failed = true; } else console.log('OK release notes created');
if (!exists('QA_15_1_6_RUNTIME_SIDEBAR_ICON_FIX.md')) { console.error('FAIL QA checklist missing'); failed = true; } else console.log('OK QA checklist created');
...
console.log('15.1.6 Manager Brain Control Panel validation passed.');
