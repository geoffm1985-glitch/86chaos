...
assert.equal(pkg.version, '17.1.5');
assert.equal(lock.version, pkg.version);
assert.equal(lock.packages[''].version, pkg.version);
assert.equal(version.version, pkg.version);
assert.equal(version.build, pkg.version);
assert.equal(version.releaseTitle, 'Mobile Bottom Toolbar Single-Line Label Repair');
assert.equal(pkg.scripts['test:source'], 'node scripts/validate-17-1-5.js');
assert.equal(pkg.scripts['validate:17.1.5'], 'node scripts/validate-17-1-5.js');
assert(pkg.scripts['test:repair:17.1.5']?.includes('test:current-release-targeted'));
...
assert(pkg.scripts['test:release:fast']?.includes('validate:17.1.5')); 
...
assert(scope.includes("const CURRENT_RELEASE_VERSION = '17.1.5'"));
...
]) assert(read(file).includes('17.1.5'), `${file} carries current version 17.1.5`);
for (const file of [
...
console.log('17.1.5 Mobile Bottom Toolbar Single-Line Label Repair validation passed; this does not certify the release.');
