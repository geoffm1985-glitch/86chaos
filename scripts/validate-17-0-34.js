...
assert.equal(version.releaseTitle, 'System Administrator Subpage Unification and PWA Back Navigation');
...
assert(pkg.scripts['test:repair:17.0.34']?.includes('test:current-release-targeted'));
...
assert(pkg.scripts['test:release:fast']?.includes('validate:17.0.34'));
...
assert(scope.includes("const CURRENT_RELEASE_VERSION = '17.0.34'"));
...
assert(management.includes('data-testid="system-admin-concept1-card"'));
...
assert(read('tests/86chaos-new-implementations/10-app-bootstrap-i18n-runtime.spec.cjs').includes('Version 17\\.0\\.34'));
...
]) assert(read(file).includes('17.0.34'), `${file} carries current version 17.0.34`);
for (const file of [
...
console.log('17.0.34 System Administrator Subpage Unification and PWA Back Navigation validation passed; this does not certify the release.');
