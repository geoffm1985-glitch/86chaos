...
const repairPack = json('scripts/repair-regression-pack-16.0.187.json');
const browserPack = json('scripts/86chaos-release-gate/repair-regression-16.0.187.json');

assert(pkg.version === '16.0.187', 'package.json version is 16.0.187');
assert(lock.version === '16.0.187' && lock.packages?.['']?.version === '16.0.187', 'package-lock root version is 16.0.187');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-187.js', 'test:source points to 16.0.187 validator');
assert(pkg.scripts['test:repair-16-0-187'] === 'npm run test:repair-current', 'test:repair-16-0-187 exists');
assert(version.version === '16.0.187' && version.build === '16.0.187', 'public version/build are 16.0.187');
assert(/CURRENT_VERSION = '16\.0\.187'/.test(appCore), 'app core CURRENT_VERSION is 16.0.187');
assert(/APP_VERSION = '16\.0\.187'/.test(read('api/_version.js')), 'api version is 16.0.187');
assert(read('RUN_86CHAOS_FULL_TEST_SUITE.ps1').includes('test-16-0-187-targeted.cjs'), 'full suite uses 16.0.187 targeted validator');
...
assert(intelligence.includes('dependencyGraphOpen') && intelligence.includes('scanPageLimit'), 'Menu Intelligence dependency graph and scan history are lazy/bounded');
...
assert(exists('api/firebase-efficiency-completion-16-0-187.test.cjs'), '16.0.187 Firebase efficiency completion regression exists');
assert(exists('scripts/test-16-0-187-targeted.cjs'), '16.0.187 targeted test script exists');
assert(repairPack.version === '16.0.187', 'repair local manifest version is 16.0.187');
assert(browserPack.version === '16.0.187', 'repair browser manifest version is 16.0.187');
assert(read('scripts/run-repair-regression-pack.cjs').includes('repair-regression-pack-16.0.187.json'), 'repair local pointer uses 16.0.187');
assert(read('scripts/run-repair-browser-regression.cjs').includes('repair-regression-16.0.187.json'), 'repair browser pointer uses 16.0.187');

if (failures) { console.error(`\n16.0.187 source validation failed with ${failures} issue(s).`); process.exit(1); }
