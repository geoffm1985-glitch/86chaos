...
assert(pkg.version === '16.0.38', 'package.json version is 16.0.38');
assert(lock.version === '16.0.38' && lock.packages?.['']?.version === '16.0.38', 'package-lock.json version is 16.0.38');
assert(version.version === '16.0.38' && version.build === '16.0.38', 'public/version.json is 16.0.38');
assert(appCore.includes("CURRENT_VERSION = '16.0.38'"), 'CURRENT_VERSION is 16.0.38');
assert(apiVersion.includes("APP_VERSION = '16.0.38'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.38'"), 'API version constants are centralized at 16.0.38');
assert(pkg.scripts?.['test:source'] === 'node scripts/validate-16-0-38.js', 'package scripts point at the 16.0.38 source validator');
...
if (failures) {
  console.error(`16.0.38 source validator failed with ${failures} issue(s).`);
  process.exit(1);
}
console.log('16.0.38 source validator passed. This is a source guard only. It does not replace unit, emulator, build, browser, or staging notification tests.');
