...
assert.equal(pkg.version, '17.1.1');
...
assert.equal(pkg.scripts['test:source'], 'node scripts/validate-17-1-1.js');
assert.equal(pkg.scripts['validate:17.1.1'], 'node scripts/validate-17-1-1.js');
assert(pkg.scripts['test:repair:17.1.1']?.includes('test:current-release-targeted'));
...
assert(pkg.scripts['test:release:fast']?.includes('validate:17.1.1'));
...
assert(conceptCss.includes('17.1.1 deep-page migration'));
...
assert(scope.includes("const CURRENT_RELEASE_VERSION = '17.1.1'"));
...
]) assert(read(file).includes('17.1.1'), `${file} carries current version 17.1.1`);
for (const file of [
...
console.log('17.1.1 Full-App Concept 1 Deep Workflow Redesign validation passed; this does not certify the release.');
