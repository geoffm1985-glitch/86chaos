...
assert.equal(version.releaseTitle, 'Automatic ZIP Extraction and Damaged Checkout Self-Heal Repair');
for (const file of ['src/core/appCore.js', 'api/_version.js', 'api/_pos-bridge-config.js']) assert(read(file).includes("'17.0.15'"));
for (const directory of ['src', 'api', 'scripts', 'test-tools', 'tests']) assert(fs.statSync(directory).isDirectory(), `${directory} is required`);
for (const file of ['firebase.json', 'vercel.json', 'firestore.rules', 'storage.rules', 'RUN_86CHAOS_PLAY_STORE_RELEASE_GATE.ps1', 'RUN_86CHAOS_UPDATE_TEST_DEPLOY.ps1', 'RELEASE_TEST_POLICY.md', 'RELEASE_17_0_15.md', '.gitignore', 'release-source-manifest.json']) assert(fs.statSync(file).isFile(), `${file} is required`);
...
assert(updater.includes("[string]$ExpectedVersion = '17.0.15'"));
...
assert(pkg.scripts['test:repair:17.0.15'].includes('api/release-workflow-self-heal-17-0-15.test.cjs'));
assert(pkg.scripts['test:release:fast'].includes('validate:17.0.15'));
assert(pkg.scripts['test:new-implementations'].includes('validate:17.0.15'));
...
console.log('17.0.15 source validation passed; this does not certify the release.');
