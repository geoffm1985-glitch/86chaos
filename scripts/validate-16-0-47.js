...
assert(pkg.version === '16.0.47', 'package.json version is 16.0.47');
assert(lock.version === '16.0.47' && lock.packages?.['']?.version === '16.0.47', 'package-lock.json version is 16.0.47');
assert(version.version === '16.0.47' && version.build === '16.0.47', 'public/version.json is 16.0.47');
assert(appCore.includes("CURRENT_VERSION = '16.0.47'"), 'CURRENT_VERSION is 16.0.47');
assert(apiVersion.includes("APP_VERSION = '16.0.47'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.47'"), 'API version constants are centralized at 16.0.47');
assert(pkg.scripts?.['test:source'] === 'node scripts/validate-16-0-47.js', 'package scripts point at the 16.0.47 source validator');
...
assert(styles.includes('16.0.47 release-gate accessibility') && appJs.includes('describeControl'), 'accessibility/tap-target hardening is present');

if (failures) {
  console.error(`16.0.47 source validator failed with ${failures} issue(s).`);
  process.exit(1);
}
console.log('16.0.47 source validator passed. This is a source guard only. It does not replace unit, emulator, build, browser, or staging notification tests.');
