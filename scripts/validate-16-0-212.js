...
assert(pkg.version === '16.0.212', 'package.json version is 16.0.212');
assert(lock.version === '16.0.212' && lock.packages?.['']?.version === '16.0.212', 'package-lock root versions are 16.0.212');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-212.js', 'test:source points to the 16.0.212 validator');
assert(version.version === '16.0.212' && version.build === '16.0.212', 'public version/build are 16.0.212');
assert(version.releaseTitle === 'Reminder Notification Delivery Repair', 'release title identifies the reminder repair');
assert(apiVersion.includes("APP_VERSION = '16.0.212'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.212'"), 'API reports 16.0.212');
assert(appCore.includes("CURRENT_VERSION = '16.0.212'"), 'app reports 16.0.212');
...
if (failures) {
  console.error(`16.0.212 source validation failed with ${failures} failure(s).`);
  process.exit(1);
}
console.log('16.0.212 source validation passed.');
