...
const assert = (condition, message) => {
  if (!condition) {
    console.error(`16.0.78 source validation failed: ${message}`);
    process.exitCode = 1;
  }
};
...
assert(pkg.version === '16.0.78', 'package.json version is 16.0.78');
assert(lock.version === '16.0.78' && lock.packages?.['']?.version === '16.0.78', 'package-lock root version is 16.0.78');
assert(pkg.scripts?.['test:source'] === 'node scripts/validate-16-0-78.js', 'test:source points at 16.0.78 validator');
assert(version.version === '16.0.78' && version.build === '16.0.78', 'public version/build is 16.0.78');
...
if (!process.exitCode) console.log('16.0.78 source validator passed.');
