...
assert.equal(pkg.version, '17.1.7');
assert.equal(lock.version, pkg.version);
assert.equal(lock.packages[''].version, pkg.version);
assert.equal(version.version, pkg.version);
assert.equal(version.build, pkg.version);
assert.equal(version.releaseTitle, '86Voice Mobile Toolbar Interaction Repair');
assert.equal(pkg.scripts['test:source'], 'node scripts/validate-17-1-7.js');
assert.equal(pkg.scripts['validate:17.1.7'], 'node scripts/validate-17-1-7.js');
assert(pkg.scripts['test:repair:17.1.7']?.includes('test:current-release-targeted'));
...
assert(pkg.scripts['test:release:fast']?.includes('validate:17.1.7')); 
...
assert(scope.includes("const CURRENT_RELEASE_VERSION = '17.1.7'"));
...
]) assert(read(file).includes('17.1.7'), `${file} carries current version 17.1.7`);
for (const file of [
...
console.log('17.1.7 86Voice Mobile Toolbar Interaction Repair validation passed; this does not certify the release.');
