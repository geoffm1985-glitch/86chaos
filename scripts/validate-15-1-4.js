...
const checks = [
  ['package.json', /"version"\s*:\s*"15\.1\.4"/, 'package.json version 15.1.4'],
  ['package.json', /"test"\s*:\s*"node scripts\/validate-15-1-4\.js"/, 'npm test points to current validator'],
  ['public/version.json', /15\.1\.4/, 'public version 15.1.4'],
  ['src/core/appCore.js', /CURRENT_VERSION\s*=\s*'15\.1\.4'/, 'runtime version 15.1.4'],
...
if (!exists('README_15_1_4_RELEASE_NOTES.md')) { console.error('FAIL release notes missing'); failed = true; } else console.log('OK release notes created');
if (!exists('QA_15_1_4_RUNTIME_SHELL_ICON_FIX.md')) { console.error('FAIL QA checklist missing'); failed = true; } else console.log('OK QA checklist created');
...
console.log('15.1.4 Manager Brain Control Panel validation passed.');
