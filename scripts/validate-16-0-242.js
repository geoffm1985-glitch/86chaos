...
const inherited=cp.spawnSync(process.execPath,['scripts/validate-16-0-233.js'],{
  ...
  env:{...process.env,CHAOS_VALIDATION_VERSION:'16.0.242',CHAOS_VALIDATION_RELEASE_TITLE:'Testing Alias Overlay Safety Repair',CHAOS_VALIDATION_SCRIPT:'scripts/validate-16-0-242.js'}
});
...
assert(pkg.version==='16.0.242'&&lock.version==='16.0.242'&&lock.packages?.['']?.version==='16.0.242','package and lock versions are 16.0.242');
assert(version.version==='16.0.242'&&version.build==='16.0.242'&&version.releaseTitle==='Testing Alias Overlay Safety Repair','public version metadata identifies the validator repair');
assert(read('api/_version.js').includes("APP_VERSION = '16.0.242'")&&read('api/_version.js').includes("SECURITY_SCHEMA_VERSION = '16.0.242'"),'API reports 16.0.242');
assert(read('src/core/appCore.js').includes("CURRENT_VERSION = '16.0.242'"),'active client reports 16.0.242');
assert(read('src/core/customerHelpKnowledge.js').includes("CUSTOMER_HELP_VERSION = '16.0.242'")&&read('src/core/customerHelpKnowledge.cjs').includes("CUSTOMER_HELP_VERSION = '16.0.242'"),'customer Help version mirrors remain synchronized');
assert(pkg.scripts['test:source']==='node scripts/validate-16-0-242.js'&&pkg.scripts['validate:16.0.242']==='node scripts/validate-16-0-242.js','16.0.242 validator is wired');
assert(pkg.scripts['test:new-implementations']?.startsWith('npm run validate:16.0.242'),'new-implementation command uses the 16.0.242 validator');
...
for(const host of ['testing.86chaos.com','experimental.86chaos.com']){
  const target=targets.validateReleaseTarget({appUrl:`https://${host}`,expectedProjectSlug:'86chaos',expectedVersion:'16.0.242',sourceVersion:'16.0.242',deployedVersion:'16.0.242'});
...
for(const host of ['app.86chaos.com','86chaos.com','www.86chaos.com','staging.86chaos.com']){
  const target=targets.validateReleaseTarget({appUrl:`https://${host}`,expectedVersion:'16.0.242',sourceVersion:'16.0.242',deployedVersion:'16.0.242'});
...
assert(diff.status===0,'Git diff evidence is available for the 16.0.242 repair');
...
if(failures){console.error(`16.0.242 source validation failed with ${failures} failure(s).`);process.exit(1);}
console.log('16.0.242 source validation passed.');
