...
assert(pkg.version === '16.0.198', 'package.json version is 16.0.198');
assert(lock.version === '16.0.198' && lock.packages?.['']?.version === '16.0.198', 'package-lock root versions are 16.0.198');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-198.js', 'test:source points to 16.0.198 validator');
assert(version.version === '16.0.198' && version.build === '16.0.198', 'public/version.json version/build are 16.0.198');
assert(version.releaseTitle === 'Actual Failed-Only Browser Gate App Fixes', '16.0.198 release title identifies actual failed-only browser gate app fixes');
assert(appCore.includes("CURRENT_VERSION = '16.0.198'"), 'app core CURRENT_VERSION is 16.0.198');
assert(apiVersion.includes("APP_VERSION = '16.0.198'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.198'"), 'api version reports 16.0.198');
...
assert(sinceRunner.includes('api/public-icon-assets-16-0-195.test.cjs'), 'targeted runner includes the public icon regression while 16.0.198 validates app-source failed-only repairs');
...
assert(exhaustiveHelper.includes('locatorFromFormDescriptor') && exhaustiveHelper.includes('restoreAndObserve(page,'), '16.0.198 uses stable form-control descriptors for failed-only form probes');
assert(managementFeature.includes('aria-label={tab.label}') && managementFeature.includes('title={tab.label}'), '16.0.198 exposes full System Administrator mobile tab labels to release-gate coverage');
assert(managementFeature.includes('aria-label={label} title={label} onClick={() => setSubTab(id)}'), '16.0.198 exposes Back Office nested tab labels to release-gate coverage');
assert(scheduleFeature.includes('aria-label={label} title={label} onClick={() => setActiveTool(id)}'), '16.0.198 exposes Schedule Builder nested tool labels to release-gate coverage');


assert(inventoryFeature.includes('const [parDrafts, setParDrafts] = useState({});') && inventoryFeature.includes('const [pendingQtyDrafts, setPendingQtyDrafts] = useState({});'), '16.0.198 keeps decimal inventory edits in local draft state while Firestore snapshots catch up');
assert(inventoryFeature.includes('const parseInventoryQuantity = (value, fallback = 0) =>') && inventoryFeature.includes('Number.parseFloat(cleaned)'), '16.0.198 uses a decimal-safe inventory quantity parser');
assert(inventoryFeature.includes('step="0.01" inputMode="decimal" aria-label={`Inventory par for') && inventoryFeature.includes('value={Object.prototype.hasOwnProperty.call(parDrafts, item.id)'), '16.0.198 makes inventory PAR inputs decimal-safe, locally controlled, and accessible');
assert(inventoryFeature.includes('step="0.01" inputMode="decimal" aria-label={`Order quantity for') && inventoryFeature.includes('parseInventoryQuantity(e.target.value, 0)'), '16.0.198 order quantity inputs accept decimal probe values instead of truncating them');
assert(inventoryFeature.includes('setPendingQtyDrafts') && inventoryFeature.includes('label: "Pending quantity", data: { pendingQty: parsed }'), '16.0.198 pending delivery quantity input uses the same decimal-safe path');
assert(!inventoryFeature.includes('setOrderOverrides(p=>({...p, [item.id]: parseInt(e.target.value)||0}))'), '16.0.198 removed parseInt truncation from order quantity controls');
assert(scheduleFeature.includes("'trade-board'") && scheduleFeature.includes("tab === 'trade-board' ? 'Trade Board'"), '16.0.198 exposes Trade Board in the persistent schedule subtab navigation');
assert(scheduleFeature.includes("openCopilotTool('template-editor')") && scheduleFeature.includes("openCopilotTool('drag')"), '16.0.198 exposes Create Template and Drag Board from the Schedule Copilot launcher');
assert(prepFeature.includes("const stateLabel = tab === 'prep' ? 'prep'") && prepFeature.includes('aria-label={stateLabel}'), '16.0.198 keeps Prep reachable by the declared /^prep$/ nested-state label');
assert(managementFeature.includes('flex flex-wrap gap-2 pb-2 border-b border-[#2A353D]') && managementFeature.includes('["quickbooks","QuickBooks"]'), '16.0.198 keeps QuickBooks visible in wrapped Back Office nested navigation');

if (failures) {
  console.error(`16.0.198 source validation failed with ${failures} failure(s).`);
  process.exit(1);
}
console.log('16.0.198 source validation passed.');
