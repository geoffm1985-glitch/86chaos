...
const checks = [
  ['package.json', /"version"\s*:\s*"15\.1\.3"/, 'package.json version 15.1.3'],
  ['package.json', /"test"\s*:\s*"node scripts\/validate-15-1-3\.js"/, 'npm test points to current validator'],
  ['public/version.json', /15\.1\.3/, 'public version 15.1.3'],
  ['src/core/appCore.js', /CURRENT_VERSION\s*=\s*'15\.1\.3'/, 'runtime version 15.1.3'],
...
if (!exists('README_15_1_3_RELEASE_NOTES.md')) { console.error('FAIL release notes missing'); failed = true; } else console.log('OK release notes created');
if (!exists('QA_15_1_3_RUNTIME_LOAD_RECOVERY.md')) { console.error('FAIL QA checklist missing'); failed = true; } else console.log('OK QA checklist created');
...
console.log('15.1.3 Manager Brain Control Panel validation passed.');
