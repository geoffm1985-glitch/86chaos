...
assert(pkg.version === '16.0.228', 'package.json version is 16.0.228');
assert(lock.version === '16.0.228' && lock.packages?.['']?.version === '16.0.228', 'package-lock root versions are 16.0.228');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-228.js', 'test:source points to the 16.0.228 validator');
assert(version.version === '16.0.228' && version.build === '16.0.228', 'public version/build are 16.0.228');
assert(version.releaseTitle === 'Restaurant Intelligence and Review Authority', 'release title identifies the restaurant intelligence candidate');
assert(apiVersion.includes("APP_VERSION = '16.0.228'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.228'"), 'API reports 16.0.228');
assert(appCore.includes("CURRENT_VERSION = '16.0.228'"), 'app reports 16.0.228');
...
if (failures) {
  console.error(`16.0.228 source validation failed with ${failures} failure(s).`);
  process.exit(1);
}
console.log('16.0.228 source validation passed.');
