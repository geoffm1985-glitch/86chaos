...
assert(pkg.version === '16.0.15', 'package.json version is 16.0.15');
assert(lock.version === '16.0.15' && lock.packages?.['']?.version === '16.0.15', 'package-lock.json version is 16.0.15');
...
assert(!lockText.includes('@types/yargs/-/yargs-16.0.15.tgz'), 'package-lock does not point to nonexistent @types/yargs 16.0.15 tarball');
...
assert(version.version === '16.0.15' && version.build === '16.0.15', 'public/version.json is 16.0.15');
assert(appCore.includes("CURRENT_VERSION = '16.0.15'"), 'CURRENT_VERSION is 16.0.15');
assert(pkg.scripts?.test === 'node scripts/validate-16-0-15.js', 'npm test runs 16.0.15 validator');
assert(pkg.scripts?.['test:ci'] === 'node scripts/validate-16-0-15.js', 'npm test:ci runs 16.0.15 validator');



// 16.0.15: Schedule hours math must understand AM/PM display strings and ignore exact duplicate shift docs.
...
assert(schedule.includes("meridiem.startsWith('p') && hour < 12"), 'shift hour parser converts PM times to 24-hour minutes');
assert(schedule.includes("meridiem.startsWith('a') && hour === 12"), 'shift hour parser converts midnight correctly');
assert(schedule.includes('return totalMinutes / 60;'), 'calculateShiftHours returns minute-based hour totals');
...
assert(schedule.includes('const schedulePeriodLaborShifts = schedulePeriodDays.flatMap'), 'projected labor uses the same deduped visible schedule shifts');
assert(schedule.includes('const userShifts = dedupeScheduleShiftsForSamePerson(scheduledHoursPeriodShifts.filter'), 'Scheduled Hours Tracker dedupes duplicate shift docs before summing');
assert(schedule.includes('parseScheduleClockMinutes(start)') && schedule.includes('parseScheduleClockMinutes(end)'), 'calculateShiftHours uses the robust clock parser');
...
if (failures) {
  console.error(`16.0.15 schedule hours math validator failed with ${failures} issue(s).`);
  process.exit(1);
}
console.log('16.0.15 schedule hours math validator passed.');
