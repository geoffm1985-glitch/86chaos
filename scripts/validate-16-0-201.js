...
const repairPack = json('scripts/repair-regression-pack-16.0.201.json');
const browserPack = json('scripts/86chaos-release-gate/repair-regression-16.0.201.json');

assert(pkg.version === '16.0.201', 'package.json version is 16.0.201');
assert(lock.version === '16.0.201' && lock.packages?.['']?.version === '16.0.201', 'package-lock root version is 16.0.201');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-201.js', 'test:source points to 16.0.201 validator');
assert(pkg.scripts['test:repair-16-0-201'] === 'npm run test:repair-current', 'test:repair-16-0-201 exists');
assert(version.version === '16.0.201' && version.build === '16.0.201', 'public version/build are 16.0.201');
assert(/CURRENT_VERSION = '16\.0\.201'/.test(appCore), 'app core CURRENT_VERSION is 16.0.201');
assert(/APP_VERSION = '16\.0\.201'/.test(read('api/_version.js')), 'api version is 16.0.201');
assert(read('RUN_86CHAOS_FULL_TEST_SUITE.ps1').includes('test-16-0-201-targeted.cjs'), 'full suite uses 16.0.201 targeted validator');
...
assert(exists('api/app-only-package-hygiene-16-0-201.test.cjs'), '16.0.201 app-only package hygiene test exists');
...
assert(exists('api/app-only-package-hygiene-16-0-201.test.cjs'), '16.0.201 package hygiene test exists');
...
assert(exists('scripts/test-16-0-201-targeted.cjs'), '16.0.201 targeted test script exists');
assert(repairPack.version === '16.0.201', 'repair local manifest version is 16.0.201');
assert(browserPack.version === '16.0.201', 'repair browser manifest version is 16.0.201');
assert(read('scripts/run-repair-regression-pack.cjs').includes('repair-regression-pack-16.0.201.json'), 'repair local pointer uses 16.0.201');
assert(read('scripts/run-repair-browser-regression.cjs').includes('repair-regression-16.0.201.json'), 'repair browser pointer uses 16.0.201');


assert(exists('scripts/86chaos-release-gate/app-only-package-hygiene.cjs'), 'shared package hygiene helper exists');
assert(exists('api/app-only-package-hygiene-16-0-201.test.cjs'), '16.0.201 working-checkout package hygiene regression exists');
assert(read('scripts/86chaos-release-gate/app-only-package-hygiene.cjs').includes('git ls-files'), 'package hygiene helper distinguishes tracked checkout contents from extracted package contents');
assert(read('scripts/run-repair-regression-pack.cjs').includes('CHAOS_ALLOW_GENERATED_TEST_ARTIFACTS'), 'repair runner grants package hygiene a narrow current-run artifact allowance');
assert(read('scripts/test-16-0-201-targeted.cjs').includes('CHAOS_ALLOW_GENERATED_TEST_ARTIFACTS'), 'targeted runner grants package hygiene the same narrow current-run artifact allowance');

assert(exists('api/page-load-readiness-16-0-201.test.cjs'), '16.0.201 page-load readiness regression exists');
assert(read('src/setupTests.js').includes('installFetchShim') && read('src/setupTests.js').includes("require('undici')"), 'Jest setup installs fetch shim before Firebase Auth imports');
assert(read('tests/86chaos-release-gate/16-accessibility-release-gate.spec.cjs').includes('loadAxeRuntime') && !read('tests/86chaos-release-gate/16-accessibility-release-gate.spec.cjs').includes("let AxeBuilder = null"), 'Accessibility release spec lazy-loads optional axe modules after Playwright inventory discovery');
assert(read('scripts/vite-build-with-asset-manifest.cjs').includes('resolveViteCli') && read('scripts/vite-build-with-asset-manifest.cjs').includes('spawnSync(process.execPath'), 'Vite build launcher uses Node plus resolved Vite CLI instead of fragile .bin shim');

assert(exists('api/page-load-readiness-16-0-201.test.cjs'), '16.0.201 page-load readiness regression exists');
assert(exists('api/app-only-package-hygiene-16-0-201.test.cjs'), '16.0.201 local readiness package hygiene regression exists');
assert(read('scripts/86chaos-release-gate/app-only-package-hygiene.cjs').includes('CHAOS_ALLOW_LOCAL_RELEASE_ARTIFACTS'), 'package hygiene helper supports local release-readiness artifact allowance');
assert(read('scripts/86chaos-release-gate/app-only-package-hygiene.cjs').includes('isLocalReadinessArtifact'), 'package hygiene helper distinguishes local generated outputs from final app-only artifacts');
assert(read('scripts/86chaos-release-gate/run-node-release-checks.cjs').includes('CHAOS_ALLOW_LOCAL_RELEASE_ARTIFACTS'), 'release readiness server tests allow generated local build/install artifacts');



const scheduleFeature = read('src/features/schedule.jsx');
assert(scheduleFeature.includes("import * as scheduleRescueDiagnosticsModule from '../core/scheduleRescueDiagnostics.cjs'"), 'Schedule chunk uses namespace import for rescue diagnostics CJS helper');
assert(scheduleFeature.includes('fallbackBuildMyScheduleIncompleteWarningView'), 'Schedule chunk has safe fallback rescue warning renderer');
const managementFeature = read('src/features/management.jsx');
assert(managementFeature.includes("import * as presenceTruthModuleValue from '../core/presenceTruth.cjs'"), 'Management chunk uses namespace import for presence CJS helper');
assert(managementFeature.includes("import * as scheduleEfficiencyModuleValue from '../core/scheduleEfficiency.cjs'"), 'Management chunk uses namespace import for schedule efficiency CJS helper');
assert(managementFeature.includes('fallbackClassifySystemAdminPresenceRow'), 'Management chunk has safe presence classifier fallback');
const appSource199 = read('src/App.js');
assert(appSource199.includes("activeSubTab={activeScheduleSubTab || ''}"), 'App surface boundary receives active Schedule subtab');
assert(appSource199.includes('activeSubTab: context.activeSubTab'), 'runtime crash reports include active subtab context');
assert(exists('api/last-known-good-route-load-16-0-201.test.cjs'), '16.0.201 last-known-good route-load regression exists');
assert(exists('api/shared-listener-release-runtime-16-0-201.test.cjs'), '16.0.201 shared listener release runtime regression exists');
assert(!/const acquireSharedLiveCollection =[\s\S]*?export const useLiveCollection =/.test(appCore) || !/const acquireSharedLiveCollection =[\s\S]*?debugLabelRef/.test(appCore.slice(appCore.indexOf('const acquireSharedLiveCollection ='), appCore.indexOf('export const useLiveCollection ='))), 'shared listener helper does not reference hook-local debugLabelRef');


assert(exists('api/schedule-subtab-navigation-16-0-201.test.cjs'), '16.0.201 schedule subtab navigation regression exists');
assert(!/<AppSurfaceErrorBoundary\s+key=\{`[^`]*activeScheduleSubTab/.test(appSource199), 'Schedule subtab state does not remount the app surface boundary');
assert(!/<React\.Fragment\s+key=\{`[^`]*activeScheduleSubTab/.test(appSource199), 'Schedule subtab state does not remount the route fragment');
assert(appSource199.includes('initialSubTab={activeScheduleSubTab || "my-schedule"}'), 'published Schedule surface preserves selected subtab after parent rerender');
assert(appSource199.includes('initialSubTab={activeScheduleSubTab || "schedule-builder"}'), 'manager Schedule surface preserves selected subtab after parent rerender');

if (failures) { console.error(`\n16.0.201 source validation failed with ${failures} issue(s).`); process.exit(1); }
console.log('\n16.0.201 source validation passed.');
