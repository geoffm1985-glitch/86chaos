...
const commands = [
  ['node', 'scripts/validate-16-0-193.js'],
  ...
  ['node', '--test', 'api/app-only-package-hygiene-16-0-193.test.cjs']
];
for (const cmd of commands) {
  console.log(`[16.0.193 targeted] ${cmd.join(' ')}`);
  const result = spawnSync(cmd[0], cmd.slice(1), { stdio: 'inherit', shell: process.platform === 'win32' });
  if (result.status !== 0) process.exit(result.status || 1);
}
console.log('16.0.193 targeted repair checks passed.');
