...
assert.equal(pkg.version,'17.0.7');assert.equal(lock.version,pkg.version);assert.equal(lock.packages[''].version,pkg.version);assert.equal(version.version,pkg.version);assert.equal(version.releaseTitle,'Vercel Build Workspace Identity Repair');
for(const f of ['src/core/appCore.js','api/_version.js','api/_pos-bridge-config.js'])assert(read(f).includes("'17.0.7'"));
...
for(const test of ['api/repair-17-0-5.test.cjs','api/release-gate-execution-17-0-5.test.cjs','api/vercel-build-identity-17-0-6.test.cjs','api/vercel-build-identity-17-0-7.test.cjs'])assert(pkg.scripts['test:hostile:contracts'].includes(test));
...
console.log('17.0.7 source validation passed; this does not certify the release.');
