...
assert(lock.version === '16.0.202' && lock.packages?.['']?.version === '16.0.202', 'package-lock root versions are 16.0.202');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-202.js', 'test:source points to 16.0.202 validator');
assert(version.version === '16.0.202' && version.build === '16.0.202', 'public/version.json version/build are 16.0.202');
assert(version.releaseTitle === 'Release Gate Maturity Determinism Repair', '16.0.202 release title identifies release-gate determinism repair');
assert(appCore.includes("CURRENT_VERSION = '16.0.202'"), 'app core CURRENT_VERSION is 16.0.202');
assert(apiVersion.includes("APP_VERSION = '16.0.202'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.202'"), 'api version reports 16.0.202');
...
if (failures) {
  console.error(`16.0.202 source validation failed with ${failures} failure(s).`);
  process.exit(1);
}
console.log('16.0.202 source validation passed.');
