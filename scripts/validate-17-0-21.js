...
const expected = '17.0.21';
const releaseTitle = 'Hermetic Release Harness Environment Isolation Repair';
...
for (const file of ['src/core/appCore.js', 'api/_version.js', 'api/_pos-bridge-config.js']) assert(read(file).includes("'17.0.21'"), `${file} current release metadata`);
for (const directory of ['src', 'api', 'scripts', 'test-tools', 'tests']) assert(fs.statSync(directory).isDirectory(), `${directory} is required`);
for (const file of ['firebase.json', 'vercel.json', 'firestore.rules', 'storage.rules', 'RUN_86CHAOS_PLAY_STORE_RELEASE_GATE.ps1', 'RUN_86CHAOS_UPDATE_TEST_DEPLOY.ps1', 'RELEASE_TEST_POLICY.md', 'RELEASE_17_0_21.md', '.gitignore', 'release-source-manifest.json']) assert(fs.statSync(file).isFile(), `${file} is required`);
...
assert(updater.includes("[string]$ExpectedVersion = '17.0.21'"));
...
assert(pkg.scripts['test:repair:17.0.21'].includes('api/release-gate-maturity-16-0-207.test.cjs'));
assert(pkg.scripts['test:repair:17.0.21'].includes('api/release-gate-certification-integrity-17-0-19.test.cjs'));
assert(pkg.scripts['test:repair:17.0.21'].includes('api/failure-extractor-bounded-17-0-20.test.cjs'));
assert(pkg.scripts['test:release:fast'].includes('validate:17.0.21'));
assert(pkg.scripts['test:new-implementations'].includes('validate:17.0.21'));
...
assert(json('test-tools/regressions/registry.json').defects.some(row => row.defectId === 'RG-HARNESS-AMBIENT-ENV-ISOLATION-1721' && row.fixedVersion === expected));
...
assert(pkg.scripts['test:repair:17.0.21'].includes('api/release-gate-harness-env-isolation-17-0-21.test.cjs'));
...
console.log('17.0.21 source validation passed; this does not certify the release.');
