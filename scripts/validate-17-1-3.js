...
assert.equal(pkg.version, '17.1.3');
assert.equal(lock.version, pkg.version);
assert.equal(lock.packages[''].version, pkg.version);
assert.equal(version.version, pkg.version);
assert.equal(version.build, pkg.version);
assert.equal(version.releaseTitle, 'Mobile Workflow and Message Board Repair');
assert.equal(pkg.scripts['test:source'], 'node scripts/validate-17-1-3.js');
assert.equal(pkg.scripts['validate:17.1.3'], 'node scripts/validate-17-1-3.js');
assert(pkg.scripts['test:repair:17.1.3']?.includes('test:current-release-targeted'));
assert(pkg.scripts['test:current-release-targeted']?.includes('api/mobile-workflow-repair-17-1-3.test.cjs'));
assert(pkg.scripts['test:release:fast']?.includes('validate:17.1.3')); 
...
assert(scope.includes("const CURRENT_RELEASE_VERSION = '17.1.3'"));
...
]) assert(read(file).includes('17.1.3'), `${file} carries current version 17.1.3`);
for (const file of [
...
console.log('17.1.3 Mobile Workflow and Message Board Repair validation passed; this does not certify the release.');
