...
assert(pkg.version === '16.0.3', 'package.json version is 16.0.3');
assert(version.version === '16.0.3' && version.build === '16.0.3', 'public/version.json is 16.0.3');
assert(appCore.includes("CURRENT_VERSION = '16.0.3'"), 'CURRENT_VERSION is 16.0.3');
assert(pkg.scripts.test === 'node scripts/validate-16-0-3.js' && pkg.scripts['test:ci'] === 'node scripts/validate-16-0-3.js', 'npm test and test:ci point to current validator');
...
assert(vercel.includes('https://*.firebaseio.com') && vercel.includes('https://*.firebasedatabase.app') && vercel.includes('script-src-elem'), 'CSP allows Firebase RTDB long-poll script fallback');
assert(appCore.includes('const monthKey = getMonthStr(m)') && appCore.includes("return Number.isFinite(date.getTime()) ? date.toLocaleDateString('en-US', { month: 'long', year: 'numeric' }) : 'Unknown Month';"), 'formatDisplayMonth safely normalizes YYYY-MM-DD and month values');
assert(appCore.includes("return Number.isFinite(date.getTime()) ? date.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' }) : 'Unknown Date';"), 'formatDisplayDate avoids visible Invalid Date output');
assert(schedule.includes('formatDisplayMonth(monthStr)') && !schedule.includes('formatDisplayMonth(currentDate)'), 'Full Schedule Active Roster uses a normalized month string label');

if (process.exitCode) process.exit(1);
console.log('16.0.3 CSP and schedule date display validator passed.');
