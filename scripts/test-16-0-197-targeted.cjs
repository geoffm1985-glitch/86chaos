...
const commands = [
  ['node', 'scripts/validate-16-0-197.js'],
  ['node', '--test', 'api/schedule-query-planner-client-test-16-0-197.test.cjs'],
  ['node', '--test', 'api/page-load-readiness-16-0-197.test.cjs'],
  ...
  ['node', '--test', 'api/app-only-package-hygiene-16-0-197.test.cjs']
];
for (const cmd of commands) {
  console.log(`[16.0.197 targeted] ${cmd.join(' ')}`);
  const childEnv = { ...process.env };
  if (cmd.includes('api/app-only-package-hygiene-16-0-197.test.cjs')) {
...
console.log('16.0.197 targeted repair checks passed.');
