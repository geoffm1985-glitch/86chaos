...
assert(pkg.version === '16.0.33', 'package.json version is 16.0.33');
assert(lock.version === '16.0.33' && lock.packages?.['']?.version === '16.0.33', 'package-lock.json version is 16.0.33');
assert(version.version === '16.0.33' && version.build === '16.0.33', 'public/version.json is 16.0.33');
assert(appCore.includes("CURRENT_VERSION = '16.0.33'"), 'CURRENT_VERSION is 16.0.33');
assert(apiVersion.includes("APP_VERSION = '16.0.33'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.33'"), 'API version constants are centralized at 16.0.33');
assert(pkg.scripts?.['test:source'] === 'node scripts/validate-16-0-33.js', 'package scripts point at the 16.0.33 source validator');
assert(exists('scripts/validate-16-0-32.js'), 'historical 16.0.32 source validator is preserved');
...
const localAiCapabilities = read('src/core/localAiCapabilities.js');
const freeAiServices = read('api/free-ai-services.js');
assert(exists('src/core/localAiCapabilities.js'), 'local/free AI capability helpers exist');
assert(localAiCapabilities.includes('FREE_AI_CAPABILITY_REGISTRY') && localAiCapabilities.includes('buildVoiceCapabilityAction') && localAiCapabilities.includes('buildOperationalReadiness'), 'local AI capability registry and 86Voice action builder exist');
assert(localAiCapabilities.includes('tryClientSideOcr') && localAiCapabilities.includes('rankSemanticRecords'), 'free OCR gateway and semantic scoring helpers exist without adding scanner UI placeholders');
assert(exists('api/free-ai-services.js'), 'protected free public-data service route exists');
assert(freeAiServices.includes('verifyRequestToken') && freeAiServices.includes('api.weather.gov') && freeAiServices.includes('openfoodfacts'), 'free data route is auth-protected and supports weather/Open Food Facts lookups');
assert(read('src/components/common.jsx').includes('buildVoiceCapabilityAction') && read('src/components/common.jsx').includes('weather_insight') && read('src/components/common.jsx').includes('food_data_lookup'), '86Voice routes free/local AI capability actions');
assert(read('src/App.js').includes('timePunches={timePunches}') && read('src/App.js').includes('timeOffRequests={timeOffRequests}'), '86Voice receives real schedule/labor context instead of mock data');

if (failures) {
  console.error(`16.0.33 source validator failed with ${failures} issue(s).`);
  process.exit(1);
}
console.log('16.0.33 source validator passed. This is a source guard only. It does not replace unit, emulator, build, browser, or staging notification tests.');
