...
assert(version.releaseTitle === 'Vercel Vite Native Binding Install Repair', 'release title is correct');
...
assert(fs.existsSync(path.join(root, 'scripts/repair-regression-pack-16.0.181.json')), 'repair regression pack manifest exists');
assert(fs.existsSync(path.join(root, 'scripts/86chaos-release-gate/repair-regression-16.0.181.json')), 'repair browser regression manifest exists');
...
const repairManifest = json('scripts/86chaos-release-gate/repair-regression-16.0.181.json');
...
assert(fs.existsSync(path.join(root, 'scripts/repair-regression-pack-16.0.181.json')), '16.0.181 repair local manifest exists');
assert(fs.existsSync(path.join(root, 'scripts/86chaos-release-gate/repair-regression-16.0.181.json')), '16.0.181 repair browser manifest exists');
assert(read('scripts/run-repair-regression-pack.cjs').includes('repair-regression-pack-16.0.181.json'), 'repair pack runner points to 16.0.181 manifest');
assert(read('scripts/run-repair-browser-regression.cjs').includes('repair-regression-16.0.181.json'), 'repair browser runner points to 16.0.181 manifest');
const repairLocal172 = json('scripts/repair-regression-pack-16.0.181.json');
assert(repairLocal172.version === '16.0.181', 'repair local manifest version is 16.0.181');
assert((repairLocal172.localCommands || []).some((entry) => entry.group === 'Current Source Validator' && Array.isArray(entry.cmd) && entry.cmd.includes('scripts/validate-16-0-181.js')), 'repair local current source validator points to 16.0.181');
...
const repairBrowser172 = json('scripts/86chaos-release-gate/repair-regression-16.0.181.json');
assert(repairBrowser172.version === '16.0.181', 'repair browser manifest version is 16.0.181');
...
// 16.0.181 staff login email edit and full-suite reporting repair.
...
const repair173 = json('scripts/repair-regression-pack-16.0.181.json');
assert((repair173.localCommands || []).some(entry => entry.group === 'Staff Email Editing'), '16.0.181 repair local manifest includes Staff Email Editing');
const repairBrowser173 = json('scripts/86chaos-release-gate/repair-regression-16.0.181.json');
assert((repairBrowser173.selected || []).some(row => row.specPath === 'tests/e2e/staff-email-edit.spec.cjs' && row.leafTitle === 'Manager changes an employee login email and the new email authenticates' && row.project === 'chromium'), '16.0.181 repair browser manifest includes staff email chromium identity');
assert((repairBrowser173.selected || []).some(row => row.specPath === 'tests/e2e/staff-email-edit.spec.cjs' && row.project === 'mobile-chromium'), '16.0.181 repair browser manifest includes staff email mobile identity');
...
assert(fullSuite.includes('86chaos-FULL-SUITE-UPLOAD-ME-16.0.181'), 'full-suite upload ZIP is versioned and finalized');
assert(pkg.scripts['test:full-suite'] && pkg.scripts['test:full-suite:local'], 'full-suite package scripts exist');
assert(pkg.scripts['test:repair-16-0-181'] === 'npm run test:repair-current', 'test:repair-16-0-181 package script exists');
assert(fs.existsSync(path.join(root, 'scripts/test-16-0-181-targeted.cjs')), '16.0.181 targeted test script exists');



// 16.0.181 zero-fail orchestration/resilience/source contracts.
...
assert(fs.existsSync(path.join(root, 'api/release-integrity-account-lifecycle-16-0-177.test.cjs')), '16.0.181 release-integrity/account-lifecycle regression exists');
...
assert(fs.existsSync(path.join(root, 'api/vercel-vite-native-binding-16-0-181.test.cjs')), '16.0.181 Vercel native-binding install regression exists');
...
// 16.0.181 ESLint ESM API parser-scope repair.
...
assert(fs.existsSync(path.join(root, 'api/lint-esm-api-16-0-179.test.cjs')), '16.0.181 ESM API lint regression exists');
