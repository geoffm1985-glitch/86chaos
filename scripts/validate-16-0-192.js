...
const repairPack = json('scripts/repair-regression-pack-16.0.192.json');
const browserPack = json('scripts/86chaos-release-gate/repair-regression-16.0.192.json');

assert(pkg.version === '16.0.192', 'package.json version is 16.0.192');
assert(lock.version === '16.0.192' && lock.packages?.['']?.version === '16.0.192', 'package-lock root version is 16.0.192');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-192.js', 'test:source points to 16.0.192 validator');
assert(pkg.scripts['test:repair-16-0-192'] === 'npm run test:repair-current', 'test:repair-16-0-192 exists');
assert(version.version === '16.0.192' && version.build === '16.0.192', 'public version/build are 16.0.192');
assert(/CURRENT_VERSION = '16\.0\.192'/.test(appCore), 'app core CURRENT_VERSION is 16.0.192');
assert(/APP_VERSION = '16\.0\.192'/.test(read('api/_version.js')), 'api version is 16.0.192');
assert(read('RUN_86CHAOS_FULL_TEST_SUITE.ps1').includes('test-16-0-192-targeted.cjs'), 'full suite uses 16.0.192 targeted validator');
...
assert(exists('api/app-only-package-hygiene-16-0-192.test.cjs'), '16.0.192 app-only package hygiene test exists');
...
assert(exists('api/app-only-package-hygiene-16-0-192.test.cjs'), '16.0.192 package hygiene test exists');
...
assert(exists('scripts/test-16-0-192-targeted.cjs'), '16.0.192 targeted test script exists');
assert(repairPack.version === '16.0.192', 'repair local manifest version is 16.0.192');
assert(browserPack.version === '16.0.192', 'repair browser manifest version is 16.0.192');
assert(read('scripts/run-repair-regression-pack.cjs').includes('repair-regression-pack-16.0.192.json'), 'repair local pointer uses 16.0.192');
assert(read('scripts/run-repair-browser-regression.cjs').includes('repair-regression-16.0.192.json'), 'repair browser pointer uses 16.0.192');

if (failures) { console.error(`\n16.0.192 source validation failed with ${failures} issue(s).`); process.exit(1); }
console.log('\n16.0.192 source validation passed.');
