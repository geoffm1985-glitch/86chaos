...
const assert = (condition, message) => {
  if (!condition) {
    console.error(`16.0.90 source validation failed: ${message}`);
    process.exitCode = 1;
  }
};
...
assert(pkg.version === '16.0.90', 'package.json version is 16.0.90');
assert(lock.version === '16.0.90' && lock.packages?.['']?.version === '16.0.90', 'package-lock root version is 16.0.90');
assert(pkg.scripts?.['test:source'] === 'node scripts/validate-16-0-90.js', 'test:source points at 16.0.90 validator');
assert(version.version === '16.0.90' && version.build === '16.0.90', 'public version/build is 16.0.90');
assert(appCore.includes("CURRENT_VERSION = '16.0.90'"), 'appCore CURRENT_VERSION is 16.0.90');
assert(apiVersion.includes("APP_VERSION = '16.0.90'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.90'"), 'API version constants are 16.0.90');

// 16.0.90 platform System Administrator authority checks.
...
// 16.0.90 System Administrator / push / Windows release-gate repairs.
...
assert(app.includes('setPushRepairLinkRequest({ requested: false, consumed: true, nonce') && app.includes('clearPushRepairLinkRequest'), 'push repair link state is marked consumed and cleared after terminal outcomes');
...
if (!process.exitCode) console.log('16.0.90 source validator passed.');
