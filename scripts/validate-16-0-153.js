...
assert(pkg.version === '16.0.153', 'package.json version is 16.0.153');
assert(lock.version === '16.0.153' && lock.packages?.['']?.version === '16.0.153', 'package-lock root versions are 16.0.153');
assert(version.version === '16.0.153' && version.build === '16.0.153', 'public/version.json version/build are 16.0.153');
assert(version.releaseTitle === 'Schedule Warning Controls, Request Off Management, and True Failed-Only Testing', 'public/version.json release title is 16.0.153 release name');
assert(appCore.includes("CURRENT_VERSION = '16.0.153'"), 'app core CURRENT_VERSION is 16.0.153');
assert(apiVersion.includes("APP_VERSION = '16.0.153'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.153'"), 'api version reports 16.0.153');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-153.js', 'test:source points at 16.0.153 validator');
...
assert(!fs.existsSync(path.join(root, 'scripts/validate-16-0-152.js')), 'previous 16.0.152 validator was replaced');
...
assert(prepareManifest.includes("includeNewInventory: selectionMode === 'failed+new'") && prepareManifest.includes("selectionMode === 'repair'") && prepareManifest.includes('currentReleaseFeatureTestsSelected') && prepareManifest.includes('duplicateIdentitiesRemoved'), 'manifest preparer separates failed+new, failed-only, and repair semantics');
...
assert(repairScope.includes("CURRENT_RELEASE_VERSION = '16.0.153'") && repairScope.includes('schedule-request-off-management.spec.cjs') && repairScope.includes('buildRepairSelection') && !repairScope.includes('old full baseline'), 'current-release repair scope is explicit and non-baseline-new');
...
assert(fs.existsSync(path.join(root, 'tests/e2e/schedule-request-off-management.spec.cjs')), 'focused 16.0.153 Playwright feature spec exists');
...
assert(!read('src/core/customerHelpKnowledge.js').includes("CUSTOMER_HELP_VERSION = '16.0.153'"), 'browser Help export version was not bumped to app version');

if (failures) { console.error(`\n${failures} validation check(s) failed.`); process.exit(1); }
console.log('\n16.0.153 source validation passed.');
