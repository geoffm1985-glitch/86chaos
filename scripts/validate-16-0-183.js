...
assert(pkg.version === '16.0.183', 'package.json version is 16.0.183');
assert(lock.version === '16.0.183' && lock.packages?.['']?.version === '16.0.183', 'package-lock root versions are 16.0.183');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-183.js', 'test:source points to 16.0.183 validator');
assert(version.version === '16.0.183' && version.build === '16.0.183', 'public/version.json version/build are 16.0.183');
assert(version.releaseTitle === 'NPM Registry Lock Metadata Repair', 'release title is correct');
assert(appCore.includes("CURRENT_VERSION = '16.0.183'"), 'app core CURRENT_VERSION is 16.0.183');
assert(apiVersion.includes("APP_VERSION = '16.0.183'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.183'"), 'api version reports 16.0.183');
...
assert(fs.existsSync(path.join(root, 'scripts/repair-regression-pack-16.0.183.json')), 'repair regression pack manifest exists');
assert(fs.existsSync(path.join(root, 'scripts/86chaos-release-gate/repair-regression-16.0.183.json')), 'repair browser regression manifest exists');
...
const repairManifest = json('scripts/86chaos-release-gate/repair-regression-16.0.183.json');
...
assert(fs.existsSync(path.join(root, 'scripts/repair-regression-pack-16.0.183.json')), '16.0.183 repair local manifest exists');
assert(fs.existsSync(path.join(root, 'scripts/86chaos-release-gate/repair-regression-16.0.183.json')), '16.0.183 repair browser manifest exists');
assert(read('scripts/run-repair-regression-pack.cjs').includes('repair-regression-pack-16.0.183.json'), 'repair pack runner points to 16.0.183 manifest');
assert(read('scripts/run-repair-browser-regression.cjs').includes('repair-regression-16.0.183.json'), 'repair browser runner points to 16.0.183 manifest');
const repairLocal172 = json('scripts/repair-regression-pack-16.0.183.json');
assert(repairLocal172.version === '16.0.183', 'repair local manifest version is 16.0.183');
assert((repairLocal172.localCommands || []).some((entry) => entry.group === 'Current Source Validator' && Array.isArray(entry.cmd) && entry.cmd.includes('scripts/validate-16-0-183.js')), 'repair local current source validator points to 16.0.183');
...
const repairBrowser172 = json('scripts/86chaos-release-gate/repair-regression-16.0.183.json');
assert(repairBrowser172.version === '16.0.183', 'repair browser manifest version is 16.0.183');
...
// 16.0.183 staff login email edit and full-suite reporting repair.
...
const repair173 = json('scripts/repair-regression-pack-16.0.183.json');
assert((repair173.localCommands || []).some(entry => entry.group === 'Staff Email Editing'), '16.0.183 repair local manifest includes Staff Email Editing');
const repairBrowser173 = json('scripts/86chaos-release-gate/repair-regression-16.0.183.json');
assert((repairBrowser173.selected || []).some(row => row.specPath === 'tests/e2e/staff-email-edit.spec.cjs' && row.leafTitle === 'Manager changes an employee login email and the new email authenticates' && row.project === 'chromium'), '16.0.183 repair browser manifest includes staff email chromium identity');
assert((repairBrowser173.selected || []).some(row => row.specPath === 'tests/e2e/staff-email-edit.spec.cjs' && row.project === 'mobile-chromium'), '16.0.183 repair browser manifest includes staff email mobile identity');
...
assert(fullSuite.includes('86chaos-FULL-SUITE-UPLOAD-ME-16.0.183'), 'full-suite upload ZIP is versioned and finalized');
...
assert(fs.existsSync(path.join(root, 'scripts/test-16-0-183-targeted.cjs')), '16.0.183 targeted test script exists');



// 16.0.183 zero-fail orchestration/resilience/source contracts.
...
assert(fs.existsSync(path.join(root, 'api/release-integrity-account-lifecycle-16-0-177.test.cjs')), '16.0.183 release-integrity/account-lifecycle regression exists');
...
// 16.0.183 ESLint ESM API parser-scope repair.
...
assert(fs.existsSync(path.join(root, 'api/lint-esm-api-16-0-179.test.cjs')), '16.0.183 ESM API lint regression exists');


// 16.0.183 Vite JSX-in-.js compatibility repair.
...
// 16.0.183 npm registry lock metadata repair.
const registryLockTest = 'api/npm-registry-lock-metadata-16-0-183.test.cjs';
assert(fs.existsSync(path.join(root, registryLockTest)), '16.0.183 npm registry lock metadata regression exists');
...
console.log('\n16.0.183 source validation passed.');
