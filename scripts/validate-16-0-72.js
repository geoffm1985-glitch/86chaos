...
const assert = (condition, message) => {
  if (!condition) {
    console.error(`16.0.72 source validation failed: ${message}`);
    process.exitCode = 1;
  }
};
...
assert(pkg.version === '16.0.72', 'package.json version is 16.0.72');
assert(lock.version === '16.0.72' && lock.packages?.['']?.version === '16.0.72', 'package-lock root version is 16.0.72');
assert(version.version === '16.0.72' && version.build === '16.0.72', 'public version/build is 16.0.72');
assert(appCore.includes("CURRENT_VERSION = '16.0.72'"), 'appCore CURRENT_VERSION is 16.0.72');
assert(apiVersion.includes("APP_VERSION = '16.0.72'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.72'"), 'API version constants are 16.0.72');
assert(pkg.scripts?.['test:source'] === 'node scripts/validate-16-0-72.js', 'test:source points at 16.0.72 validator');
...
assert(schedule.includes('const mySchedulePeriodBounds = getScheduleOuterWeekBounds'), 'My Schedule lists the full schedule/pay-period window instead of only same-month dates');
assert(schedule.includes('No published shifts found for this schedule period.'), 'My Schedule empty state uses schedule-period wording');
...
assert(schedule.includes('const isMyPublishedUpcomingShift = (shift) => shiftMatchesPerson(shift, schedulePerson) && isScheduleShiftPublished(shift) && isShiftStillCurrentOrUpcoming(shift, scheduleNow)'), 'My Schedule uses flexible published detection for employee-facing shifts');
...
if (!process.exitCode) console.log('16.0.72 source validator passed.');
