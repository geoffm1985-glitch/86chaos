...
assert(lock.version === '16.0.19' && lock.packages?.['']?.version === '16.0.19', 'package-lock.json version is 16.0.19');
...
// 16.0.19: Schedule hours math must understand AM/PM display strings and ignore exact duplicate shift docs.
...
// 16.0.19: Scheduled Hours Tracker must use merged per-day time intervals, not raw document sums.
...
if (failures) {
  console.error(`16.0.19 schedule hours source-of-truth validator failed with ${failures} issue(s).`);
  process.exit(1);
}

console.log('16.0.19 schedule hours source-of-truth validator passed.');
