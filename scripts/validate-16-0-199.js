...
assert(pkg.version === '16.0.199', 'package.json version is 16.0.199');
assert(lock.version === '16.0.199' && lock.packages?.['']?.version === '16.0.199', 'package-lock root versions are 16.0.199');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-199.js', 'test:source points to 16.0.199 validator');
assert(version.version === '16.0.199' && version.build === '16.0.199', 'public/version.json version/build are 16.0.199');
assert(version.releaseTitle === 'Remaining Failed-Only Browser Gate Blocker Repair', '16.0.199 release title identifies remaining failed-only browser gate blocker repair');
assert(appCore.includes("CURRENT_VERSION = '16.0.199'"), 'app core CURRENT_VERSION is 16.0.199');
assert(apiVersion.includes("APP_VERSION = '16.0.199'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.199'"), 'api version reports 16.0.199');
...
assert(sinceRunner.includes('api/public-icon-assets-16-0-195.test.cjs'), 'targeted runner includes the public icon regression while 16.0.199 validates app-source failed-only repairs');
...
assert(exhaustiveHelper.includes('locatorFromFormDescriptor') && exhaustiveHelper.includes('restoreAndObserve(page,'), '16.0.199 uses stable form-control descriptors for failed-only form probes');
assert(managementFeature.includes('aria-label={tab.label}') && managementFeature.includes('title={tab.label}'), '16.0.199 exposes full System Administrator mobile tab labels to release-gate coverage');
assert(managementFeature.includes('aria-label={label} title={label} onClick={() => setSubTab(id)}'), '16.0.199 exposes Back Office nested tab labels to release-gate coverage');
assert(scheduleFeature.includes('aria-label={label} title={label} onClick={() => setActiveTool(id)}'), '16.0.199 exposes Schedule Builder nested tool labels to release-gate coverage');


assert(inventoryFeature.includes('const [parDrafts, setParDrafts] = useState({});') && inventoryFeature.includes('const [pendingQtyDrafts, setPendingQtyDrafts] = useState({});'), '16.0.199 keeps decimal inventory edits in local draft state while Firestore snapshots catch up');
assert(inventoryFeature.includes('const parseInventoryQuantity = (value, fallback = 0) =>') && inventoryFeature.includes('Number.parseFloat(cleaned)'), '16.0.199 uses a decimal-safe inventory quantity parser');
assert(inventoryFeature.includes('step="0.01" inputMode="decimal" aria-label={`Inventory par for') && inventoryFeature.includes('value={Object.prototype.hasOwnProperty.call(parDrafts, item.id)'), '16.0.199 makes inventory PAR inputs decimal-safe, locally controlled, and accessible');
assert(inventoryFeature.includes('step="0.01" inputMode="decimal" aria-label={`Order quantity for') && inventoryFeature.includes('parseInventoryQuantity(e.target.value, 0)'), '16.0.199 order quantity inputs accept decimal probe values instead of truncating them');
assert(inventoryFeature.includes('setPendingQtyDrafts') && inventoryFeature.includes('label: "Pending quantity", data: { pendingQty: parsed }'), '16.0.199 pending delivery quantity input uses the same decimal-safe path');
assert(!inventoryFeature.includes('setOrderOverrides(p=>({...p, [item.id]: parseInt(e.target.value)||0}))'), '16.0.199 removed parseInt truncation from order quantity controls');
assert(scheduleFeature.includes("'trade-board'") && scheduleFeature.includes("tab === 'trade-board' ? 'Trade Board'"), '16.0.199 exposes Trade Board in the persistent schedule subtab navigation');
assert(scheduleFeature.includes("openCopilotTool('template-editor')") && scheduleFeature.includes("openCopilotTool('drag')"), '16.0.199 exposes Create Template and Drag Board from the Schedule Copilot launcher');
assert(prepFeature.includes("const stateLabel = tab === 'prep' ? 'prep'") && prepFeature.includes('aria-label={stateLabel}'), '16.0.199 keeps Prep reachable by the declared /^prep$/ nested-state label');
assert(managementFeature.includes('flex flex-wrap gap-2 pb-2 border-b border-[#2A353D]') && managementFeature.includes('["quickbooks","QuickBooks"]'), '16.0.199 keeps QuickBooks visible in wrapped Back Office nested navigation');


const exhaustiveMatrix = read('tests/86chaos-release-gate/exhaustive-surface-matrix.cjs');
const currentFailedOnlyFallback = read('scripts/86chaos-release-gate/reported-failed-only-20260825-125909.json');
assert(exhaustiveHelper.includes("else if (type === 'color') sample = '#123456';"), '16.0.199 probes color inputs with a legal six-digit hex value');
assert(!/byLabel\.or\(/.test(exhaustiveHelper), '16.0.199 form descriptors do not use ambiguous byLabel.or(selector).first resolution');
assert(exhaustiveHelper.includes('structuralKey') && exhaustiveHelper.includes('selectorOrdinal') && exhaustiveHelper.includes('.nth(Number(row.selectorOrdinal || 0))'), '16.0.199 resolves form controls by deterministic structural descriptor and ordinal');
assert(exhaustiveHelper.includes("const STATE_INTERACTIVE_SELECTOR = 'button, a, [role=\"button\"], [role=\"tab\"], [role=\"menuitem\"]';"), '16.0.199 discovers state controls before Playwright visible geometry filtering');
assert(!/rect\.width > 0 && rect\.height > 0 && style\.visibility !== 'hidden' && style\.display !== 'none'/.test(exhaustiveHelper), '16.0.199 state control discovery no longer rejects candidates solely for zero current geometry');
assert(exhaustiveHelper.includes("el.hidden || el.getAttribute('aria-hidden') === 'true'") && exhaustiveHelper.includes("style.visibility === 'hidden' || style.display === 'none'"), '16.0.199 still rejects genuinely hidden state controls');
...
assert(!alreadyVisibleSource.includes('bodyText(') && !alreadyVisibleSource.includes('\\b(?:Open\\s+)?'), '16.0.199 active-state evidence no longer comes from arbitrary body prose');
assert(alreadyVisibleSource.includes('aria-selected') && alreadyVisibleSource.includes('aria-current') && alreadyVisibleSource.includes('h1, h2, h3, h4, h5, h6'), '16.0.199 active-state evidence remains structural');
assert(exhaustiveMatrix.indexOf("['Schedule Builder', 'Open Copilot Tools']") > exhaustiveMatrix.indexOf("['Schedule Builder'],") && exhaustiveMatrix.indexOf("['Schedule Builder', 'Open Copilot Tools']") < exhaustiveMatrix.indexOf("['Schedule Builder', 'Coverage']"), '16.0.199 tests Open Copilot Tools before Copilot-opening Schedule Builder states');
for (const required of ["['Schedule Builder', 'Coverage']", "['Schedule Builder', 'Templates']", "['Schedule Builder', /Create Template|Edit Template/i]", "['Schedule Builder', 'Drag Board']", "['Schedule Builder', 'Warnings']", "['Schedule Builder', 'Edit Presets']", "['Schedule Builder', 'Auto-Fill']", "['Schedule Builder', /^Event$/i]"]) {
  assert(exhaustiveMatrix.includes(required), `16.0.199 keeps Schedule Builder state in matrix: ${required}`);
}
assert(exhaustiveMatrix.includes("['QuickBooks']"), '16.0.199 keeps QuickBooks declared in Back Office route states');
assert(managementFeature.includes('["quickbooks","QuickBooks"]') && managementFeature.includes('aria-label={label}'), '16.0.199 keeps the real accessible QuickBooks Back Office navigation button');
assert(prepFeature.includes('maintenance-record-action-button w-9 h-9') && (prepFeature.match(/maintenance-record-action-button w-9 h-9/g) || []).length >= 2, '16.0.199 applies existing PM mobile tap-target class to Preventative Maintenance edit/delete controls');
assert(read('src/styles.css').includes('width: 42px !important;') && read('src/styles.css').includes('@media (min-width: 640px)') && read('src/styles.css').includes('width: 32px !important;'), '16.0.199 preserves existing mobile 42px and desktop compact maintenance action sizing');
assert(currentFailedOnlyFallback.includes('2026-08-25T12-59-09') && currentFailedOnlyFallback.includes('"totalSelected": 5') && currentFailedOnlyFallback.includes('"desktopSelected": 3') && currentFailedOnlyFallback.includes('"mobileSelected": 2'), '16.0.199 bundles the authoritative five-failure failed-only fallback');
assert(read('scripts/86chaos-release-gate/prepare-failed-only-manifest.cjs').includes('reported-failed-only-20260825-125909.json'), '16.0.199 failed-only fallback plumbing prefers the latest five-failure evidence');
assert(sinceRunner.includes('api/failed-only-browser-gate-16-0-199.test.cjs'), 'targeted runner includes the 16.0.199 failed-only browser gate regression');

if (failures) {
  console.error(`16.0.199 source validation failed with ${failures} failure(s).`);
  process.exit(1);
}
console.log('16.0.199 source validation passed.');
