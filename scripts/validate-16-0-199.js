...
const repairPack = json('scripts/repair-regression-pack-16.0.199.json');
const browserPack = json('scripts/86chaos-release-gate/repair-regression-16.0.199.json');

assert(pkg.version === '16.0.199', 'package.json version is 16.0.199');
assert(lock.version === '16.0.199' && lock.packages?.['']?.version === '16.0.199', 'package-lock root version is 16.0.199');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-199.js', 'test:source points to 16.0.199 validator');
assert(pkg.scripts['test:repair-16-0-199'] === 'npm run test:repair-current', 'test:repair-16-0-199 exists');
assert(version.version === '16.0.199' && version.build === '16.0.199', 'public version/build are 16.0.199');
assert(/CURRENT_VERSION = '16\.0\.199'/.test(appCore), 'app core CURRENT_VERSION is 16.0.199');
assert(/APP_VERSION = '16\.0\.199'/.test(read('api/_version.js')), 'api version is 16.0.199');
assert(read('RUN_86CHAOS_FULL_TEST_SUITE.ps1').includes('test-16-0-199-targeted.cjs'), 'full suite uses 16.0.199 targeted validator');
...
assert(exists('api/app-only-package-hygiene-16-0-199.test.cjs'), '16.0.199 app-only package hygiene test exists');
...
assert(exists('api/app-only-package-hygiene-16-0-199.test.cjs'), '16.0.199 package hygiene test exists');
...
assert(exists('scripts/test-16-0-199-targeted.cjs'), '16.0.199 targeted test script exists');
assert(repairPack.version === '16.0.199', 'repair local manifest version is 16.0.199');
assert(browserPack.version === '16.0.199', 'repair browser manifest version is 16.0.199');
assert(read('scripts/run-repair-regression-pack.cjs').includes('repair-regression-pack-16.0.199.json'), 'repair local pointer uses 16.0.199');
assert(read('scripts/run-repair-browser-regression.cjs').includes('repair-regression-16.0.199.json'), 'repair browser pointer uses 16.0.199');


assert(exists('scripts/86chaos-release-gate/app-only-package-hygiene.cjs'), 'shared package hygiene helper exists');
assert(exists('api/app-only-package-hygiene-16-0-199.test.cjs'), '16.0.199 working-checkout package hygiene regression exists');
...
assert(read('scripts/test-16-0-199-targeted.cjs').includes('CHAOS_ALLOW_GENERATED_TEST_ARTIFACTS'), 'targeted runner grants package hygiene the same narrow current-run artifact allowance');

assert(exists('api/page-load-readiness-16-0-199.test.cjs'), '16.0.199 page-load readiness regression exists');
...
assert(exists('api/page-load-readiness-16-0-199.test.cjs'), '16.0.199 page-load readiness regression exists');
assert(exists('api/app-only-package-hygiene-16-0-199.test.cjs'), '16.0.199 local readiness package hygiene regression exists');
...
assert(exists('api/last-known-good-route-load-16-0-199.test.cjs'), '16.0.199 last-known-good route-load regression exists');

if (failures) { console.error(`\n16.0.199 source validation failed with ${failures} issue(s).`); process.exit(1); }
console.log('\n16.0.199 source validation passed.');
