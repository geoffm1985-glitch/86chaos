...
assert.equal(pkg.version, '17.0.37');
assert.equal(lock.version, pkg.version);
assert.equal(lock.packages[''].version, pkg.version);
assert.equal(version.version, pkg.version);
assert.equal(version.build, pkg.version);
assert.equal(version.releaseTitle, 'Desktop System Administrator Concept 1 Isolation');
assert.equal(pkg.scripts['test:source'], 'node scripts/validate-17-0-37.js');
assert.equal(pkg.scripts['validate:17.0.37'], 'node scripts/validate-17-0-37.js');
assert(pkg.scripts['test:repair:17.0.37']?.includes('test:current-release-targeted'));
...
assert(pkg.scripts['test:release:fast']?.includes('validate:17.0.37'));
...
assert(scope.includes("const CURRENT_RELEASE_VERSION = '17.0.37'"));
...
assert(read('tests/86chaos-new-implementations/10-app-bootstrap-i18n-runtime.spec.cjs').includes('Version 17\\.0\\.37'));
...
]) assert(read(file).includes('17.0.37'), `${file} carries current version 17.0.37`);
for (const file of [
...
console.log('17.0.37 Desktop System Administrator Concept 1 Isolation validation passed; this does not certify the release.');
