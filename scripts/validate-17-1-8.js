...
assert.equal(pkg.version, '17.1.8');
assert.equal(lock.version, pkg.version);
assert.equal(lock.packages[''].version, pkg.version);
assert.equal(version.version, pkg.version);
assert.equal(version.build, pkg.version);
assert.equal(version.releaseTitle, '86Voice Resilient Mobile Capture Repair');
assert.equal(pkg.scripts['test:source'], 'node scripts/validate-17-1-8.js');
assert.equal(pkg.scripts['validate:17.1.8'], 'node scripts/validate-17-1-8.js');
assert(pkg.scripts['test:repair:17.1.8']?.includes('test:current-release-targeted'));
...
assert(pkg.scripts['test:release:fast']?.includes('validate:17.1.8')); 
...
assert(scope.includes("const CURRENT_RELEASE_VERSION = '17.1.8'"));
...
]) assert(read(file).includes('17.1.8'), `${file} carries current version 17.1.8`);
for (const file of [
...
assert(deltaRepair.includes('starts 86Voice recognition directly from the user microphone gesture'));
...
assert(read('src/components/common.jsx').includes('openAndListen: openDock'));
...
console.log('17.1.8 86Voice Resilient Mobile Capture Repair validation passed; this does not certify the release.');
