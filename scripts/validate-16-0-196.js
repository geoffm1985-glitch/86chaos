...
assert(lock.version === '16.0.196' && lock.packages?.['']?.version === '16.0.196', 'package-lock root versions are 16.0.196');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-196.js', 'test:source points to 16.0.196 validator');
assert(version.version === '16.0.196' && version.build === '16.0.196', 'public/version.json version/build are 16.0.196');
assert(version.releaseTitle === 'Failed-Only Browser Gate Remaining Blocker Repair', '16.0.196 release title identifies failed-only browser gate repair');
assert(appCore.includes("CURRENT_VERSION = '16.0.196'"), 'app core CURRENT_VERSION is 16.0.196');
assert(apiVersion.includes("APP_VERSION = '16.0.196'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.196'"), 'api version reports 16.0.196');
...
assert(sinceRunner.includes('api/public-icon-assets-16-0-195.test.cjs'), 'targeted runner includes 16.0.196 failed-only browser gate regression');

if (failures) {
  console.error(`16.0.196 source validation failed with ${failures} failure(s).`);
  process.exit(1);
}
console.log('16.0.196 source validation passed.');
