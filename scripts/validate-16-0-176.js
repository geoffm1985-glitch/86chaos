...
assert(pkg.version === '16.0.176', 'package.json version is 16.0.176');
assert(lock.version === '16.0.176' && lock.packages?.['']?.version === '16.0.176', 'package-lock root versions are 16.0.176');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-176.js', 'test:source points to 16.0.176 validator');
assert(version.version === '16.0.176' && version.build === '16.0.176', 'public/version.json version/build are 16.0.176');
assert(version.releaseTitle === 'Full Suite Zero-Fail Security, QA Orchestration and Integrity Repair', 'release title is correct');
assert(appCore.includes("CURRENT_VERSION = '16.0.176'"), 'app core CURRENT_VERSION is 16.0.176');
assert(apiVersion.includes("APP_VERSION = '16.0.176'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.176'"), 'api version reports 16.0.176');
...
assert(fs.existsSync(path.join(root, 'scripts/repair-regression-pack-16.0.176.json')), 'repair regression pack manifest exists');
assert(fs.existsSync(path.join(root, 'scripts/86chaos-release-gate/repair-regression-16.0.176.json')), 'repair browser regression manifest exists');
...
const repairManifest = json('scripts/86chaos-release-gate/repair-regression-16.0.176.json');
...
assert(fs.existsSync(path.join(root, 'scripts/repair-regression-pack-16.0.176.json')), '16.0.176 repair local manifest exists');
assert(fs.existsSync(path.join(root, 'scripts/86chaos-release-gate/repair-regression-16.0.176.json')), '16.0.176 repair browser manifest exists');
assert(read('scripts/run-repair-regression-pack.cjs').includes('repair-regression-pack-16.0.176.json'), 'repair pack runner points to 16.0.176 manifest');
assert(read('scripts/run-repair-browser-regression.cjs').includes('repair-regression-16.0.176.json'), 'repair browser runner points to 16.0.176 manifest');
const repairLocal172 = json('scripts/repair-regression-pack-16.0.176.json');
assert(repairLocal172.version === '16.0.176', 'repair local manifest version is 16.0.176');
...
const repairBrowser172 = json('scripts/86chaos-release-gate/repair-regression-16.0.176.json');
assert(repairBrowser172.version === '16.0.176', 'repair browser manifest version is 16.0.176');
...
// 16.0.176 staff login email edit and full-suite reporting repair.
...
const repair173 = json('scripts/repair-regression-pack-16.0.176.json');
assert((repair173.localCommands || []).some(entry => entry.group === 'Staff Email Editing'), '16.0.176 repair local manifest includes Staff Email Editing');
const repairBrowser173 = json('scripts/86chaos-release-gate/repair-regression-16.0.176.json');
assert((repairBrowser173.selected || []).some(row => row.specPath === 'tests/e2e/staff-email-edit.spec.cjs' && row.leafTitle === 'Manager changes an employee login email and the new email authenticates' && row.project === 'chromium'), '16.0.176 repair browser manifest includes staff email chromium identity');
assert((repairBrowser173.selected || []).some(row => row.specPath === 'tests/e2e/staff-email-edit.spec.cjs' && row.project === 'mobile-chromium'), '16.0.176 repair browser manifest includes staff email mobile identity');
...
assert(fullSuite.includes('86chaos-FULL-SUITE-UPLOAD-ME-16.0.176'), 'full-suite upload ZIP is versioned and finalized');
assert(pkg.scripts['test:full-suite'] && pkg.scripts['test:full-suite:local'], 'full-suite package scripts exist');
assert(pkg.scripts['test:repair-16-0-176'] === 'npm run test:repair-current', 'test:repair-16-0-176 package script exists');
assert(fs.existsSync(path.join(root, 'scripts/test-16-0-176-targeted.cjs')), '16.0.176 targeted test script exists');



// 16.0.176 zero-fail orchestration/resilience/source contracts.
...
assert(fs.existsSync(path.join(root, 'api/full-suite-windows-runner-16-0-176.test.cjs')), '16.0.176 full-suite runner regression exists');
assert(fs.existsSync(path.join(root, 'api/presence-workspace-summary-16-0-176.test.cjs')), '16.0.176 presence resilience regression exists');
assert(fs.existsSync(path.join(root, 'api/system-admin-nuke-users-16-0-176.test.cjs')), '16.0.176 Nuke Users regression exists');
assert(fs.existsSync(path.join(root, 'api/cost-provenance-16-0-176.test.cjs')), '16.0.176 cost provenance regression exists');
assert(fs.existsSync(path.join(root, 'api/lint-config-16-0-176.test.cjs')), '16.0.176 lint regression exists');
assert(fs.existsSync(path.join(root, 'api/dependency-security-16-0-176.test.cjs')), '16.0.176 dependency-security regression exists');

if (failures) { console.error(`\n${failures} validation check(s) failed.`); process.exit(1); }
console.log('\n16.0.176 source validation passed.');
