...
assert(pkg.version === '16.0.204', 'package.json version is 16.0.204');
assert(lock.version === '16.0.204' && lock.packages?.['']?.version === '16.0.204', 'package-lock root versions are 16.0.204');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-204.js', 'test:source points to 16.0.204 validator');
assert(version.version === '16.0.204' && version.build === '16.0.204', 'public/version.json version/build are 16.0.204');
assert(version.releaseTitle === 'Schedule Hydration and Form Probe Determinism Repair', '16.0.204 release title identifies schedule hydration/form probe repair');
assert(appCore.includes("CURRENT_VERSION = '16.0.204'"), 'app core CURRENT_VERSION is 16.0.204');
assert(apiVersion.includes("APP_VERSION = '16.0.204'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.204'"), 'api version reports 16.0.204');
...
if (failures) {
  console.error(`16.0.204 source validation failed with ${failures} failure(s).`);
  process.exit(1);
}
console.log('16.0.204 source validation passed.');
