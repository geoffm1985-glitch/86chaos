...
assert(pkg.version === '16.0.190', 'package.json version is 16.0.190');
assert(lock.version === '16.0.190' && lock.packages?.['']?.version === '16.0.190', 'package-lock root versions are 16.0.190');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-190.js', 'test:source points to 16.0.190 validator');
...
assert(version.version === '16.0.190' && version.build === '16.0.190', 'public/version.json version/build are 16.0.190');
assert(version.releaseTitle === 'Request Off Admin All-Employees Visibility Repair', 'release title is correct');
assert(appCore.includes("CURRENT_VERSION = '16.0.190'"), 'app core CURRENT_VERSION is 16.0.190');
assert(apiVersion.includes("APP_VERSION = '16.0.190'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.190'"), 'api version reports 16.0.190');
...
// Targeted 16.0.190 Schedule Builder time readability repair.
...
assert(timeReadabilitySourceTest.includes('Schedule Builder IN and OUT time controls remain readable') && timeReadabilitySourceTest.includes('schedule-builder-time-input') && timeReadabilitySourceTest.includes('height:\\s*44px !important'), '16.0.190 Schedule Builder time readability regression exists');
assert(sinceRunner.includes('api/schedule-builder-time-readability-source-16-0-185.test.cjs'), 'since-16.0.170 targeted test runner includes 16.0.190 time readability regression');

// Targeted 16.0.190 Month View print readability repair.
...
assert(monthPrintReadability187.includes('font-size:\\s*8\\.6px') && monthPrintReadability187.includes('white-space:\\s*nowrap'), '16.0.190 Month View print readability source regression exists');
assert(sinceRunner.includes('api/month-view-print-readability-source-16-0-187.test.cjs'), 'since-16.0.170 targeted test runner includes 16.0.190 Month View print readability regression');

// Targeted 16.0.190 Request Off workflow visibility repair.
...
assert(requestOffWorkflowVisibility188.includes('loads all active requested days') && requestOffWorkflowVisibility188.includes("useState"), '16.0.190 Request Off workflow visibility regression exists');
assert(sinceRunner.includes('api/request-off-workflow-visibility-16-0-188.test.cjs'), 'since-16.0.170 targeted test runner includes 16.0.190 Request Off visibility regression');
...
// Targeted 16.0.190 Request Off workflow date-range repair.
...
assert(requestOffDateRangeSourceTest.includes('Request Off workflow date filters load the selected month or custom range directly'), '16.0.190 Request Off date-range regression exists');
assert(sinceRunner.includes('api/request-off-workflow-date-range-16-0-189.test.cjs'), 'since-16.0.170 targeted test runner includes 16.0.190 Request Off date-range regression');
...
// Targeted 16.0.190 Request Off admin all-employees visibility repair.
...
assert(requestOffAdminVisibilitySourceTest.includes('server-backed manager list') && requestOffAdminVisibilitySourceTest.includes('all employees'), '16.0.190 Request Off admin visibility source regression exists');

if (failures) { console.error(`\n${failures} validation check(s) failed.`); process.exit(1); }
console.log('\n16.0.190 source validation passed.');
