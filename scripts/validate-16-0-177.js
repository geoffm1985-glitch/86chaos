...
assert(version.releaseTitle === 'Partial Resume Release Gate And Login Workspace Repair', 'release title is correct');
...
assert(!fs.existsSync(path.join(root, 'scripts/validate-16-0-175.js')), 'previous validator was replaced');
...
// Targeted 16.0.177 release-gate resume and login/workspace readiness repairs.
...
assert(partialResumeSourceTest.includes('partial-resume manifest reruns only failed timed-out and not-run') && partialResumeSourceTest.includes('156') && partialResumeSourceTest.includes('65 tests that already passed'), '16.0.177 partial-resume source regression test exists');
assert(loginWorkspaceResumeTest.includes('Server login bootstrap') && loginWorkspaceResumeTest.includes('release-gate route settling selects the current QA workspace'), '16.0.177 login/workspace source regression test exists');
assert(sinceRunner.includes('api/partial-resume-release-gate-source-16-0-177.test.cjs') && sinceRunner.includes('api/login-workspace-resume-16-0-177.test.cjs'), 'since-16.0.170 targeted test runner includes 16.0.177 regressions');
