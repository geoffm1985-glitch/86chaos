...
assert.equal(pkg.version, '17.0.36');
assert.equal(lock.version, pkg.version);
assert.equal(lock.packages[''].version, pkg.version);
assert.equal(version.version, pkg.version);
assert.equal(version.build, pkg.version);
assert.equal(version.releaseTitle, 'Complete System Administrator Directory and Desktop Repair');
assert.equal(pkg.scripts['test:source'], 'node scripts/validate-17-0-36.js');
assert.equal(pkg.scripts['validate:17.0.36'], 'node scripts/validate-17-0-36.js');
assert(pkg.scripts['test:repair:17.0.36']?.includes('test:current-release-targeted'));
...
assert(pkg.scripts['test:release:fast']?.includes('validate:17.0.36'));
...
assert(scope.includes("const CURRENT_RELEASE_VERSION = '17.0.36'"));
...
assert(read('tests/86chaos-new-implementations/10-app-bootstrap-i18n-runtime.spec.cjs').includes('Version 17\\.0\\.35'));
...
]) assert(read(file).includes('17.0.36'), `${file} carries current version 17.0.36`);
for (const file of [
...
console.log('17.0.36 Complete System Administrator Directory and Desktop Repair validation passed; this does not certify the release.');
