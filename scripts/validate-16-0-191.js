...
const repairPack = json('scripts/repair-regression-pack-16.0.191.json');
const browserPack = json('scripts/86chaos-release-gate/repair-regression-16.0.191.json');

assert(pkg.version === '16.0.191', 'package.json version is 16.0.191');
assert(lock.version === '16.0.191' && lock.packages?.['']?.version === '16.0.191', 'package-lock root version is 16.0.191');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-191.js', 'test:source points to 16.0.191 validator');
assert(pkg.scripts['test:repair-16-0-191'] === 'npm run test:repair-current', 'test:repair-16-0-191 exists');
assert(version.version === '16.0.191' && version.build === '16.0.191', 'public version/build are 16.0.191');
assert(/CURRENT_VERSION = '16\.0\.191'/.test(appCore), 'app core CURRENT_VERSION is 16.0.191');
assert(/APP_VERSION = '16\.0\.191'/.test(read('api/_version.js')), 'api version is 16.0.191');
assert(read('RUN_86CHAOS_FULL_TEST_SUITE.ps1').includes('test-16-0-191-targeted.cjs'), 'full suite uses 16.0.191 targeted validator');
...
assert(exists('docs/firebase-efficiency-16.0.191-report.md') && exists('docs/firebase-efficiency-16.0.191-report.json'), '16.0.191 before/after efficiency report exists');
assert(exists('api/firebase-efficiency-finalization-16-0-191.test.cjs'), '16.0.191 finalization behavioral tests exist');
assert(exists('api/app-only-package-hygiene-16-0-191.test.cjs'), '16.0.191 app-only package hygiene test exists');
...
assert(exists('docs/firebase-efficiency-16.0.191-report.md') && exists('docs/firebase-efficiency-16.0.191-report.json'), '16.0.191 before/after efficiency report exists');
assert(exists('api/firebase-efficiency-finalization-16-0-191.test.cjs'), '16.0.191 behavior tests exist');
assert(exists('api/app-only-package-hygiene-16-0-191.test.cjs'), '16.0.191 package hygiene test exists');
...
assert(exists('api/firebase-efficiency-finalization-16-0-191.test.cjs'), '16.0.191 Firebase efficiency completion regression exists');
assert(exists('scripts/test-16-0-191-targeted.cjs'), '16.0.191 targeted test script exists');
assert(repairPack.version === '16.0.191', 'repair local manifest version is 16.0.191');
assert(browserPack.version === '16.0.191', 'repair browser manifest version is 16.0.191');
assert(read('scripts/run-repair-regression-pack.cjs').includes('repair-regression-pack-16.0.191.json'), 'repair local pointer uses 16.0.191');
assert(read('scripts/run-repair-browser-regression.cjs').includes('repair-regression-16.0.191.json'), 'repair browser pointer uses 16.0.191');

if (failures) { console.error(`\n16.0.191 source validation failed with ${failures} issue(s).`); process.exit(1); }
