...
const requestOffLegacyStartDateSourceTest = read('api/request-off-workflow-legacy-start-date-16-0-191.test.cjs');

assert(pkg.version === '16.0.191', 'package.json version is 16.0.191');
assert(lock.version === '16.0.191' && lock.packages?.['']?.version === '16.0.191', 'package-lock root versions are 16.0.191');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-191.js', 'test:source points to 16.0.191 validator');
...
assert(version.version === '16.0.191' && version.build === '16.0.191', 'public/version.json version/build are 16.0.191');
assert(version.releaseTitle === 'Request Off Legacy Start Date Visibility Repair', 'release title is correct');
assert(appCore.includes("CURRENT_VERSION = '16.0.191'"), 'app core CURRENT_VERSION is 16.0.191');
assert(apiVersion.includes("APP_VERSION = '16.0.191'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.191'"), 'api version reports 16.0.191');
assert(fs.existsSync(path.join(root, 'scripts/validate-16-0-191.js')), '16.0.191 validator exists');
...
// Targeted 16.0.191 Schedule Builder time readability repair.
...
assert(timeReadabilitySourceTest.includes('Schedule Builder IN and OUT time controls remain readable') && timeReadabilitySourceTest.includes('schedule-builder-time-input') && timeReadabilitySourceTest.includes('height:\\s*44px !important'), '16.0.191 Schedule Builder time readability regression exists');
assert(sinceRunner.includes('api/schedule-builder-time-readability-source-16-0-185.test.cjs'), 'since-16.0.170 targeted test runner includes 16.0.191 time readability regression');

// Targeted 16.0.191 Month View print readability repair.
...
assert(monthPrintReadability187.includes('font-size:\\s*8\\.6px') && monthPrintReadability187.includes('white-space:\\s*nowrap'), '16.0.191 Month View print readability source regression exists');
assert(sinceRunner.includes('api/month-view-print-readability-source-16-0-187.test.cjs'), 'since-16.0.170 targeted test runner includes 16.0.191 Month View print readability regression');

// Targeted 16.0.191 Request Off workflow visibility repair.
...
assert(requestOffWorkflowVisibility188.includes('loads all active requested days') && requestOffWorkflowVisibility188.includes("useState"), '16.0.191 Request Off workflow visibility regression exists');
assert(sinceRunner.includes('api/request-off-workflow-visibility-16-0-188.test.cjs'), 'since-16.0.170 targeted test runner includes 16.0.191 Request Off visibility regression');
...
// Targeted 16.0.191 Request Off workflow date-range repair.
const requestOffDateRangeSourceTest = read('api/request-off-workflow-date-range-16-0-189.test.cjs');
const requestOffAdminVisibilitySourceTest = read('api/request-off-workflow-admin-all-users-16-0-190.test.cjs');
...
assert(schedule.includes("mergeRequestOffWorkflowRows(timeOffRequests || [], workflowDateScopedRequests || [], workflowApiRequests || [])"), 'Request Off workflow merges parent, date-scoped, and server-backed rows');
...
assert(requestOffDateRangeSourceTest.includes('Request Off workflow date filters load the selected month or custom range directly'), '16.0.191 Request Off date-range regression exists');
assert(sinceRunner.includes('api/request-off-workflow-date-range-16-0-189.test.cjs'), 'since-16.0.170 targeted test runner includes 16.0.191 Request Off date-range regression');
...
// Targeted 16.0.191 Request Off admin all-employees visibility repair.
assert(schedule.includes("const [workflowApiRequests, setWorkflowApiRequests] = useState([])"), 'Request Off workflow stores server-backed manager rows');
assert(schedule.includes("requestOffApi('workflow-list', { startDate: workflowRequestRange.start, endDate: workflowRequestRange.end, dateFilter })"), 'Request Off workflow calls manager workflow-list API for selected range');
assert(schedule.includes("mergeRequestOffWorkflowRows(timeOffRequests || [], workflowDateScopedRequests || [], workflowApiRequests || [])"), 'Request Off workflow merges parent, client date-scoped, and server manager rows');
assert(timeOffRequestApi.includes("if (action === 'workflow-list') return handleWorkflowList(ctx, body)"), 'Request Off API exposes workflow-list action');
assert(timeOffRequestApi.includes("const workspaceFields = ['restaurantId', 'workspaceId', 'tenantId', 'clientId']"), 'Request Off workflow-list queries all workspace id aliases');
assert(timeOffRequestApi.includes('row.scheduleDateKey') && timeOffRequestApi.includes('row.scheduleDayKey'), 'Request Off API normalizes legacy schedule date aliases');
assert(timeOffRequestApi.includes("[row.restaurantId, row.workspaceId, row.tenantId, row.clientId]"), 'Request Off workspace matching accepts any workspace alias instead of first non-empty only');
assert(timeOffRequestTest.includes('manager workflow-list returns all workspace Request Off rows across legacy workspace and date fields'), 'Request Off API unit regression covers admin all-employees visibility');
assert(requestOffAdminVisibilitySourceTest.includes('server-backed manager list') && requestOffAdminVisibilitySourceTest.includes('all employees'), '16.0.191 Request Off admin visibility source regression exists');


// Targeted 16.0.191 Request Off legacy startDate visibility repair.
assert(schedule.includes('request?.date || request?.requestDate || request?.requestedDate || request?.startDate || request?.dateKey'), 'Request Off client date resolver includes legacy startDate alias');
assert(timeOffRequestApi.includes('row.date || row.requestDate || row.requestedDate || row.startDate || row.dateKey'), 'Request Off server date resolver includes legacy startDate alias');
assert(timeOffRequestTest.includes('manager workflow-list includes future legacy Request Off rows stored with startDate'), 'Request Off API unit regression covers future legacy startDate rows');
assert(timeOffRequestTest.includes('legacy-startdate-request') && timeOffRequestTest.includes('legacy-workspace-startdate-request') && timeOffRequestTest.includes('wrong-workspace-startdate-request'), 'Request Off startDate regression covers restaurantId, workspaceId, and workspace isolation');
assert(requestOffLegacyStartDateSourceTest.includes('request\\?\\.startDate') && requestOffLegacyStartDateSourceTest.includes('row\\.startDate'), '16.0.191 source regression verifies client and server startDate aliases');
assert(requestOffLegacyStartDateSourceTest.includes("requestOffApi\\('workflow-list'") && requestOffLegacyStartDateSourceTest.includes('mergeRequestOffWorkflowRows'), '16.0.191 source regression verifies existing manager workflow-list and merge architecture');
assert(sinceRunner.includes('api/request-off-workflow-legacy-start-date-16-0-191.test.cjs'), 'since-16.0.170 targeted test runner includes 16.0.191 startDate regression');

if (failures) { console.error(`\n${failures} validation check(s) failed.`); process.exit(1); }
console.log('\n16.0.191 source validation passed.');
