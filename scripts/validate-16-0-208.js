...
assert(pkg.version === '16.0.208', 'package.json version is 16.0.208');
assert(lock.version === '16.0.208' && lock.packages?.['']?.version === '16.0.208', 'package-lock root versions are 16.0.208');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-208.js', 'test:source points to 16.0.208 validator');
assert(version.version === '16.0.208' && version.build === '16.0.208', 'public/version.json version/build are 16.0.208');
assert(version.releaseTitle === 'Delta Gate Runtime Stabilization', '16.0.208 release title identifies delta gate runtime stabilization');
assert(appCore.includes("CURRENT_VERSION = '16.0.208'"), 'app core CURRENT_VERSION is 16.0.208');
assert(apiVersion.includes("APP_VERSION = '16.0.208'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.208'"), 'api version reports 16.0.208');
...
assert(fs.readFileSync(path.join(root, 'tests/86chaos-full-audit/utils/audit-helpers.cjs'), 'utf8').includes('Login did not leave the login screen'), '16.0.208 login helper fails explicitly if still on login');
assert(fs.readFileSync(path.join(root, 'tests/86chaos-release-gate/utils/exhaustive-ui-helpers.cjs'), 'utf8').includes('openResponsiveStateDirectory'), '16.0.208 state helper opens responsive directory before missing state');
assert(fs.readFileSync(path.join(root, 'tests/86chaos-release-gate/28-exhaustive-route-state-control-graph.spec.cjs'), 'utf8').includes('const expensiveProbe = stateIndex === 0'), '16.0.208 exhaustive graph avoids repeated expensive probes');
assert(fs.readFileSync(path.join(root, 'src/features/schedule.jsx'), 'utf8').includes('aria-label="Full schedule shift list" tabIndex={0}'), '16.0.208 full schedule scroll region is keyboard focusable');

if (failures) {
  console.error(`16.0.208 source validation failed with ${failures} failure(s).`);
  process.exit(1);
}
console.log('16.0.208 source validation passed.');
