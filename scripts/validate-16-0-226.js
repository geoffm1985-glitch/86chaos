...
assert(pkg.version === '16.0.226', 'package.json version is 16.0.226');
assert(lock.version === '16.0.226' && lock.packages?.['']?.version === '16.0.226', 'package-lock root versions are 16.0.226');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-226.js', 'test:source points to the 16.0.226 validator');
assert(version.version === '16.0.226' && version.build === '16.0.226', 'public version/build are 16.0.226');
assert(version.releaseTitle === 'Request Off Bulk Archive Determinism Repair', 'release title identifies the Request Off traversal repair');
assert(apiVersion.includes("APP_VERSION = '16.0.226'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.226'"), 'API reports 16.0.226');
assert(appCore.includes("CURRENT_VERSION = '16.0.226'"), 'app reports 16.0.226');
...
if (failures) {
  console.error(`16.0.226 source validation failed with ${failures} failure(s).`);
  process.exit(1);
}
console.log('16.0.226 source validation passed.');
