...
assert(pkg.version === '16.0.14', 'package.json version is 16.0.14');
assert(lock.version === '16.0.14' && lock.packages?.['']?.version === '16.0.14', 'package-lock.json version is 16.0.14');
...
assert(!lockText.includes('@types/yargs/-/yargs-16.0.14.tgz'), 'package-lock does not point to nonexistent @types/yargs 16.0.14 tarball');
...
assert(version.version === '16.0.14' && version.build === '16.0.14', 'public/version.json is 16.0.14');
assert(appCore.includes("CURRENT_VERSION = '16.0.14'"), 'CURRENT_VERSION is 16.0.14');
assert(pkg.scripts?.test === 'node scripts/validate-16-0-14.js', 'npm test runs 16.0.14 validator');
assert(pkg.scripts?.['test:ci'] === 'node scripts/validate-16-0-14.js', 'npm test:ci runs 16.0.14 validator');


// 16.0.14: Schedule Builder must load Event Calendar records while the schedule screen is open.
...
if (failures) {
  console.error(`16.0.14 schedule builder event loader validator failed with ${failures} issue(s).`);
  process.exit(1);
}
console.log('16.0.14 schedule builder event loader validator passed.');
