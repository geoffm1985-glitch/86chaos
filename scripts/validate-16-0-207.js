...
assert(pkg.version === '16.0.207', 'package.json version is 16.0.207');
assert(lock.version === '16.0.207' && lock.packages?.['']?.version === '16.0.207', 'package-lock root versions are 16.0.207');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-207.js', 'test:source points to 16.0.207 validator');
assert(version.version === '16.0.207' && version.build === '16.0.207', 'public/version.json version/build are 16.0.207');
assert(version.releaseTitle === 'Delta Gate Determinism Repair', '16.0.207 release title identifies delta gate determinism repair');
assert(appCore.includes("CURRENT_VERSION = '16.0.207'"), 'app core CURRENT_VERSION is 16.0.207');
assert(apiVersion.includes("APP_VERSION = '16.0.207'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.207'"), 'api version reports 16.0.207');
...
if (failures) {
  console.error(`16.0.207 source validation failed with ${failures} failure(s).`);
  process.exit(1);
}
console.log('16.0.207 source validation passed.');
