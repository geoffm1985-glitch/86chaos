...
const assert = (condition, message) => {
  if (!condition) {
    console.error(`16.0.68 source validation failed: ${message}`);
    process.exitCode = 1;
  }
};
...
assert(lock.version === '16.0.68' && lock.packages?.['']?.version === '16.0.68', 'package-lock root version is 16.0.68');
assert(version.version === '16.0.68' && version.build === '16.0.68', 'public version/build is 16.0.68');
assert(appCore.includes("CURRENT_VERSION = '16.0.68'"), 'appCore CURRENT_VERSION is 16.0.68');
assert(apiVersion.includes("APP_VERSION = '16.0.68'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.68'"), 'API version constants are 16.0.68');
assert(pkg.scripts?.['test:source'] === 'node scripts/validate-16-0-68.js', 'test:source points at 16.0.68 validator');
...
if (!process.exitCode) console.log('16.0.68 source validator passed.');
