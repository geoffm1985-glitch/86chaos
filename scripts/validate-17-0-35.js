...
assert.equal(pkg.version, '17.0.35');
assert.equal(lock.version, pkg.version);
assert.equal(lock.packages[''].version, pkg.version);
assert.equal(version.version, pkg.version);
assert.equal(version.build, pkg.version);
assert.equal(version.releaseTitle, 'System Administrator Subpage Polish and QA Seed Host Repair');
assert.equal(pkg.scripts['test:source'], 'node scripts/validate-17-0-35.js');
assert.equal(pkg.scripts['validate:17.0.35'], 'node scripts/validate-17-0-35.js');
assert(pkg.scripts['test:repair:17.0.35']?.includes('test:current-release-targeted'));
...
assert(pkg.scripts['test:release:fast']?.includes('validate:17.0.35'));
...
assert(scope.includes("const CURRENT_RELEASE_VERSION = '17.0.35'"));
...
]) assert(read(file).includes('17.0.35'), `${file} carries current version 17.0.35`);
for (const file of [
