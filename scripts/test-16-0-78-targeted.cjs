...
const assert = (condition, message) => {
  if (!condition) {
    console.error(`16.0.78 targeted Schedule Builder chip visual test failed: ${message}`);
    process.exitCode = 1;
  }
};
...
assert(version.version === '16.0.78' && version.build === '16.0.78', 'version.json reports 16.0.78');
assert(pkg.version === '16.0.78', 'package.json reports 16.0.78');
assert(lock.version === '16.0.78' && lock.packages?.['']?.version === '16.0.78', 'package-lock root version is 16.0.78');
assert(pkg.scripts?.['test:source'] === 'node scripts/validate-16-0-78.js', 'test:source points to the 16.0.78 validator');
assert(apiVersion.includes("APP_VERSION = '16.0.78'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.78'"), 'API version constants are 16.0.78');
assert(appCore.includes("CURRENT_VERSION = '16.0.78'"), 'appCore CURRENT_VERSION is 16.0.78');
...
assert(idx >= 0, '16.0.78 final Schedule Builder chip surface block exists');
...
assert(!/16\.0\.78[\s\S]*font-weight\s*:/.test(finalBlock), '16.0.78 does not change the current font weight');
assert(!/16\.0\.78[\s\S]*(^|[^-])width:\s*100% !important/m.test(finalBlock), 'final 16.0.78 chip surface does not force full-cell width');
assert(!/16\.0\.78[\s\S]*overflow:\s*visible !important/.test(finalBlock), 'final 16.0.78 chip surface does not allow time text outside the colored background');
assert(!/16\.0\.78[\s\S]*white-space:\s*normal !important/.test(finalBlock), 'final 16.0.78 chip surface does not reintroduce wrapping');
assert(!/16\.0\.78[\s\S]*<br\s*\/?\s*>/.test(finalBlock), 'no line break tags are introduced for shift labels');

if (!process.exitCode) console.log('16.0.78 targeted Schedule Builder chip visual CSS tests passed.');
