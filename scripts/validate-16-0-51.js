...
assert(pkg.version === '16.0.51', 'package.json version is 16.0.51');
assert(lock.version === '16.0.51' && lock.packages?.['']?.version === '16.0.51', 'package-lock.json version is 16.0.51');
assert(version.version === '16.0.51' && version.build === '16.0.51', 'public/version.json is 16.0.51');
assert(appCore.includes("CURRENT_VERSION = '16.0.51'"), 'CURRENT_VERSION is 16.0.51');
assert(apiVersion.includes("APP_VERSION = '16.0.51'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.51'"), 'API version constants are centralized at 16.0.51');
assert(pkg.scripts?.['test:source'] === 'node scripts/validate-16-0-51.js', 'package scripts point at the 16.0.51 source validator');
...
assert((styles.includes('16.0.47 release-gate accessibility') || styles.includes('16.0.48 release-gate accessibility') || styles.includes('16.0.50 release-gate accessibility') || styles.includes('16.0.51 mobile Schedule Builder readability')) && appJs.includes('describeControl'), 'accessibility/tap-target hardening is present');
...
assert(styles.includes('16.0.51 mobile Schedule Builder readability') && styles.includes('scheduled-hours-tracker-table'), 'mobile schedule times and hours tracker labels stay readable and horizontal');
...
assert(!appJs.includes('fcmTokens: arrayUnion(currentToken)') && appJs.includes('primaryTokenMissing') && appJs.includes("pushTokenDedupeVersion: '16.0.51'") && appJs.includes('writePushProfilePatch'), 'push token sync repairs the primary token and pushDevices without writing the disallowed browser fcmTokens array');

if (failures) {
  console.error(`16.0.51 source validator failed with ${failures} issue(s).`);
  process.exit(1);
}
console.log('16.0.51 source validator passed. This is a source guard only. It does not replace unit, emulator, build, browser, or staging notification tests.');

// 16.0.51 specific guardrails
try {
  const app = require('fs').readFileSync('src/App.js', 'utf8');
  if (/\[activeTabState,\s*isMenuOpen,\s*isWorkspaceSwitcherOpen\]/.test(app)) {
    console.error('16.0.51 guard failed: accessibility observer must not reference menu state before hook initialization.');
    process.exitCode = 1;
  }
  const sw = require('fs').readFileSync('public/firebase-messaging-sw.js', 'utf8');
  if (!sw.includes('/notification-badge.png')) {
    console.error('16.0.51 guard failed: notification badge fallback missing.');
    process.exitCode = 1;
  }
} catch (error) {
  console.error('16.0.51 guard failed:', error.message);
  process.exitCode = 1;
}
