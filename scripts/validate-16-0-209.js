...
assert(pkg.version === '16.0.209', 'package.json version is 16.0.209');
assert(lock.version === '16.0.209' && lock.packages?.['']?.version === '16.0.209', 'package-lock root versions are 16.0.209');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-209.js', 'test:source points to 16.0.209 validator');
assert(version.version === '16.0.209' && version.build === '16.0.209', 'public/version.json version/build are 16.0.209');
assert(version.releaseTitle === 'Failed-Only Request Off Mobile Repair', '16.0.209 release title identifies failed-only Request Off mobile repair');
assert(appCore.includes("CURRENT_VERSION = '16.0.209'"), 'app core CURRENT_VERSION is 16.0.209');
assert(apiVersion.includes("APP_VERSION = '16.0.209'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.209'"), 'api version reports 16.0.209');
...
if (failures) {
  console.error(`16.0.209 source validation failed with ${failures} failure(s).`);
  process.exit(1);
}
console.log('16.0.209 source validation passed.');
