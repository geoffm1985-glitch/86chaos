...
assert.equal(pkg.version, '17.1.12');
assert.equal(lock.version, pkg.version);
assert.equal(lock.packages[''].version, pkg.version);
assert.equal(version.version, pkg.version);
assert.equal(version.build, pkg.version);
assert.equal(version.releaseTitle, 'Android PWA 86Voice Visible Panel Repair');
assert.equal(pkg.scripts['test:source'], 'node scripts/validate-17-1-12.js');
assert.equal(pkg.scripts['validate:17.1.12'], 'node scripts/validate-17-1-12.js');
assert(pkg.scripts['test:repair:17.1.12']?.includes('test:current-release-targeted'));
assert(pkg.scripts['test:current-release-targeted']?.includes('api/mobile-voice-pwa-panel-17-1-12.test.cjs'));
assert(pkg.scripts['test:current-release-targeted']?.includes('api/mobile-voice-touch-activation-17-1-11.test.cjs'));
...
assert(pkg.scripts['test:release:fast']?.includes('validate:17.1.12')); 
...
assert(scope.includes("const CURRENT_RELEASE_VERSION = '17.1.12'"));
assert(scope.includes('27-mobile-voice-pwa-panel.spec.cjs'));
assert(scope.includes('26-mobile-voice-physical-touch.spec.cjs'));
...
]) assert(read(file).includes('17.1.12'), `${file} carries current version 17.1.12`);
for (const file of [
...
assert(app17110.includes('controller?.openAndListen'));
assert(app17110.includes('controller?.openPanel'));
assert(app17110.includes('chaos:voice-open-and-listen'));
...
const touch17111 = read('api/mobile-voice-touch-activation-17-1-11.test.cjs');
const shell17111 = read('src/components/concept17.jsx');
assert(touch17111.includes('physical pointer-down'));
assert(shell17111.includes('onPointerDown={activateVoiceFromPointer}'));
assert(shell17111.includes('onContextMenu={suppressVoiceBrowserCallout}'));
assert(conceptCss.includes('17.1.11 mobile 86Voice physical-touch repair'));
assert(exists('tests/86chaos-new-implementations/26-mobile-voice-physical-touch.spec.cjs'));
assert(exists('RELEASE_17_1_11.md'));

const pwaPanel17112 = read('api/mobile-voice-pwa-panel-17-1-12.test.cjs');
const pwaPanel17112Browser = read('tests/86chaos-new-implementations/27-mobile-voice-pwa-panel.spec.cjs');
assert(pwaPanel17112.includes('portals the mobile 86Voice panel out of the zero-sized legacy dock'));
assert(voiceCommon.includes("import { createPortal, flushSync } from 'react-dom';"));
assert(voiceCommon.includes('className="voice-command-panel-surface cockpit-panel'));
assert(voiceCommon.includes("document.querySelector('.concept17-shell') || document.body"));
assert(voiceCommon.includes('createPortal(voicePanel, mobileVoicePortalTarget)'));
assert(voiceCommon.includes('try { flushSync(revealPanel); } catch (_) { revealPanel(); }'));
assert(conceptCss.includes('17.1.12 Android installed-PWA 86Voice panel repair'));
assert(conceptCss.includes('.voice-command-panel-surface'));
assert(conceptCss.includes('z-index: 120 !important'));
assert(pwaPanel17112Browser.includes('dockAncestor'));
assert(pwaPanel17112Browser.includes("expect(geometry.position).toBe('fixed')"));
assert(exists('RELEASE_17_1_12.md'));

console.log('17.1.12 Android PWA 86Voice visible-panel validation passed; this does not certify the release.');
