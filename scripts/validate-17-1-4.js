...
assert.equal(pkg.version, '17.1.4');
assert.equal(lock.version, pkg.version);
assert.equal(lock.packages[''].version, pkg.version);
assert.equal(version.version, pkg.version);
assert.equal(version.build, pkg.version);
assert.equal(version.releaseTitle, 'Release-Gate Browser Stability Repair');
assert.equal(pkg.scripts['test:source'], 'node scripts/validate-17-1-4.js');
assert.equal(pkg.scripts['validate:17.1.4'], 'node scripts/validate-17-1-4.js');
assert(pkg.scripts['test:repair:17.1.4']?.includes('test:current-release-targeted'));
...
assert(pkg.scripts['test:release:fast']?.includes('validate:17.1.4')); 
...
assert(scope.includes("const CURRENT_RELEASE_VERSION = '17.1.4'"));
...
]) assert(read(file).includes('17.1.4'), `${file} carries current version 17.1.4`);
for (const file of [
...
console.log('17.1.4 Release-Gate Browser Stability Repair validation passed; this does not certify the release.');
