...
const inherited = childProcess.spawnSync(process.execPath, ['scripts/validate-16-0-233.js'], {
  ...
  env: {
    ...process.env,
    CHAOS_VALIDATION_VERSION: '16.0.235',
    CHAOS_VALIDATION_RELEASE_TITLE: 'Mobile Schedule PDF Delivery Repair',
    CHAOS_VALIDATION_SCRIPT: 'scripts/validate-16-0-235.js'
  }
});
...
assert(pkg.version === '16.0.235' && lock.version === '16.0.235' && lock.packages?.['']?.version === '16.0.235', 'package and lock versions are 16.0.235');
assert(version.version === '16.0.235' && version.build === '16.0.235' && version.releaseTitle === 'Mobile Schedule PDF Delivery Repair', 'public version metadata identifies the mobile PDF repair');
assert(read('api/_version.js').includes("APP_VERSION = '16.0.235'") && read('api/_version.js').includes("SECURITY_SCHEMA_VERSION = '16.0.235'"), 'API reports 16.0.235');
assert(read('src/core/appCore.js').includes("CURRENT_VERSION = '16.0.235'"), 'active client reports 16.0.235');
assert(read('src/core/customerHelpKnowledge.js').includes("CUSTOMER_HELP_VERSION = '16.0.235'") && read('src/core/customerHelpKnowledge.cjs').includes("CUSTOMER_HELP_VERSION = '16.0.235'"), 'customer Help version mirrors remain synchronized');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-235.js' && pkg.scripts['validate:16.0.235'] === 'node scripts/validate-16-0-235.js', '16.0.235 validator is wired');
...
assert(pkg.scripts['test:new-implementations']?.startsWith('npm run validate:16.0.235'), 'new-implementation command uses the 16.0.235 validator');
...
if (failures) { console.error(`16.0.235 source validation failed with ${failures} failure(s).`); process.exit(1); }
console.log('16.0.235 source validation passed.');
