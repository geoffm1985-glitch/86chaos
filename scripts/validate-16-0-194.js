...
const repairPack = json('scripts/repair-regression-pack-16.0.194.json');
const browserPack = json('scripts/86chaos-release-gate/repair-regression-16.0.194.json');

assert(pkg.version === '16.0.194', 'package.json version is 16.0.194');
assert(lock.version === '16.0.194' && lock.packages?.['']?.version === '16.0.194', 'package-lock root version is 16.0.194');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-194.js', 'test:source points to 16.0.194 validator');
assert(pkg.scripts['test:repair-16-0-194'] === 'npm run test:repair-current', 'test:repair-16-0-194 exists');
assert(version.version === '16.0.194' && version.build === '16.0.194', 'public version/build are 16.0.194');
assert(/CURRENT_VERSION = '16\.0\.194'/.test(appCore), 'app core CURRENT_VERSION is 16.0.194');
assert(/APP_VERSION = '16\.0\.194'/.test(read('api/_version.js')), 'api version is 16.0.194');
assert(read('RUN_86CHAOS_FULL_TEST_SUITE.ps1').includes('test-16-0-194-targeted.cjs'), 'full suite uses 16.0.194 targeted validator');
...
assert(exists('api/app-only-package-hygiene-16-0-194.test.cjs'), '16.0.194 app-only package hygiene test exists');
...
assert(exists('api/app-only-package-hygiene-16-0-194.test.cjs'), '16.0.194 package hygiene test exists');
...
assert(exists('scripts/test-16-0-194-targeted.cjs'), '16.0.194 targeted test script exists');
assert(repairPack.version === '16.0.194', 'repair local manifest version is 16.0.194');
assert(browserPack.version === '16.0.194', 'repair browser manifest version is 16.0.194');
assert(read('scripts/run-repair-regression-pack.cjs').includes('repair-regression-pack-16.0.194.json'), 'repair local pointer uses 16.0.194');
assert(read('scripts/run-repair-browser-regression.cjs').includes('repair-regression-16.0.194.json'), 'repair browser pointer uses 16.0.194');


assert(exists('scripts/86chaos-release-gate/app-only-package-hygiene.cjs'), 'shared package hygiene helper exists');
assert(exists('api/app-only-package-hygiene-16-0-194.test.cjs'), '16.0.194 working-checkout package hygiene regression exists');
...
assert(read('scripts/test-16-0-194-targeted.cjs').includes('CHAOS_ALLOW_GENERATED_TEST_ARTIFACTS'), 'targeted runner grants package hygiene the same narrow current-run artifact allowance');
if (failures) { console.error(`\n16.0.194 source validation failed with ${failures} issue(s).`); process.exit(1); }
console.log('\n16.0.194 source validation passed.');
