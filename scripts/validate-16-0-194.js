...
assert(version.version === '16.0.194' && version.build === '16.0.194', 'public/version.json version/build are 16.0.194');
assert(version.releaseTitle === 'Failed-Only Runner Launch Repair', '16.0.194 release title identifies failed-only runner launch repair');
assert(appCore.includes("CURRENT_VERSION = '16.0.194'"), 'app core CURRENT_VERSION is 16.0.194');
...
assert(fs.existsSync(path.join(root, 'scripts/validate-16-0-194.js')), '16.0.194 validator exists');
...
if (failures) {
  console.error(`16.0.194 source validation failed with ${failures} failure(s).`);
  process.exit(1);
}
