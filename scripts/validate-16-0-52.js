...
assert(pkg.version === '16.0.52', 'package.json version is 16.0.52');
assert(lock.version === '16.0.52' && lock.packages?.['']?.version === '16.0.52', 'package-lock.json version is 16.0.52');
assert(version.version === '16.0.52' && version.build === '16.0.52', 'public/version.json is 16.0.52');
assert(appCore.includes("CURRENT_VERSION = '16.0.52'"), 'CURRENT_VERSION is 16.0.52');
assert(apiVersion.includes("APP_VERSION = '16.0.52'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.52'"), 'API version constants are centralized at 16.0.52');
assert(pkg.scripts?.['test:source'] === 'node scripts/validate-16-0-52.js', 'package scripts point at the 16.0.52 source validator');
...
assert((styles.includes('16.0.51 mobile Schedule Builder readability') || styles.includes('16.0.52 failed-only release-gate repairs')) && styles.includes('scheduled-hours-tracker-table'), 'mobile schedule times and hours tracker labels stay readable and horizontal');
...
assert(!appJs.includes('fcmTokens: arrayUnion(currentToken)') && appJs.includes('primaryTokenMissing') && appJs.includes("pushTokenDedupeVersion: '16.0.52'") && appJs.includes('writePushProfilePatch'), 'push token sync repairs the primary token and pushDevices without writing the disallowed browser fcmTokens array');
...
assert(read('tests/86chaos-release-gate/20-firebase-cost-idempotency.spec.cjs').includes('openRouteInApp'), 'Firebase idempotency test uses in-app route changes instead of full reload listener churn');
...
if (failures) {
  console.error(`16.0.52 source validator failed with ${failures} issue(s).`);
  process.exit(1);
}
console.log('16.0.52 source validator passed. This is a source guard only. It does not replace unit, emulator, build, browser, or staging notification tests.');

// 16.0.52 specific guardrails
try {
  const app = require('fs').readFileSync('src/App.js', 'utf8');
  if (/\[activeTabState,\s*isMenuOpen,\s*isWorkspaceSwitcherOpen\]/.test(app)) {
    console.error('16.0.52 guard failed: accessibility observer must not reference menu state before hook initialization.');
    process.exitCode = 1;
  }
  const sw = require('fs').readFileSync('public/firebase-messaging-sw.js', 'utf8');
  if (!sw.includes('/notification-badge.png')) {
    console.error('16.0.52 guard failed: notification badge fallback missing.');
    process.exitCode = 1;
  }
} catch (error) {
  console.error('16.0.52 guard failed:', error.message);
  process.exitCode = 1;
}
