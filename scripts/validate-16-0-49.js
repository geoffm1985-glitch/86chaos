...
assert(pkg.version === '16.0.49', 'package.json version is 16.0.49');
assert(lock.version === '16.0.49' && lock.packages?.['']?.version === '16.0.49', 'package-lock.json version is 16.0.49');
assert(version.version === '16.0.49' && version.build === '16.0.49', 'public/version.json is 16.0.49');
assert(appCore.includes("CURRENT_VERSION = '16.0.49'"), 'CURRENT_VERSION is 16.0.49');
assert(apiVersion.includes("APP_VERSION = '16.0.49'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.49'"), 'API version constants are centralized at 16.0.49');
assert(pkg.scripts?.['test:source'] === 'node scripts/validate-16-0-49.js', 'package scripts point at the 16.0.49 source validator');
...
assert((styles.includes('16.0.47 release-gate accessibility') || styles.includes('16.0.48 release-gate accessibility') || styles.includes('16.0.49 release-gate accessibility')) && appJs.includes('describeControl'), 'accessibility/tap-target hardening is present');
...
assert(appJs.includes('fcmTokens: arrayUnion(currentToken)') && appJs.includes('primaryTokenMissing') && appJs.includes("pushTokenDedupeVersion: '16.0.49'"), 'push token sync repairs missing primary token and stores canonical token array');

if (failures) {
  console.error(`16.0.49 source validator failed with ${failures} issue(s).`);
  process.exit(1);
}
console.log('16.0.49 source validator passed. This is a source guard only. It does not replace unit, emulator, build, browser, or staging notification tests.');

// 16.0.49 specific guardrails
try {
  const app = require('fs').readFileSync('src/App.js', 'utf8');
  if (/\[activeTabState,\s*isMenuOpen,\s*isWorkspaceSwitcherOpen\]/.test(app)) {
    console.error('16.0.49 guard failed: accessibility observer must not reference menu state before hook initialization.');
    process.exitCode = 1;
  }
  const sw = require('fs').readFileSync('public/firebase-messaging-sw.js', 'utf8');
  if (!sw.includes('/notification-badge.png')) {
    console.error('16.0.49 guard failed: notification badge fallback missing.');
    process.exitCode = 1;
  }
} catch (error) {
  console.error('16.0.49 guard failed:', error.message);
  process.exitCode = 1;
}
