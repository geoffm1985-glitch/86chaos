...
assert(lock.version === '16.0.197' && lock.packages?.['']?.version === '16.0.197', 'package-lock root versions are 16.0.197');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-197.js', 'test:source points to 16.0.197 validator');
assert(version.version === '16.0.197' && version.build === '16.0.197', 'public/version.json version/build are 16.0.197');
assert(version.releaseTitle === 'Failed-Only Browser Gate Final Form/Surface Repair', '16.0.197 release title identifies failed-only browser gate repair');
assert(appCore.includes("CURRENT_VERSION = '16.0.197'"), 'app core CURRENT_VERSION is 16.0.197');
assert(apiVersion.includes("APP_VERSION = '16.0.197'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.197'"), 'api version reports 16.0.197');
...
assert(sinceRunner.includes('api/public-icon-assets-16-0-195.test.cjs'), 'targeted runner includes 16.0.197 failed-only browser gate regression');
...
assert(exhaustiveHelper.includes('locatorFromFormDescriptor') && exhaustiveHelper.includes('restoreAndObserve(page,'), '16.0.197 uses stable form-control descriptors for failed-only form probes');
assert(managementFeature.includes('aria-label={tab.label}') && managementFeature.includes('title={tab.label}'), '16.0.197 exposes full System Administrator mobile tab labels to release-gate coverage');
assert(managementFeature.includes('aria-label={label} title={label} onClick={() => setSubTab(id)}'), '16.0.197 exposes Back Office nested tab labels to release-gate coverage');
assert(scheduleFeature.includes('aria-label={label} title={label} onClick={() => setActiveTool(id)}'), '16.0.197 exposes Schedule Builder nested tool labels to release-gate coverage');

if (failures) {
  console.error(`16.0.197 source validation failed with ${failures} failure(s).`);
  process.exit(1);
}
console.log('16.0.197 source validation passed.');
