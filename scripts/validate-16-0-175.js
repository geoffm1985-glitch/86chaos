...
assert(pkg.version === '16.0.175', 'package.json version is 16.0.175');
assert(lock.version === '16.0.175' && lock.packages?.['']?.version === '16.0.175', 'package-lock root versions are 16.0.175');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-175.js', 'test:source points to 16.0.175 validator');
assert(version.version === '16.0.175' && version.build === '16.0.175', 'public/version.json version/build are 16.0.175');
assert(version.releaseTitle === 'Windows Full-Suite Integrity and Dependency Security Repair', 'release title is correct');
assert(appCore.includes("CURRENT_VERSION = '16.0.175'"), 'app core CURRENT_VERSION is 16.0.175');
assert(apiVersion.includes("APP_VERSION = '16.0.175'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.175'"), 'api version reports 16.0.175');
...
assert(fs.existsSync(path.join(root, 'scripts/repair-regression-pack-16.0.175.json')), 'repair regression pack manifest exists');
assert(fs.existsSync(path.join(root, 'scripts/86chaos-release-gate/repair-regression-16.0.175.json')), 'repair browser regression manifest exists');
...
const repairManifest = json('scripts/86chaos-release-gate/repair-regression-16.0.175.json');
...
assert(fs.existsSync(path.join(root, 'scripts/repair-regression-pack-16.0.175.json')), '16.0.175 repair local manifest exists');
assert(fs.existsSync(path.join(root, 'scripts/86chaos-release-gate/repair-regression-16.0.175.json')), '16.0.175 repair browser manifest exists');
assert(read('scripts/run-repair-regression-pack.cjs').includes('repair-regression-pack-16.0.175.json'), 'repair pack runner points to 16.0.175 manifest');
assert(read('scripts/run-repair-browser-regression.cjs').includes('repair-regression-16.0.175.json'), 'repair browser runner points to 16.0.175 manifest');
const repairLocal172 = json('scripts/repair-regression-pack-16.0.175.json');
assert(repairLocal172.version === '16.0.175', 'repair local manifest version is 16.0.175');
...
const repairBrowser172 = json('scripts/86chaos-release-gate/repair-regression-16.0.175.json');
assert(repairBrowser172.version === '16.0.175', 'repair browser manifest version is 16.0.175');
...
// 16.0.175 staff login email edit and full-suite reporting repair.
...
const repair173 = json('scripts/repair-regression-pack-16.0.175.json');
assert((repair173.localCommands || []).some(entry => entry.group === 'Staff Email Editing'), '16.0.175 repair local manifest includes Staff Email Editing');
const repairBrowser173 = json('scripts/86chaos-release-gate/repair-regression-16.0.175.json');
assert((repairBrowser173.selected || []).some(row => row.specPath === 'tests/e2e/staff-email-edit.spec.cjs' && row.leafTitle === 'Manager changes an employee login email and the new email authenticates' && row.project === 'chromium'), '16.0.175 repair browser manifest includes staff email chromium identity');
assert((repairBrowser173.selected || []).some(row => row.specPath === 'tests/e2e/staff-email-edit.spec.cjs' && row.project === 'mobile-chromium'), '16.0.175 repair browser manifest includes staff email mobile identity');
...
assert(fullSuite.includes('86chaos-FULL-SUITE-UPLOAD-ME-16.0.175'), 'full-suite upload ZIP is versioned and finalized');
...
assert(fs.existsSync(path.join(root, 'scripts/test-16-0-174-targeted.cjs')), '16.0.175 targeted test script exists');

if (failures) { console.error(`\n${failures} validation check(s) failed.`); process.exit(1); }
console.log('\n16.0.175 source validation passed.');
