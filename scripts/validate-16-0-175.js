...
assert(version.releaseTitle === 'Staff Email Identity Guard And Targeted Regression Harness', 'release title is correct');
...
assert(!fs.existsSync(path.join(root, 'scripts/validate-16-0-174.js')), 'previous validator was replaced');
...
assert(identityContaminationTest.includes('preserves target name when caller identity is accidentally submitted') && identityContaminationTest.includes('P Test') && identityContaminationTest.includes('Geoff Test'), '16.0.175 identity-contamination regression test exists');
...
assert(staffEmailSourceTest.includes('Firebase login email') && staffEmailSourceTest.includes('auth.updateUser') && monthPrintSourceTest.includes('generated print document'), '16.0.175 source regression tests exist for staff email and month print repairs');
...
assert(posthogSourceTest.includes('PostHog client is environment gated') && posthogSourceTest.includes('Vercel CSP allows PostHog'), '16.0.175 PostHog regression tests exist');
...
assert(scheduleIdentitySourceTest.includes('Month View dedupes duplicate published shift documents') && scheduleIdentitySourceTest.includes('resolveSchedulePersonForShift uses shift employee aliases without Firestore doc id'), '16.0.175 schedule identity regression tests exist');
