...
const commands = [
  ...
  { group: 'mobile layout Playwright smoke', command: 'node scripts/86chaos-release-gate/run-observable-command.cjs --label "Mobile layout Playwright smoke" --heartbeat 20 --timeout 180 -- node node_modules/@playwright/test/cli.js test --config=playwright.layout.config.cjs', required: true },
