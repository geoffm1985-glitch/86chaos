...
const assert = (condition, message) => {
  if (!condition) {
    console.error(`16.0.73 source validation failed: ${message}`);
    process.exitCode = 1;
  }
};
...
assert(pkg.version === '16.0.73', 'package.json version is 16.0.73');
assert(lock.version === '16.0.73' && lock.packages?.['']?.version === '16.0.73', 'package-lock root version is 16.0.73');
assert(version.version === '16.0.73' && version.build === '16.0.73', 'public version/build is 16.0.73');
assert(appCore.includes("CURRENT_VERSION = '16.0.73'"), 'appCore CURRENT_VERSION is 16.0.73');
assert(apiVersion.includes("APP_VERSION = '16.0.73'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.73'"), 'API version constants are 16.0.73');
assert(pkg.scripts?.['test:source'] === 'node scripts/validate-16-0-73.js', 'test:source points at 16.0.73 validator');
...
if (!process.exitCode) console.log('16.0.73 source validator passed.');
