...
const assert = (condition, message) => {
  if (!condition) {
    console.error(`16.0.83 source validation failed: ${message}`);
    process.exitCode = 1;
  }
};
...
assert(pkg.version === '16.0.83', 'package.json version is 16.0.83');
assert(lock.version === '16.0.83' && lock.packages?.['']?.version === '16.0.83', 'package-lock root version is 16.0.83');
assert(pkg.scripts?.['test:source'] === 'node scripts/validate-16-0-83.js', 'test:source points at 16.0.83 validator');
assert(version.version === '16.0.83' && version.build === '16.0.83', 'public version/build is 16.0.83');
assert(appCore.includes("CURRENT_VERSION = '16.0.83'"), 'appCore CURRENT_VERSION is 16.0.83');
assert(apiVersion.includes("APP_VERSION = '16.0.83'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.83'"), 'API version constants are 16.0.83');
...
// 16.0.83 release-gate request-header regression checks.
...
// 16.0.83 Schedule Builder readability checks.
...
assert(styles.includes('16.0.82: Schedule Builder time chip readability'), 'styles preserve the 16.0.82 schedule chip readability override');
...
// 16.0.83 admin employee delete permission regression checks.
const management = read('src/features/management.jsx');
assert(management.includes('/api/delete-user uses the Firebase Admin SDK to remove both the Auth'), 'roster super-delete explains server-side Admin SDK deletion as source of truth');
assert(management.includes('Do not issue a second client-side delete'), 'roster super-delete avoids second forbidden client-side users delete');
assert(management.includes('/api/delete-user already deletes the Database Profile with the Admin SDK'), 'global user delete relies on the server API for profile deletion');
assert(management.includes('would show a false permission error after the server delete succeeded'), 'global delete documents the previous false permission-error failure mode');
assert(management.includes('The endpoint deletes Auth and Firestore profiles with the Admin SDK'), 'bulk fallback deletion uses API-owned user/profile removal');
assert(!management.includes('await deleteDoc(doc(db, "users", u.id));'), 'management UI no longer performs forbidden client-side users delete with double-quoted path');
assert(!management.includes("await deleteDoc(doc(db, 'users', u.id));"), 'management UI no longer performs forbidden client-side users delete with single-quoted path');
assert(management.includes("if (!deleteResponse.ok || deleteResult?.ok === false) throw new Error(deleteResult?.error || 'User deletion failed.');") || management.includes("if (!response.ok || result?.ok === false) throw new Error(result?.error || 'User deletion failed.');"), 'delete handlers still surface backend API failures instead of pretending deletes succeeded');

if (!process.exitCode) console.log('16.0.83 source validator passed.');
