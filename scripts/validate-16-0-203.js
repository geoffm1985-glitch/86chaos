...
assert(lock.version === '16.0.203' && lock.packages?.['']?.version === '16.0.203', 'package-lock root versions are 16.0.203');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-203.js', 'test:source points to 16.0.203 validator');
assert(version.version === '16.0.203' && version.build === '16.0.203', 'public/version.json version/build are 16.0.203');
assert(version.releaseTitle === 'Release Gate Failed+New Manifest Repair', '16.0.203 release title identifies failed+new manifest repair');
assert(appCore.includes("CURRENT_VERSION = '16.0.203'"), 'app core CURRENT_VERSION is 16.0.203');
assert(apiVersion.includes("APP_VERSION = '16.0.203'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.203'"), 'api version reports 16.0.203');
...
assert(exhaustiveHelper.includes("`${parts.join('')}:visible`") && exhaustiveHelper.includes('formControlSelectorFor'), 'form descriptor reconstruction preserves visible structural selector semantics');
assert(scheduleOracleSpec.includes('timeout: 45000') && scheduleOracleSpec.includes('Schedule Builder should hydrate current-run QA staff/events'), 'schedule oracle waits for deterministic data hydration');
...
if (failures) {
  console.error(`16.0.203 source validation failed with ${failures} failure(s).`);
  process.exit(1);
}
console.log('16.0.203 source validation passed.');
