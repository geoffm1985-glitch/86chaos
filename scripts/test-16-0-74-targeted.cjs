...
function assert(condition, message) {
  if (!condition) {
    console.error(`16.0.74 targeted test failed: ${message}`);
    process.exitCode = 1;
  }
}

assert(version.version === '16.0.74', 'version.json reports 16.0.74');
...
if (!process.exitCode) console.log('16.0.74 targeted schedule publish/delete integrity tests passed.');

assert(schedule.indexOf('const activeLocalDeleteKeySet') < schedule.indexOf('const isMyPublishedShift'), 'active local delete markers are initialized before My Schedule filter uses them');
assert(!schedule.includes('activeLocalDeleteKeysSet'), 'schedule does not reference the misspelled activeLocalDeleteKeysSet runtime variable');
