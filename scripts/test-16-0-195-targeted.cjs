...
const commands = [
  ['node', 'scripts/validate-16-0-195.js'],
  ...
  ['node', '--test', 'api/app-only-package-hygiene-16-0-195.test.cjs']
];
for (const cmd of commands) {
  console.log(`[16.0.195 targeted] ${cmd.join(' ')}`);
  const childEnv = { ...process.env };
  if (cmd.includes('api/app-only-package-hygiene-16-0-195.test.cjs')) childEnv.CHAOS_ALLOW_GENERATED_TEST_ARTIFACTS = '1';
  const result = spawnSync(cmd[0], cmd.slice(1), { stdio: 'inherit', shell: process.platform === 'win32', env: childEnv });
  if (result.status !== 0) process.exit(result.status || 1);
}
console.log('16.0.195 targeted repair checks passed.');
