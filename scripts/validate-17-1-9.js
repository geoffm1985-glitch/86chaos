...
assert.equal(pkg.version, '17.1.9');
assert.equal(lock.version, pkg.version);
assert.equal(lock.packages[''].version, pkg.version);
assert.equal(version.version, pkg.version);
assert.equal(version.build, pkg.version);
assert.equal(version.releaseTitle, '86Voice Panel-First Reliability Repair');
assert.equal(pkg.scripts['test:source'], 'node scripts/validate-17-1-9.js');
assert.equal(pkg.scripts['validate:17.1.9'], 'node scripts/validate-17-1-9.js');
assert(pkg.scripts['test:repair:17.1.9']?.includes('test:current-release-targeted'));
assert(pkg.scripts['test:current-release-targeted']?.includes('api/mobile-voice-panel-first-17-1-9.test.cjs'));
assert(pkg.scripts['test:current-release-targeted']?.includes('api/voice-command-transcription-model-17-1-9.test.cjs'));
...
assert(pkg.scripts['test:release:fast']?.includes('validate:17.1.9')); 
...
assert(scope.includes("const CURRENT_RELEASE_VERSION = '17.1.9'"));
assert(scope.includes('22-release-gate-mic-delta-repair.spec.cjs'));
...
]) assert(read(file).includes('17.1.9'), `${file} carries current version 17.1.9`);
for (const file of [
...
assert(deltaRepair.includes('keeps recognition on an explicit user microphone gesture without a timer'));
assert(voiceCommon.includes('startListening({ autoStart: true, fromUserGesture: true })'));
assert(!/pendingVoiceStartTimerRef\.current = setTimeout[\s\S]{0,250}startListening/.test(voiceCommon));
...
assert(voice1717.includes('getUserMedia'));
assert(voice1717Browser.includes('concept17-mobile-voice-button'));
assert(voice1717Browser.includes('permissionRequests'));
...
const voice1718 = read('api/mobile-voice-resilient-capture-17-1-8.test.cjs');
const voice1718Browser = read('tests/86chaos-new-implementations/24-mobile-voice-resilient-capture.spec.cjs');
const voiceRoute1718 = read('api/voice-command.js');
const common1718 = read('src/components/common.jsx');
assert(voice1718.includes('MediaRecorder'));
assert(voice1718Browser.includes('MockMediaRecorder'));
assert(voiceRoute1718.includes("mode === 'transcribe'"));
assert(voiceRoute1718.includes("providerAudioMimeType = audioMimeType.split(';')[0]"));
assert(voiceRoute1718.includes('inlineData: { mimeType: providerAudioMimeType, data: audioBase64 }'));
assert(common1718.includes('startRecordedVoice'));
assert(common1718.includes('data-testid="voice-command-status"'));

const voice1719 = read('api/mobile-voice-panel-first-17-1-9.test.cjs');
const voiceRoute1719 = read('api/voice-command.js');
const app1719 = read('src/App.js');
const common1719 = read('src/components/common.jsx');
assert(voice1719.includes('opens the 86Voice panel without starting capture'));
assert(app1719.includes('controller?.openPanel'));
assert(app1719.includes('chaos:voice-open-panel'));
const openDock1719Start = common1719.indexOf('const openDock = () => {');
const openDock1719End = common1719.indexOf('const openDockAndListen', openDock1719Start);
assert(openDock1719Start >= 0 && openDock1719End > openDock1719Start);
assert(!common1719.slice(openDock1719Start, openDock1719End).includes('startListening'));
assert(common1719.includes('data-testid="voice-command-error"'));
assert(common1719.includes('Tap Start Listening when you are ready.'));
const voiceModel1719Start = voiceRoute1719.indexOf('function voiceTranscriptionModel()');
const voiceModel1719End = voiceRoute1719.indexOf('const VOICE_AUDIO_MAX_BYTES', voiceModel1719Start);
const voiceModel1719 = voiceRoute1719.slice(voiceModel1719Start, voiceModel1719End);
assert(voiceModel1719.includes('VOICE_TRANSCRIBE_GEMINI_MODEL'));
assert(!voiceModel1719.includes('VOICE_GEMINI_MODEL'));
assert(voiceModel1719.includes('gemini-3.5-flash-lite'));
assert(voiceRoute1719.includes('VOICE_TRANSCRIPTION_PROVIDER_REJECTED'));
assert(voiceRoute1719.includes('diagnosticCode'));
assert(conceptCss.includes('17.1.9 panel-first 86Voice repair'));
assert(exists('RELEASE_17_1_9.md'));

console.log('17.1.9 86Voice Panel-First Reliability Repair validation passed; this does not certify the release.');
