...
assert.equal(pkg.version, '17.1.0');
assert.equal(lock.version, pkg.version);
assert.equal(lock.packages[''].version, pkg.version);
assert.equal(version.version, pkg.version);
assert.equal(version.build, pkg.version);
assert.equal(version.releaseTitle, 'Full-App Concept 1 Responsive Redesign');
assert.equal(pkg.scripts['test:source'], 'node scripts/validate-17-1-0.js');
assert.equal(pkg.scripts['validate:17.1.0'], 'node scripts/validate-17-1-0.js');
assert(pkg.scripts['test:repair:17.1.0']?.includes('test:current-release-targeted'));
...
assert(pkg.scripts['test:release:fast']?.includes('validate:17.1.0'));
...
assert(exists('RELEASE_17_1_0.md'));
...
assert(scope.includes("const CURRENT_RELEASE_VERSION = '17.1.0'"));
...
]) assert(read(file).includes('17.1.0'), `${file} carries current version 17.1.0`);
for (const file of [
...
console.log('17.1.0 Full-App Concept 1 Responsive Redesign validation passed; this does not certify the release.');
