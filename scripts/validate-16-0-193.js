...
assert(lock.version === '16.0.193' && lock.packages?.['']?.version === '16.0.193', 'package-lock root versions are 16.0.193');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-193.js', 'test:source points to 16.0.193 validator');
assert(version.version === '16.0.193' && version.build === '16.0.193', 'public/version.json version/build are 16.0.193');
assert(version.releaseTitle === 'Failed-Only Runner Recovery', '16.0.193 release title identifies failed-only runner recovery');
assert(appCore.includes("CURRENT_VERSION = '16.0.193'"), 'app core CURRENT_VERSION is 16.0.193');
assert(apiVersion.includes("APP_VERSION = '16.0.193'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.193'"), 'api version reports 16.0.193');
assert(fs.existsSync(path.join(root, 'scripts/validate-16-0-193.js')), '16.0.193 validator exists');
...
assert(failedOnlyPrepare.includes('reported-failed-only-20260822-173450.json'), 'failed-only runner bundles the authoritative 16-identity baseline');
...
if (failures) {
  console.error(`16.0.193 source validation failed with ${failures} failure(s).`);
  process.exit(1);
}
console.log('16.0.193 source validation passed.');
