...
const preflight = readJson(path.join(outDir, '86chaos-full-audit-preflight-report.json'));
const pwStarted = fs.existsSync(path.join(outDir, '86chaos-full-audit-playwright-started.marker'));
const pwFinished = fs.existsSync(path.join(outDir, '86chaos-full-audit-playwright-finished.marker'));
let text = '';
...
text += `Playwright Started: ${pwStarted}\n`;
text += `Playwright Finished: ${pwFinished}\n`;
...
if (pw) {
  ...
  text += section('SUMMARY', [
    'No Playwright JSON report found for this run.',
    `Playwright started marker: ${pwStarted}`,
    `Playwright finished marker: ${pwFinished}`,
    'This usually means the command stopped before browser tests, or the Playwright process was interrupted before writing JSON.'
  ].join('\n'));
}

if (preflight) text += section('PREFLIGHT REPORT', JSON.stringify(preflight, null, 2));
