...
const inherited=cp.spawnSync(process.execPath,['scripts/validate-16-0-233.js'],{
  ...
  env:{...process.env,CHAOS_VALIDATION_VERSION:'16.0.241',CHAOS_VALIDATION_RELEASE_TITLE:'Cross-Platform Validator Hash Repair',CHAOS_VALIDATION_SCRIPT:'scripts/validate-16-0-241.js'}
});
...
assert(pkg.version==='16.0.241'&&lock.version==='16.0.241'&&lock.packages?.['']?.version==='16.0.241','package and lock versions are 16.0.241');
assert(version.version==='16.0.241'&&version.build==='16.0.241'&&version.releaseTitle==='Cross-Platform Validator Hash Repair','public version metadata identifies the validator repair');
assert(read('api/_version.js').includes("APP_VERSION = '16.0.241'")&&read('api/_version.js').includes("SECURITY_SCHEMA_VERSION = '16.0.241'"),'API reports 16.0.241');
assert(read('src/core/appCore.js').includes("CURRENT_VERSION = '16.0.241'"),'active client reports 16.0.241');
assert(read('src/core/customerHelpKnowledge.js').includes("CUSTOMER_HELP_VERSION = '16.0.241'")&&read('src/core/customerHelpKnowledge.cjs').includes("CUSTOMER_HELP_VERSION = '16.0.241'"),'customer Help version mirrors remain synchronized');
assert(pkg.scripts['test:source']==='node scripts/validate-16-0-241.js'&&pkg.scripts['validate:16.0.241']==='node scripts/validate-16-0-241.js','16.0.241 validator is wired');
assert(pkg.scripts['test:new-implementations']?.startsWith('npm run validate:16.0.241'),'new-implementation command uses the 16.0.241 validator');
...
const qaEnv={CHAOS_ALLOW_MUTATION:'true',CHAOS_RELEASE_GATE_RUN_ID:'validator-16-0-241',SYSTEM_ADMIN_EMAIL:'86chaos.qa.system-admin.20260729-1302@example.test',OWNER_EMAIL:'86chaos.qa.owner.20260729-1302@example.test',MANAGER_EMAIL:'86chaos.qa.manager.20260729-1302@example.test',STAFF_EMAIL:'86chaos.qa.staff.20260729-1302@example.test'};
for(const host of ['testing.86chaos.com','experimental.86chaos.com']){
  const target=targets.validateReleaseTarget({appUrl:`https://${host}`,expectedProjectSlug:'86chaos',expectedVersion:'16.0.241',sourceVersion:'16.0.241',deployedVersion:'16.0.241'});
...
for(const host of ['app.86chaos.com','86chaos.com','www.86chaos.com','staging.86chaos.com']){
  const target=targets.validateReleaseTarget({appUrl:`https://${host}`,expectedVersion:'16.0.241',sourceVersion:'16.0.241',deployedVersion:'16.0.241'});
...
assert(diff.status===0,'Git diff evidence is available for the 16.0.241 repair');
...
if(failures){console.error(`16.0.241 source validation failed with ${failures} failure(s).`);process.exit(1);}
console.log('16.0.241 source validation passed.');
