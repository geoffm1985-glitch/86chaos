...
assert(pkg.version === '16.0.11', 'package.json version is 16.0.11');
assert(lock.version === '16.0.11' && lock.packages?.['']?.version === '16.0.11', 'package-lock.json version is 16.0.11');
assert(version.version === '16.0.11' && version.build === '16.0.11', 'public/version.json is 16.0.11');
assert(appCore.includes("CURRENT_VERSION = '16.0.11'"), 'CURRENT_VERSION is 16.0.11');
assert(pkg.scripts?.test === 'node scripts/validate-16-0-11.js', 'npm test runs 16.0.11 validator');
assert(pkg.scripts?.['test:ci'] === 'node scripts/validate-16-0-11.js', 'npm test:ci runs 16.0.11 validator');

// 16.0.11 mobile keyboard fix: modal focus effect must not restart every typed character.
...
if (failures) {
  console.error(`16.0.11 mobile modal keyboard focus validator failed with ${failures} issue(s).`);
  process.exit(1);
}
console.log('16.0.11 mobile modal keyboard focus validator passed.');
