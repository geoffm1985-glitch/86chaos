...
const expected = '17.0.23';
const releaseTitle = 'Node Test Reporter Compatibility Repair';
...
for (const file of ['src/core/appCore.js', 'api/_version.js', 'api/_pos-bridge-config.js']) assert(read(file).includes("'17.0.23'"), `${file} current release metadata`);
for (const directory of ['src', 'api', 'scripts', 'test-tools', 'tests']) assert(fs.statSync(directory).isDirectory(), `${directory} is required`);
for (const file of ['firebase.json', 'vercel.json', 'firestore.rules', 'storage.rules', 'RUN_86CHAOS_PLAY_STORE_RELEASE_GATE.ps1', 'RUN_86CHAOS_UPDATE_TEST_DEPLOY.ps1', 'RELEASE_TEST_POLICY.md', 'RELEASE_17_0_23.md', '.gitignore', 'release-source-manifest.json']) assert(fs.statSync(file).isFile(), `${file} is required`);
...
assert(updater.includes("[string]$ExpectedVersion = '17.0.23'"));
...
assert(pkg.scripts['test:repair:17.0.23'].includes('api/release-gate-maturity-16-0-207.test.cjs'));
assert(pkg.scripts['test:repair:17.0.23'].includes('api/release-gate-certification-integrity-17-0-19.test.cjs'));
assert(pkg.scripts['test:repair:17.0.23'].includes('api/failure-extractor-bounded-17-0-20.test.cjs'));
assert(pkg.scripts['test:release:fast'].includes('validate:17.0.23'));
assert(pkg.scripts['test:new-implementations'].includes('validate:17.0.23'));
...
assert(pkg.scripts['test:repair:17.0.23'].includes('api/release-gate-harness-env-isolation-17-0-21.test.cjs'));
...
assert(pkg.scripts['test:repair:17.0.23'].includes('run-hermetic-repair-tests.cjs'));
assert(pkg.scripts['test:repair:17.0.23'].includes('api/release-gate-local-regression-env-isolation-17-0-22.test.cjs'));
...
assert(pkg.scripts['test:repair:17.0.23'].includes('api/release-gate-node-test-reporter-compatibility-17-0-23.test.cjs'));
assert(pkg.scripts['test:hostile:contracts'].includes('api/release-gate-node-test-reporter-compatibility-17-0-23.test.cjs'));
assert(json('test-tools/regressions/registry.json').defects.some(row => row.defectId === 'RG-NODE-TEST-REPORTER-COMPATIBILITY-1723' && row.fixedVersion === expected));
console.log('17.0.23 source validation passed; this does not certify the release.');
