...
assert(pkg.version === '16.0.229', 'package.json version is 16.0.229');
assert(lock.version === '16.0.229' && lock.packages?.['']?.version === '16.0.229', 'package-lock root versions are 16.0.229');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-229.js', 'test:source points to the 16.0.229 validator');
assert(version.version === '16.0.229' && version.build === '16.0.229', 'public version/build are 16.0.229');
assert(version.releaseTitle === 'Restaurant Helper Runtime Repair', 'release title identifies the restaurant helper runtime repair');
assert(apiVersion.includes("APP_VERSION = '16.0.229'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.229'"), 'API reports 16.0.229');
assert(appCore.includes("CURRENT_VERSION = '16.0.229'"), 'app reports 16.0.229');
...
if (failures) {
  console.error(`16.0.229 source validation failed with ${failures} failure(s).`);
  process.exit(1);
}
console.log('16.0.229 source validation passed.');
