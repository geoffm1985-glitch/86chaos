...
assert(pkg.version === '16.0.168', 'package.json version is 16.0.168');
assert(lock.version === '16.0.168' && lock.packages?.['']?.version === '16.0.168', 'package-lock root versions are 16.0.168');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-168.js', 'test:source points to 16.0.168 validator');
assert(version.version === '16.0.168' && version.build === '16.0.168', 'public/version.json version/build are 16.0.168');
assert(version.releaseTitle === 'System Administrator Global User Roster Repair', 'release title is correct');
assert(appCore.includes("CURRENT_VERSION = '16.0.168'"), 'app core CURRENT_VERSION is 16.0.168');
assert(apiVersion.includes("APP_VERSION = '16.0.168'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.168'"), 'api version reports 16.0.168');
assert(!fs.existsSync(path.join(root, 'scripts/validate-16-0-167.js')), 'previous validator was replaced');
...
// System Administrator global people roster repair.
const management = read('src/features/management.jsx');
const peopleRoute = read('api/system-admin/people.js');
const safeRows = read('api/system-admin-safe-rows.cjs');
assert(fs.existsSync(path.join(root, 'api/system-admin/people.js')), 'System Administrator people endpoint exists');
assert(peopleRoute.includes("authorize(req, app, { allowTenantAdmin: false, allowCrossProjectMaster: true })") && peopleRoute.includes('ctx.isSuperAdmin !== true'), 'people endpoint requires server System Administrator authority');
assert(peopleRoute.includes("db.collection('users')") && peopleRoute.includes('FieldPath.documentId()') && peopleRoute.includes('startAfter(cursor)'), 'people endpoint uses Admin SDK pagination by document id');
assert(peopleRoute.includes('safePlatformUser'), 'people endpoint returns safe platform user rows');
assert(safeRows.includes('function safePlatformUser') && safeRows.includes('workspaceIdsForPlatformUser') && safeRows.includes('countUniqueActivePushDevices'), 'safe platform user helper preserves workspace IDs and token counts');
const safePlatformBody = safeRows.slice(safeRows.indexOf('function safePlatformUser'));
assert(!/fcmToken\s*:|fcmTokens\s*:|pushTokens\s*:|pushDevices\s*:/.test(safePlatformBody), 'safePlatformUser does not return raw push token fields');
assert(management.includes('/api/system-admin/people') && management.includes('loadSystemAdminPeopleRoster'), 'active management component loads authoritative people roster');
assert(management.includes("subTab === 'tenants' || subTab === 'push'"), 'server people loader is used for tenants and push');
assert(management.includes("subTab === 'users' || subTab === 'live'") && !management.includes("subTab === 'users' || subTab === 'push' || subTab === 'live'"), 'direct browser users listener is restricted to users/live and excludes push');
assert(management.includes('getSystemAdminUserWorkspaceIds(u).includes(selectedClient.id)'), 'Client Users modal matches primary or secondary workspace memberships');
assert(management.includes('getSystemAdminUserWorkspaceIds(u).some(workspaceId => selectedPushRestaurantIds.includes(workspaceId))'), 'Push selected recipients use workspace membership helper');
assert(management.includes("getUserPushDeviceCount(u) > 0 ? 'On' : 'Off'"), 'Client Push display uses safe push device count');
assert(management.includes('Promise.all([loadSystemAdminWorkspaceRoster({ refreshing: true }), loadSystemAdminPeopleRoster({ refreshing: true })])'), 'Client refresh reloads workspaces and people roster together');
assert(management.includes('pushDeviceCount') && management.includes('pushLastSyncAt'), 'Push Control Center uses server safe count and last sync fields');

if (failures) { console.error(`\n${failures} validation check(s) failed.`); process.exit(1); }
console.log('\n16.0.168 source validation passed.');
