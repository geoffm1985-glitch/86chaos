...
assert(pkg.version === '16.0.210', 'package.json version is 16.0.210');
assert(lock.version === '16.0.210' && lock.packages?.['']?.version === '16.0.210', 'package-lock root versions are 16.0.210');
assert(pkg.scripts['test:source'] === 'node scripts/validate-16-0-210.js', 'test:source points to 16.0.210 validator');
assert(version.version === '16.0.210' && version.build === '16.0.210', 'public/version.json version/build are 16.0.210');
assert(version.releaseTitle === 'Failed-Only Archive Test Determinism Repair', '16.0.210 release title identifies the archive-only test repair');
assert(appCore.includes("CURRENT_VERSION = '16.0.210'"), 'app core CURRENT_VERSION is 16.0.210');
assert(apiVersion.includes("APP_VERSION = '16.0.210'") && apiVersion.includes("SECURITY_SCHEMA_VERSION = '16.0.210'"), 'api version reports 16.0.210');

assert(requestOffSpec.includes('return fixture.anchor || fixture.currentWeekStart || overCoverageDate'), 'Request Off focused clock uses the seeded anchor before later coverage dates');
assert(requestOffSpec.includes("page.locator('.request-off-workflow-panel div.font-black.text-white.text-sm')"), 'Request Off readiness targets the rendered workflow request card instead of dropdown text');
...
assert((archiveOnlyBlock.match(/waitForRequestOffEmployee\(page, 'Allen QA'/g) || []).length >= 2, 'archive-only browser test waits for the actual Allen row before and after employee filtering');
...
assert(schedule.includes('role="tablist" aria-label="Schedule Builder tools"') && schedule.includes('role="tab" aria-label={label} title={label}'), 'Schedule Builder tool controls use valid tablist/tab ARIA semantics');
assert(auditHelpers.includes('Workspace chooser still visible after selecting') && auditHelpers.includes('candidateButtons') && auditHelpers.includes('expectVersion(page, expected = EXPECTED_VERSION)'), 'login/version helper waits through workspace chooser selection before version assertions');
assert(exhaustiveHelper.includes('firstVisibleFromLocator') && exhaustiveHelper.includes('const globalFormProbeRegistry = new Set()') && exhaustiveHelper.includes('const globalMutationActionabilityRegistry = new Set()'), 'exhaustive helper chooses visible duplicate controls and de-duplicates repeated form/mutation probes');
assert(routeStateGraphSpec.includes('removing the release-gate timeout caused by hundreds of redundant route transitions') && !routeStateGraphSpec.includes('Always start each state from a clean route surface'), 'route-state graph avoids redundant same-route dispatch before every declared state');
assert(responsiveSpec.includes('const routeText=await gotoTab(page,route.tab') && responsiveSpec.includes('const routeGated=PERMISSION_GATE_RE.test(routeText)'), 'responsive matrix settles each route once per viewport before walking all declared states');
assert(nestedAccessibilitySpec.includes('const routeText=await gotoTab(page,route.tab') && nestedAccessibilitySpec.includes('const routeGated=PERMISSION_GATE_RE.test(routeText)'), 'nested accessibility settles each route once before walking declared states');
...
assert(fs.readFileSync(path.join(root, 'tests/86chaos-full-audit/utils/audit-helpers.cjs'), 'utf8').includes('Login did not leave the login screen'), '16.0.209 login helper fails explicitly if still on login');
assert(fs.readFileSync(path.join(root, 'tests/86chaos-release-gate/utils/exhaustive-ui-helpers.cjs'), 'utf8').includes('openResponsiveStateDirectory'), '16.0.209 state helper opens responsive directory before missing state');
assert(fs.readFileSync(path.join(root, 'tests/86chaos-release-gate/28-exhaustive-route-state-control-graph.spec.cjs'), 'utf8').includes('const expensiveProbe = stateIndex === 0'), '16.0.209 exhaustive graph avoids repeated expensive probes');
assert(fs.readFileSync(path.join(root, 'src/features/schedule.jsx'), 'utf8').includes('aria-label="Full schedule shift list" tabIndex={0}'), '16.0.209 full schedule scroll region is keyboard focusable');

assert(fs.readFileSync(path.join(root, 'tests/e2e/schedule-request-off-management.spec.cjs'), 'utf8').includes("getByRole('tab', { name: /^Warnings$/i })"), '16.0.209 Request Off warning test targets Warnings tab semantics');
assert(fs.readFileSync(path.join(root, 'src/core/scheduleWarningControls.shared.js'), 'utf8').includes('if (workspaceId && requestWorkspace && requestWorkspace !== workspaceId) return false;'), '16.0.209 Request Off bulk eligibility allows visible legacy rows without workspace metadata while rejecting explicit wrong workspace');

if (failures) {
  console.error(`16.0.210 source validation failed with ${failures} failure(s).`);
  process.exit(1);
}
console.log('16.0.210 source validation passed.');
